...
offline_builder_expected=$(metadata_value CROWDSEC_OFFLINE_STARTUP_BUILDER_SHA256)
docker_start_expected=$(metadata_value CROWDSEC_DOCKER_START_SHA256)
offline_startup_expected=$(metadata_value CROWDSEC_OFFLINE_STARTUP_SHA256)
check_file_hash "$offline_builder_expected" \
  "$app_dir/files/appsec/prepare-offline-startup.sh" 'offline startup builder'
...
cmp -s "$app_dir/files/appsec/rules/crowdsec-01-exact-913100-exception.yaml" \
  "$scratch/rendered-exception.yaml"
decode_rendered_binary prepare-offline-startup.sh "$scratch/rendered-offline-startup-builder.sh"
cmp -s "$app_dir/files/appsec/prepare-offline-startup.sh" \
  "$scratch/rendered-offline-startup-builder.sh"
...
for empty_hub_env in COLLECTIONS PARSERS SCENARIOS POSTOVERFLOWS APPSEC_CONFIGS APPSEC_RULES; do
  rg -Uq "name: ${empty_hub_env}[[:space:]]+value: \"\"" "$rendered"
done
rg -q "${docker_start_expected}" "$rendered"
rg -q "${offline_startup_expected}" "$rendered"
rg -q "${offline_builder_expected}" "$rendered"
rg -q 'exec /offline-startup/docker_start.sh' "$rendered"
...
download_and_check "$(metadata_value CROWDSEC_DOCKER_START_SOURCE_URL)" \
  "$docker_start_expected" docker_start.sh
sh "$app_dir/files/appsec/prepare-offline-startup.sh" \
  "$scratch/docker_start.sh" "$scratch/docker_start-offline.sh" \
  "$docker_start_expected" "$offline_startup_expected" >/dev/null
...
printf '%s\n' 'PASS: source archive, offline startup, plugin registry archive, pinned Hub YAML과 Hub data 49개가 고정 hash와 일치합니다.'
