#!/usr/bin/env python3
"""QUALITY-01 Unbound sonar alias 한 건만 지원 API로 조회·적용·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "domain": "imcherry5778.xyz",
    "description": (
        "SonarQube Pomerium browser UI only; scanner uses ClusterIP (QUALITY-01)"
    ),
}
LEGACY_ALIAS_DESCRIPTION = "SonarQube Pomerium browser UI alias (QUALITY-01)"
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("sonar alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("QUALITY-01 Unbound: k3s-01 precondition=match, sonar alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing sonar alias differs from the QUALITY-01 declaration")
        print(
            "QUALITY-01 Unbound: k3s-01 precondition=match, "
            f"sonar alias=match, uuid={alias['uuid']}"
        )
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            expected_without_description = {
                key: value
                for key, value in EXPECTED_ALIAS.items()
                if key != "description"
            }
            if (
                alias.get("host") != host_uuid
                or not exact_match(alias, expected_without_description)
                or alias.get("description")
                not in {EXPECTED_ALIAS["description"], LEGACY_ALIAS_DESCRIPTION}
            ):
                raise RuntimeError("existing sonar alias differs; refusing to overwrite")
            if exact_match(alias, EXPECTED_ALIAS):
                print(
                    "QUALITY-01 Unbound: sonar alias already matches, "
                    f"uuid={alias['uuid']}"
                )
                return
            alias_uuid = alias.get("uuid")
            if not alias_uuid:
                raise RuntimeError("existing sonar alias UUID is missing")
            payload = {"alias": {**EXPECTED_ALIAS, "host": host_uuid}}
            status, body = client.post(
                f"/api/unbound/settings/set_host_alias/{alias_uuid}", payload
            )
            if status != 200 or body.get("result") != "saved":
                raise RuntimeError("set_host_alias did not return saved")
            ...
            if host_uuid_after != host_uuid or len(aliases_after) != 1:
                raise RuntimeError("updated sonar alias verification count mismatch")
            ...
            if (
                ...
                raise RuntimeError("updated sonar alias differs from the declaration")
            print(
                "QUALITY-01 Unbound: sonar alias metadata updated, "
                f"uuid={alias_uuid}"
            )
            return
        payload = {"alias": {**EXPECTED_ALIAS, "host": host_uuid}}
        ...
        host_uuid_after, aliases_after = preflight(client)
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("sonar alias verification count mismatch")
        alias = aliases_after[0]
        if (
            alias.get("uuid") != alias_uuid
            or alias.get("host") != host_uuid
            ...
            raise RuntimeError("saved sonar alias differs from the declaration")
        print(f"QUALITY-01 Unbound: sonar alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target sonar alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
        raise RuntimeError("rollback target differs from the exact QUALITY-01 alias")
    alias_uuid = alias.get("uuid")
    if not alias_uuid:
    ...
    if aliases_after:
        raise RuntimeError("sonar alias still exists after rollback")
    print(f"QUALITY-01 Unbound: sonar alias rolled back, uuid={alias_uuid}")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"QUALITY-01 Unbound failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        raise SystemExit(1)
