#!/usr/bin/env bash
# SIGN-01 적용 직전 k3s RAM 정지선만 판정한다.
set -Eeuo pipefail
...
if available < 8192:
    raise SystemExit("SIGN-01 capacity stop: k3s available memory < 8 GiB")
if swap != 0:
    raise SystemExit("SIGN-01 capacity stop: k3s swap is in use")

for key in sorted(values):
    print(f"{key}={values[key]}")
print(f"capacity_warning_band={'yes' if available < 12288 else 'no'}")
print("SIGN-01 capacity stop/go=GO")
PY
