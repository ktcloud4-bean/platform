#!/usr/bin/env python3
"""AWX-01 Unbound awx alias 한 건만 지원 API로 조회·적용·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "awx",
    "domain": "imcherry5778.xyz",
    "description": "AWX GitOps automation alias (AWX-01)",
}
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("awx alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("AWX-01 Unbound: k3s-01 precondition=match, awx alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing awx alias differs from the AWX-01 declaration")
        print(
            "AWX-01 Unbound: k3s-01 precondition=match, "
            f"awx alias=match, uuid={alias['uuid']}"
        )
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing awx alias differs; refusing to overwrite")
            print(f"AWX-01 Unbound: awx alias already matches, uuid={alias['uuid']}")
            return
        ...
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("awx alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved awx alias differs from the declaration")
        print(f"AWX-01 Unbound: awx alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target awx alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
    ...
    if aliases_after:
        raise RuntimeError("awx alias still exists after rollback")
    print(f"AWX-01 Unbound: awx alias rolled back, uuid={alias_uuid}")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"AWX-01 Unbound failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        raise SystemExit(1)
