#!/usr/bin/env node
/* AWX-07의 실제 OIDC session으로 privileged apply·approval·관리 권한 경계를 판정한다. */
'use strict';
...
function parseArgs(argv) {
  ...
  for (const key of ['connect-ip', 'daily-username', 'privileged-username',
    'daily-password-file', 'daily-totp-file', 'privileged-password-file',
    'privileged-totp-file', 'object-file']) {
...
async function api(page, method, path, body, expected) {
  ...
  if (!expected.includes(response.status)) fail(`${method} ${path}: HTTP ${response.status}, expected ${expected.join('/')}`);
  return response;
}
async function awxLogin(browser, username, passwordFile, totpFile) {
  const context = await browser.newContext({ignoreHTTPSErrors: false});
  const page = await context.newPage();
  await page.goto(AWX_URL, {waitUntil: 'domcontentloaded', timeout: 60000});
  if (new URL(page.url()).hostname === 'sso.imcherry5778.xyz') await keycloakLogin(page, username, passwordFile, totpFile);
  ...
  if (await oidcLink.isVisible({timeout: 15000}).catch(() => false)) await oidcLink.click();
  else await page.goto(`${AWX_URL}/sso/login/oidc/`, {waitUntil: 'domcontentloaded', timeout: 60000});
  if (new URL(page.url()).hostname === 'sso.imcherry5778.xyz' && await page.locator('input[name="username"]').isVisible({timeout: 2000}).catch(() => false)) {
    await keycloakLogin(page, username, passwordFile, totpFile);
  }
  await page.waitForURL((url) => url.hostname === 'awx.imcherry5778.xyz', {timeout: 60000});
  const me = await page.evaluate(async () => (await fetch('/api/v2/me/', {credentials: 'same-origin'})).json());
  if (me.count !== 1 || me.results[0].username !== username) fail(`AWX OIDC identity mismatch for ${username}`);
  return {context, page};
}
async function main() {
  ...
  try {
    const operator = await awxLogin(browser, args['daily-username'], args['daily-password-file'], args['daily-totp-file']);
    const approver = await awxLogin(browser, args['privileged-username'], args['privileged-password-file'], args['privileged-totp-file']);
    ...
    await api(operator.page, 'PATCH', `/api/v2/credentials/${objects.credential}/`, {name: 'AWX-07 netbird-01 node exporter'}, [403]);
    await api(approver.page, 'POST', `/api/v2/job_templates/${objects.check}/launch/`, {}, [403]);
    await api(approver.page, 'POST', `/api/v2/workflow_job_templates/${objects.workflow}/launch/`, {}, [403]);
    await api(approver.page, 'PATCH', `/api/v2/job_templates/${objects.apply}/`, {name: 'AWX-07 netbird node exporter apply'}, [403]);
    console.log('AWX07_RBAC=PASS operator_direct_apply_approval_credential_manage=403 approver_check_workflow_execute_template_manage=403');
...
main().catch((error) => { console.error(`AWX-07 browser verification failed: ${error.message}`); process.exit(1); });
