#!/usr/bin/env bash
# HEADLAMP-02 public PKCE Keycloak client를 check-first 방식으로만 추가한다.
...
사용법:
  ...
  ./gitops/tools/headlamp-02/provision-keycloak-client.sh --check|--apply

--check는 platform realm의 clientId=headlamp 객체를 안전한 비밀 제외 필드로만 비교한다.
...
readonly repo_root
readonly client_declaration=${repo_root}/gitops/tools/headlamp-02/keycloak-client.json
...
client_matches() {
  jq -e '
    ...
    .[0].redirectUris == ["https://headlamp.imcherry5778.xyz/oidc-callback"] and
    .[0].webOrigins == ["https://headlamp.imcherry5778.xyz"] and
...
curl_admin "${issuer}/admin/realms/platform/clients?clientId=headlamp" >"${clients_json}"
client_count=$(jq 'length' "${clients_json}")
case "${client_count}" in
  0)
    echo "HEADLAMP-02 Keycloak 차이: headlamp client 0건 -> 신규 public PKCE client 추가 대상"
    ;;
  1)
    if client_matches; then
      echo "HEADLAMP-02 Keycloak 차이: headlamp client 1건 -> 선언 일치"
    else
      safe_client_summary
      echo "live headlamp client가 HEADLAMP-02 선언과 다르다. 자동 보정하지 않는다." >&2
      exit 1
    fi
    ;;
  *)
    echo "clientId=headlamp live 객체가 ${client_count}건이다. 변경하지 않는다." >&2
    exit 1
    ;;
esac
...
if [[ "${client_count}" -eq 0 ]]; then
  ...
  if [[ "${http_status}" != 201 ]]; then
    echo "HEADLAMP-02 Keycloak client 생성 실패: HTTP ${http_status}" >&2
    exit 1
  fi
  echo "HEADLAMP-02: 기존 realm 객체를 수정하지 않고 headlamp public PKCE client 한 건을 추가했다."
fi

curl_admin "${issuer}/admin/realms/platform/clients?clientId=headlamp" >"${clients_json}"
client_matches || {
  echo "생성 후 headlamp client 선언 일치 검증에 실패했다." >&2
  exit 1
}
echo "HEADLAMP-02: Keycloak public PKCE client 선언 검증 통과"
