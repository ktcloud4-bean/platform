#!/usr/bin/env python3
"""HEADLAMP-02 Unbound headlamp alias 한 건만 지원 API로 조회·적용·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "headlamp",
    "domain": "imcherry5778.xyz",
    "description": "Headlamp Keycloak OIDC Kubernetes RBAC alias (HEADLAMP-02)",
}
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("headlamp alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("HEADLAMP-02 Unbound: k3s-01 precondition=match, headlamp alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing headlamp alias differs from the HEADLAMP-02 declaration")
        print(
            "HEADLAMP-02 Unbound: k3s-01 precondition=match, "
            f"headlamp alias=match, uuid={alias['uuid']}"
        )
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing headlamp alias differs; refusing to overwrite")
            print(f"HEADLAMP-02 Unbound: headlamp alias already matches, uuid={alias['uuid']}")
            return
        ...
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("headlamp alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved headlamp alias differs from the declaration")
        print(f"HEADLAMP-02 Unbound: headlamp alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target headlamp alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
        raise RuntimeError("rollback target differs from the exact HEADLAMP-02 alias")
    ...
    if aliases_after:
        raise RuntimeError("headlamp alias still exists after rollback")
    print(f"HEADLAMP-02 Unbound: headlamp alias rolled back, uuid={alias_uuid}")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"HEADLAMP-02 Unbound failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        raise SystemExit(1)
