...
function requestWrongAudienceToken(args) {
  ...
  return currentTotp(args['totp-file']).then((code) => new Promise((resolve, reject) => {
    ...
    const request = https.request({
      ...
      lookup: (_hostname, _options, callback) => callback(null, args['connect-ip'], 4),
    }, (response) => {
      let body = '';
...
async function verifyWrongAudienceDeny(page, args) {
  ...
  if (!token) return;
  try {
    const response = await proxyRequest(page, '/api/v1/namespaces', {
      headers: { Authorization: `Bearer ${token}` },
    });
    expectStatus('DENY_WRONG_AUDIENCE', response.status, 401);
  } finally {
    token = '';
  }
}
...
async function headlampToken(context, args) {
  const cookies = await context.cookies(HEADLAMP_URL);
...
async function main() {
  ...
  try {
    ...
    await verifyWrongAudienceDeny(page, args);
