#!/usr/bin/env python3
"""SUPPLY-05-FIX-04 policy를 immutable SHA에서 검증하고 literal main으로 복구한다."""
...
def restore(remote: Remote, main_sha: str) -> None:
    ...
    wait(remote, "wazuh", "main", main_sha)
...
def execute(task_sha: str, main_sha: str, tool_dir: Path) -> None:
    ...
    try:
        ...
        try:
            ...
            env = dict(os.environ)
            env["SUPPLY05FIX04_TASK_SHA"] = task_sha
            env["SUPPLY05FIX04_MAIN_SHA"] = main_sha
            result = subprocess.run([str(tool_dir / "verify-live.sh")], env=env, check=False)
            if result.returncode:
                raise SafeError("scoped live verification failed")
            print(f"SUPPLY05FIX04_ARGO_IMMUTABLE=PASS root={task_sha} child={CHILD}", flush=True)
        finally:
            if changed:
                restore(remote, main_sha)
                print(f"SUPPLY05FIX04_ARGO_RESTORE=PASS main={main_sha}", flush=True)
    finally:
...
def main() -> int:
    ...
    if not all(len(value) == 40 and all(c in "0123456789abcdef" for c in value) for value in (args.task_sha, args.main_sha)):
        print("SUPPLY05FIX04=FAIL reason=full lowercase SHA is required", file=sys.stderr)
        return 2
    try:
        execute(args.task_sha, args.main_sha, Path(__file__).resolve().parent)
    except SafeError as error:
        print(f"SUPPLY05FIX04=FAIL reason={error}", file=sys.stderr)
        return 1
    return 0
