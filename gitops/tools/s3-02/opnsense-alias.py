#!/usr/bin/env python3
"""S3-02 Unbound filer alias 한 건만 지원 API로 조회·적용·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "filer",
    "domain": "imcherry5778.xyz",
    "description": "SeaweedFS filer 웹 UI Pomerium Route alias (S3-02)",
}
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("filer alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("S3-02 Unbound: k3s-01 precondition=match, filer alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing filer alias differs from the S3-02 declaration")
        print(
            "S3-02 Unbound: k3s-01 precondition=match, "
            f"filer alias=match, uuid={alias['uuid']}"
        )
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing filer alias differs; refusing to overwrite")
            print(f"S3-02 Unbound: filer alias already matches, uuid={alias['uuid']}")
            return
        ...
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("filer alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved filer alias differs from the declaration")
        print(f"S3-02 Unbound: filer alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target filer alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
    ...
    if aliases_after:
        raise RuntimeError("filer alias still exists after rollback")
    print(f"S3-02 Unbound: filer alias rolled back, uuid={alias_uuid}")
