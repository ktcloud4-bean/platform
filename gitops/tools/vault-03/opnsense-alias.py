#!/usr/bin/env python3
"""VAULT-03 Unbound vault alias 한 건만 지원 API로 조회·적용·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "vault",
    "domain": "imcherry5778.xyz",
    "description": "Vault standard Ingress alias (VAULT-03)",
}
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("vault alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("VAULT-03 Unbound: k3s-01 precondition=match, vault alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing vault alias differs from the VAULT-03 declaration")
        print(
            "VAULT-03 Unbound: k3s-01 precondition=match, "
            f"vault alias=match, uuid={alias['uuid']}"
        )
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing vault alias differs; refusing to overwrite")
            print(f"VAULT-03 Unbound: vault alias already matches, uuid={alias['uuid']}")
            return
        ...
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("vault alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved vault alias differs from the declaration")
        print(f"VAULT-03 Unbound: vault alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target vault alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
        raise RuntimeError("rollback target differs from the exact VAULT-03 alias")
    ...
    if aliases_after:
        raise RuntimeError("vault alias still exists after rollback")
    print(f"VAULT-03 Unbound: vault alias rolled back, uuid={alias_uuid}")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"VAULT-03 Unbound failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        raise SystemExit(1)
