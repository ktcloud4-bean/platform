#!/usr/bin/env python3
"""REG-01 Unbound harbor alias 1건을 지원 API로만 조회·추가·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "harbor",
    "domain": "imcherry5778.xyz",
    "description": "Harbor UI and direct registry API alias (REG-01)",
}
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("harbor alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("REG-01 Unbound: k3s-01 precondition=match, harbor alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing harbor alias differs from the REG-01 declaration")
        print(
            f"REG-01 Unbound: k3s-01 precondition=match, harbor alias=match, uuid={alias['uuid']}"
        )
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing harbor alias differs; refusing to overwrite")
            print(f"REG-01 Unbound: harbor alias already matches, uuid={alias['uuid']}")
            return
        ...
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("harbor alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved harbor alias differs from the declaration")
        print(f"REG-01 Unbound: harbor alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target harbor alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
        raise RuntimeError("rollback target differs from the exact REG-01 alias")
    ...
    if aliases_after:
        raise RuntimeError("harbor alias still exists after rollback")
    print(f"REG-01 Unbound: harbor alias rolled back, uuid={alias_uuid}")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"REG-01 Unbound failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        raise SystemExit(1)
