#!/usr/bin/env python3
"""SCM-01 Unbound git alias 1건을 지원 API로 조회·추가·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "git",
    "domain": "imcherry5778.xyz",
    "description": "Gitea UI alias (SCM-01)",
}
...
def load_env(path: Path) -> dict[str, str]:
    ...
    if metadata.st_uid != os.getuid() or metadata.st_mode & 0o077:
        raise RuntimeError("env file must be owned by caller with group/other mode 0")
    values: dict[str, str] = {}
...
class Client:
    def __init__(self, values: dict[str, str]):
        ...
        token = base64.b64encode(
            f"{values['OPN_KEY']}:{values['OPN_SECRET']}".encode()
        ).decode()
        self.headers = {
        ...
        if values.get("OPN_CACERT"):
            self.context.load_verify_locations(cafile=values["OPN_CACERT"])
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("git alias is not unique")
    return hosts[0]["uuid"], aliases
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("SCM-01 Unbound: k3s-01 precondition=match, git alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing git alias differs from SCM-01 declaration")
        print(f"SCM-01 Unbound: git alias=match, uuid={alias['uuid']}")
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing git alias differs; refusing to overwrite")
            print(f"SCM-01 Unbound: git alias already matches, uuid={alias['uuid']}")
            return
        ...
        if status != 200 or body.get("result") != "saved" or not body.get("uuid"):
            raise RuntimeError("add_host_alias did not return saved with UUID")
        alias_uuid = body["uuid"]
        reconfigure(client)
        host_uuid_after, aliases_after = preflight(client)
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("git alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved git alias differs from declaration")
        print(f"SCM-01 Unbound: git alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target git alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
        raise RuntimeError("rollback target differs from exact SCM-01 alias")
    ...
    if aliases_after:
        raise RuntimeError("git alias still exists after rollback")
    print(f"SCM-01 Unbound: git alias rolled back, uuid={alias_uuid}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"SCM-01 Unbound failed: {type(error).__name__}: {error}", file=sys.stderr)
        raise SystemExit(1)
