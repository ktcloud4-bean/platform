#!/usr/bin/env python3
"""CI-01 Unbound jenkins alias 1건을 지원 API로만 조회·추가·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "jenkins",
    "domain": "imcherry5778.xyz",
    "description": "Jenkins controller UI alias (CI-01)",
}
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("jenkins alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("CI-01 Unbound: k3s-01 precondition=match, jenkins alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing jenkins alias differs from the CI-01 declaration")
        print(
            f"CI-01 Unbound: k3s-01 precondition=match, jenkins alias=match, uuid={alias['uuid']}"
        )
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing jenkins alias differs; refusing to overwrite")
            print(f"CI-01 Unbound: jenkins alias already matches, uuid={alias['uuid']}")
            return
        ...
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("jenkins alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved jenkins alias differs from the declaration")
        print(f"CI-01 Unbound: jenkins alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target jenkins alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
        raise RuntimeError("rollback target differs from the exact CI-01 alias")
    ...
    if aliases_after:
        raise RuntimeError("jenkins alias still exists after rollback")
    print(f"CI-01 Unbound: jenkins alias rolled back, uuid={alias_uuid}")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"CI-01 Unbound failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        raise SystemExit(1)
