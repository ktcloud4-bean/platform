#!/usr/bin/env bash
# capacity-plan.md의 CI-01 배포 직후 stop/go를 한 번 판정한다.
set -Eeuo pipefail
...
sudo -n /usr/local/bin/k3s kubectl get pvc -A -o json | jq -r '
  ...
  | "k3s_pvc_request_gib=\(.)"
'
printf 'jenkins_pvc_count='; sudo -n /usr/local/bin/k3s kubectl -n jenkins get pvc -o json | jq '.items | length'
printf 'k3s_disk_pressure='; sudo -n /usr/local/bin/k3s kubectl get nodes -o json | jq -er '
  if (.items | length) != 1 then error("CI-01은 단일 k3s node 기준선만 판정한다")
...
checks = [
    ...
    (float(k3s["k3s_swap_used_mib"]) == 0, "k3s swap is in use"),
    (float(k3s["k3s_root_used_percent"]) < 80, "k3s root >= 80%"),
    (float(k3s["k3s_pvc_request_gib"]) < 120, "PVC request >= 120 GiB"),
    (int(k3s["jenkins_pvc_count"]) == 1, "Jenkins PVC 선언이 JENKINS_HOME 한 개가 아니다"),
    (k3s["k3s_disk_pressure"] == "False", "k3s DiskPressure=True"),
]
for passed, reason in checks:
    if not passed:
        raise SystemExit(f"CI-01 capacity stop: {reason}")
merged = {**pve, **k3s}
for key in sorted(merged):
    print(f"{key}={merged[key]}")
warn = float(k3s["k3s_pvc_request_gib"]) >= 96 or float(k3s["k3s_memory_available_mib"]) < 12288
print(f"capacity_warning_band={'yes' if warn else 'no'}")
PY

echo "CI-01 capacity-plan stop criteria=pass, stop/go=GO"
