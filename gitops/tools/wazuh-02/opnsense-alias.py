#!/usr/bin/env python3
"""WAZUH-02 Unbound `wazuh` alias 한 건을 지원 API로만 조회·적용·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "wazuh",
    "domain": "imcherry5778.xyz",
    "description": "WAZUH-02 Wazuh Dashboard alias",
}
...
def preflight(client: Client):
    ...
    if len(candidates) > 1:
        raise RuntimeError("wazuh alias is not unique")
    matched = None
    if candidates:
        alias = candidates[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing wazuh alias differs from the WAZUH-02 declaration")
        matched = alias
...
def main():
    ...
    if args.mode == "check":
        print(f"WAZUH-02 Unbound: k3s-01 precondition=match, wazuh={'match' if matched else 'absent'}")
        return
    ...
    if args.mode == "apply":
        if matched is None:
            ...
            if status != 200 or body.get("result") != "saved" or not body.get("uuid"):
                raise RuntimeError("add_host_alias failed for wazuh")
            reconfigure(client)
            host_uuid_after, matched_after = preflight(client)
            if host_uuid_after != host_uuid or matched_after is None:
                raise RuntimeError("WAZUH-02 alias verification failed after apply")
        print("WAZUH-02 Unbound: wazuh alias applied or already matched")
        return

    if matched is None:
        raise RuntimeError("rollback target alias is absent: wazuh")
    alias_uuid = matched.get("uuid")
    if not alias_uuid:
        raise RuntimeError("wazuh rollback UUID is missing")
    ...
    if status != 200 or body.get("result") not in {"deleted", "saved"}:
        raise RuntimeError("del_host_alias did not succeed for wazuh")
    reconfigure(client)
    _, matched_after = preflight(client)
    if matched_after is not None:
        raise RuntimeError("WAZUH-02 wazuh alias still exists after rollback")
    print("WAZUH-02 Unbound: wazuh alias rolled back")
...
if __name__ == "__main__":
    try:
        ...
        print(f"WAZUH-02 Unbound failed: {type(error).__name__}: {error}", file=sys.stderr)
        raise SystemExit(1)
