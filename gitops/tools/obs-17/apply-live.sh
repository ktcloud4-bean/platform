#!/usr/bin/env bash
# OBS-17을 immutable Argo SHA에서 검증한 뒤 literal main으로 반드시 복구한다.
set -Eeuo pipefail
...
readonly main_revision=${OBS17_MAIN_REVISION:?시작 main SHA가 필요하다}
readonly config_revision=${OBS17_CONFIG_REVISION:?obs 설정 commit SHA가 필요하다}
readonly root_revision=${OBS17_ROOT_REVISION:?immutable root pointer SHA가 필요하다}
...
fail() {
  local stage=$1
  shift
  echo "OBS-17 적용 실패 단계=${stage} 원인=$*" >&2
  exit 1
}
...
main_state_matches() {
  remote_kubectl -n argocd get applications.argoproj.io platform-root obs -o json | jq -e --arg main "${main_revision}" '
    ...
    $root_app.spec.source.targetRevision == "main" and $root_app.status.sync.revision == $main and
    $root_app.status.sync.status == "Synced" and $root_app.status.health.status == "Healthy" and
    $obs_app.spec.source.targetRevision == "main" and $obs_app.status.sync.revision == $main and
...
wait_for_main_restore() {
  ...
  echo 'OBS-17 적용 실패 단계=rollback 원인=platform-root/obs가 main에서 Synced/Healthy로 복구되지 않았다.' >&2
  return 1
}
...
OBS17_EXPECTED_CONFIG_REVISION="${config_revision}" \
OBS17_EXPECTED_ROOT_REVISION="${root_revision}" \
  "${repo_root}/gitops/tools/obs-17/verify-live.sh"
