#!/usr/bin/env python3
"""POM-01 Unbound access alias 1건을 지원 API로만 조회·추가·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "access",
    "domain": "imcherry5778.xyz",
    "description": "Pomerium protected Dashy Portal alias (POM-01)",
}
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("access alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("POM-01 Unbound: k3s-01 precondition=match, access alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing access alias differs from the POM-01 declaration")
        print(
            f"POM-01 Unbound: k3s-01 precondition=match, access alias=match, uuid={alias['uuid']}"
        )
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing access alias differs; refusing to overwrite")
            print(f"POM-01 Unbound: access alias already matches, uuid={alias['uuid']}")
            return
        ...
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("access alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved access alias differs from the declaration")
        print(f"POM-01 Unbound: access alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target access alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
        raise RuntimeError("rollback target differs from the exact POM-01 alias")
    ...
    if aliases_after:
        raise RuntimeError("access alias still exists after rollback")
    print(f"POM-01 Unbound: access alias rolled back, uuid={alias_uuid}")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"POM-01 Unbound failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        raise SystemExit(1)
