#!/usr/bin/env python3
"""BOARD-DEMO-01 Unbound board alias 한 건을 지원 API로만 관리한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "board",
    "domain": "imcherry5778.xyz",
    "description": "Pomerium protected Board Demo alias (BOARD-DEMO-01)",
}
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("board alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("BOARD-DEMO-01 Unbound: k3s-01 precondition=match, board alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing board alias differs from the BOARD-DEMO-01 declaration")
        print("BOARD-DEMO-01 Unbound: k3s-01 precondition=match, board alias=match")
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing board alias differs; refusing to overwrite")
            print("BOARD-DEMO-01 Unbound: board alias already matches")
            return
        ...
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("board alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved board alias differs from the declaration")
        print("BOARD-DEMO-01 Unbound: board alias applied")
        return

    if not aliases:
        raise RuntimeError("rollback target board alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
        raise RuntimeError("rollback target differs from the exact BOARD-DEMO-01 alias")
    ...
    if aliases_after:
        raise RuntimeError("board alias still exists after rollback")
    print("BOARD-DEMO-01 Unbound: board alias rolled back")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"BOARD-DEMO-01 Unbound failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        raise SystemExit(1)
