#!/usr/bin/env python3
"""GITOPS-02 Unbound argo alias 한 건만 지원 API로 조회·적용·rollback한다."""
...
EXPECTED_ALIAS = {
    ...
    "hostname": "argo",
    "domain": "imcherry5778.xyz",
    "description": "Argo CD Pomerium Route alias (GITOPS-02)",
}
...
def preflight(client: Client):
    ...
    if len(aliases) > 1:
        raise RuntimeError("argo alias is not unique")
...
def main():
    ...
    if args.mode == "check":
        if not aliases:
            print("GITOPS-02 Unbound: k3s-01 precondition=match, argo alias=absent")
            return
        alias = aliases[0]
        if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
            raise RuntimeError("existing argo alias differs from the GITOPS-02 declaration")
        print(
            "GITOPS-02 Unbound: k3s-01 precondition=match, "
            f"argo alias=match, uuid={alias['uuid']}"
        )
        return
    ...
    if args.mode == "apply":
        if aliases:
            alias = aliases[0]
            if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
                raise RuntimeError("existing argo alias differs; refusing to overwrite")
            print(f"GITOPS-02 Unbound: argo alias already matches, uuid={alias['uuid']}")
            return
        ...
        if host_uuid_after != host_uuid or len(aliases_after) != 1:
            raise RuntimeError("argo alias verification count mismatch")
        ...
        if (
            ...
            raise RuntimeError("saved argo alias differs from the declaration")
        print(f"GITOPS-02 Unbound: argo alias applied, uuid={alias_uuid}")
        return

    if not aliases:
        raise RuntimeError("rollback target argo alias is absent")
    alias = aliases[0]
    if alias.get("host") != host_uuid or not exact_match(alias, EXPECTED_ALIAS):
        raise RuntimeError("rollback target differs from the exact GITOPS-02 alias")
    ...
    if aliases_after:
        raise RuntimeError("argo alias still exists after rollback")
    print(f"GITOPS-02 Unbound: argo alias rolled back, uuid={alias_uuid}")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"GITOPS-02 Unbound failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        raise SystemExit(1)
