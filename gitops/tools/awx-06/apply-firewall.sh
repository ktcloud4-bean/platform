#!/usr/bin/env bash
# AWX-06: AWX execution source(k3s-01)에서 netbird-01 TCP 22만 허용한다.
# Rules[new] sequence는 자동 재계산될 수 있으므로 NET-04 block 직전으로 이동한다.
set -euo pipefail

script_dir=$(cd "$(dirname "$0")" && pwd)
repo_root=$(git -C "$script_dir" rev-parse --show-toplevel)
# shellcheck source=lib-opnsense.sh
source "$repo_root/gitops/tools/loki-02/lib-opnsense.sh"
trap loki02_cleanup EXIT HUP INT TERM

action=''
if (( $# > 0 )); then action=$1; fi
readonly rule_names='marker'
...
rule_interface() {
  case $1 in marker) echo opt2 ;; *) loki02_fail "알 수 없는 rule: $1" ;; esac
}

rule_source() {
  case $1 in marker) echo 10.10.20.10 ;; *) loki02_fail "알 수 없는 rule: $1" ;; esac
}

rule_description() {
  case $1 in marker) echo 'AWX-06: k3s-01 AWX execution에서 netbird-01 SSH TCP 22 marker만 허용' ;; *) loki02_fail "알 수 없는 rule: $1" ;; esac
}

block_description() {
  case $1 in marker) echo 'NET-04: k3s-01에서 비공개·특수용 IPv4 기본 차단·기록' ;; *) loki02_fail "알 수 없는 rule: $1" ;; esac
}
...
owned_rules() {
  jq '[.[] | select(.description | startswith("AWX-06:"))]' <<<"$1"
}
...
validate_rule() {
  ...
  jq -e --arg enabled "$expected_enabled" --arg description "$description" --arg iface "$iface" --arg source "$source" '
    ...
    | ($m | length == 1) and
      ...
      ($m[0].destination_net == "10.10.40.10") and ($m[0].destination_port == "22") and
      ($m[0].log == "1")
...
apply_live() {
  ...
  jq -e 'length == 0' <<<"$(owned_rules "$rows_json")" >/dev/null || loki02_fail '기존 AWX-06 rule이 있어 apply를 중단한다.'
  for name in $rule_names; do anchor_uuid "$name" "$rows_json" >/dev/null; done

  install -d -m 700 /home/imcherry/.local/state-backups
  state_dir=$(mktemp -d /home/imcherry/.local/state-backups/awx-06-firewall-XXXXXXXX)
  chmod 700 "$state_dir"
  ...
  for name in $rule_names; do
    ...
    jq -n --arg description "$description" --arg iface "$iface" --arg source "$source" '{rule:{
      ...
      destination_net:"10.10.40.10", destination_port:"22", gateway:"", log:"1", description:$description
    }}' > "$body"
...
rollback_live() {
  ...
  [[ $state_dir == /home/imcherry/.local/state-backups/awx-06-firewall-* ]] || loki02_fail '명시적인 AWX-06 복구 지점이 필요하다.'
  ...
  jq -e 'length == 0' <<<"$(owned_rules "$rows_json")" >/dev/null || loki02_fail 'rollback 뒤에도 AWX-06 rule이 남아 있다.'
