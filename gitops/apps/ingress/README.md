...
`CROWDSEC-01`은 이 소유 경계 안에서 지원되는 같은 `HelmChartConfig`에 community
...
적용하지 않으며, 실패 시 plugin registration과 이 secret mount만 제거해 같은 packaged
image와 이 문서의 production 기준선을 회복한다.
