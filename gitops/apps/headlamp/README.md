...
root/child Application은 항상 `targetRevision: main`을 사용한다. 작업 branch나 특정
commit SHA를 Argo revision으로 쓰지 않는다. final 선언에는 `headlamp-reader`
ServiceAccount·ClusterRole·ClusterRoleBinding을 포함하지 않아 Argo가 정확한 bootstrap
resource만 prune한다.

이 제거는 실제 Keycloak OIDC·Pomerium·Kubernetes RBAC 검증과 rollback 준비가 끝난
`HEADLAMP-02` live gate에서만 진행한다. main 반영 뒤 검증 실패 시 public main을 다시
쓰지 않고 즉시 단일 revert로 bootstrap 기준선을 복구한다.
