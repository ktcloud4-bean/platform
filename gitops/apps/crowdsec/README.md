# CROWDSEC-01 CrowdSec AppSec PoC
...
- upstream test Pod의 floating `curl` image와 사용하지 않는 online updater ServiceAccount를
  렌더하지 않는다.
- read-only CRS ConfigMap을 init container가 SHA-256 검증 후 writable `emptyDir`에 복사하고,
  이를 CrowdSec의 Coraza `RootFS`인 `/var/lib/crowdsec/data`에 마운트한다. 이 경로가
...
단일 원본은 [`release-metadata.env`](release-metadata.env)와
[`crs-snapshot.SHA256`](crs-snapshot.SHA256)이다.
...
성능은 같은 client/backend에서 warm-up 뒤 control/WAF 각각 1,000 GET, concurrency 10,
3 round다. [`benchmark-live.sh`](../../../docs/evidence/crowdsec-01/benchmark-live.sh)는
Kubernetes를 읽기만 하며 다음 gate를 자동 판정한다.
...
1. `CROWDSEC-01` 선언 enablement squash commit을 `git revert`한 단일 rollback commit으로
