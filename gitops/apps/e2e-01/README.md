...
`verifyImages` Enforce를 소유한다. `policies/`의 cluster-wide validate 정책은 계속
`Audit`이며, 예외·만료·전역 Enforce 승격은 `POL-02`가 소유한다.
...
Agent init을 사용해 공개키 Secret 두 개와 registry pull Secret 하나를 namespace에 한 번
materialize한 뒤 삭제된다. 이는 지속 동기화 operator가 아니며 원문은 Git, Job 로그와 명령
인자에 넣지 않는다. [Kyverno v1.18 verifyImages](https://kyverno.io/docs/policy-types/cluster-policy/verify-images/overview/)는
Pod의 namespaced imagePullSecret을 registry credential로 사용하고,
`k8s://namespace/secret`의 `cosign.pub`를 trust input으로 읽는다.
Kyverno admission controller에는 이 namespace의 current/previous 공개키 Secret 이름 두 개에
대한 `get`만 허용하며 Secret 목록 조회와 registry Secret 읽기는 허용하지 않는다.
...
서명한다. tlog/Fulcio/Rekor를 사용하지 않는 SIGN-01 경계와 맞춰 `ignoreTlog: true`이며,
signature는 digest-only subject의 OCI 1.1 referrer다. 키 회전 뒤에는 이 작업의 provision을
다시 실행해 파생 trust를 갱신하고, previous 제거 시점은 `POL-02`가 결정한다.
...
| `0` | namespaced Kyverno Policy | 이 namespace의 Harbor release Pod만 Enforce |
...
namespace와 namespaced Policy·runtime Secret이 사라진 뒤 `AppProject/e2e-01`만 수동 삭제한다.
