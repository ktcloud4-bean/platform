...
이 디렉터리는 `KC-01`의 Keycloak server, ClusterIP Service와 표준
...
| `-1` | 없음 (bootstrap은 별도 수동 경계) | 상시 Keycloak Application은 runtime 리소스만 reconcile하며 완료 Job을 재생성하지 않는다 |
...
재실행하기 전에 DB와 임시 admin client 상태를 확인한다. 완료된 bootstrap Job은 이 Application의
정상 desired state에 남기지 않으며, 필요할 때만 [`gitops/apps/keycloak-bootstrap/README.md`](../keycloak-bootstrap/README.md)의
수동 경계를 사용한다. 실행 후 Job과 Pod는 증거를 남긴 뒤 정리하고, Keycloak DB 상태만 지속 상태로 취급한다.
