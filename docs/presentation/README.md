...
`온프레미스–AWS 하이브리드 보안 플랫폼` 발표(23장·낭독 약 14분 5초)의 산출물과 근거를 소유한다.
발표 자료는 `PRESENT-DECK-01` 이후 팀이 직접 23장으로 고쳤고, `PRESENT-SCRIPT-01`의 대본은 그
23장을 기준으로 한다. 백로그의 22장 구성 계약 문단은 stale이며 별도 보정이 필요하다.
...
| `ktcloud4-bean.pptx` | 23장 발표자료 (slide는 팀, speaker notes는 대본) | `PRESENT-DECK-01` (뼈대는 `PRESENT-DECK-SKELETON-01`) |
...
## 발표자료 (`ktcloud4-bean.pptx`)

`PRESENT-DECK-SKELETON-01`이 layout 8종·색·모티프·시간 배분을 만들었고, `PRESENT-DECK-01`이
같은 뼈대 위에 최종 문구·승인 아키텍처 이미지·실제 UI 증거를 채웠다.

이 아래 절들은 `PRESENT-DECK-01`이 만든 22장 시점의 기록이다. 이후 팀이 슬라이드를 직접 고쳐
현재 파일은 23장이며, 온프레미스 보안 도구 두 장과 AWS IRSA·CIEM·CSPM 세 장이 들어가고
시나리오·백업/복구·결론·Q&A 장이 빠졌다. 색·타이포·layout 규칙은 그대로 이어진다.
...
`PRESENT-DECK-01`이 잡은 22장 845초 배분은 슬라이드가 23장으로 바뀌면서 유효하지 않다.
장별 시간은 `presentation-script.md`의 시간 배분 표가 소유하며, 합계는 그대로 845초다.

### 슬라이드 계약

각 장의 `PURPOSE`·`MESSAGE`·`ASSET`·`EVIDENCE`는 `PRESENT-DECK-01`이 최종 내용으로 대체했고,
계약 원문은 speaker notes에 남겨 두었다가 `PRESENT-SCRIPT-01`이 대본으로 교체하면서 걷어냈다.
슬라이드 문구가 어떤 완료 증거에 기대는지는 백로그 `13. 발표 아키텍처·슬라이드·대본`이 소유한다.
...
- `PRESENT-DECK-01`은 승인 이미지 한 장을 8~11장에 같은 좌표로 배치하고, 1장과 21장 배경에는
  ...
  원본을 슬라이드에 따로 넣지 않았다.
- `PRESENT-SCRIPT-01`이 이 PPTX의 speaker notes 전부를 페이지별 대본으로 대체했다.
  `PURPOSE`/`MESSAGE`/`ASSET`/`EVIDENCE` 계약은 이제 notes에 없고, 어떤 완료 증거에 기대는지는
  백로그 `13. 발표 아키텍처·슬라이드·대본`과 이 문서의 source matrix가 소유한다.

## 발표 대본 (`presentation-script.md`)

대본이 원본이고 speaker notes는 그 사본이다. 대본을 고치면 notes도 같이 갱신한다.

- 발표자 한 명(고광근) 기준이며 낭독 4,785자·845초(14분 5초)다. 화면 전환과 호흡을 포함한
  1회 리허설은 14분 30초~15분이고, Q&A와 데모 영상은 포함하지 않는다.
- 장별 목표 시간은 대본 글자 수를 분당 340자로 나눈 값이다. 320자/분으로 느리게 읽으면
  14분 57초, 370자/분으로 빠르게 읽으면 12분 56초다. 시간을 추정으로 적지 않는다.
- 대본에는 슬라이드 화면에 있는 값만 넣는다. 화면 밖 근거는 `발표 중 주의`에 옮기고
  질문이 올 때 답한다.

### 발표 자료가 대본에 남긴 숙제

- 7~11장은 강조 위치만 다른 같은 그림이다. 7장에서 이 사실을 먼저 말하지 않으면 청중이
  매 장 그림을 새로 읽는다.
- 11장 그림 안 `조치는 사람 승인 뒤에만 실행된다`와 18장 CIEM의 자동 차단은 서로 다른 층위다.
  대본 18장 마지막 줄이 이 차이를 설명한다.
- 19장 CSPM 캡처에 AWS 계정 번호가 마스킹 없이 보인다. 심사 자리에서는 문제가 없지만
  외부 공개용으로 다시 쓸 때는 가려야 한다.
- 6장 간트의 4주차 종료일은 사용자 원본 로드맵의 `4주차(~8월 14일)`를 그대로 쓰고 있다.
  대본은 이 날짜를 읽지 않고 주차 단위로만 말한다.
