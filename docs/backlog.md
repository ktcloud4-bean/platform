...
| `DEMO-AWS-HR-01 READY` | HR System 세션 4의 결정론적 촬영 인터페이스를 기존 EKS 공급망 통제 위에 추가 (`gitops/tools/`, `docs/`, `docs/evidence/`) | `DEMO-PLAN-02`, `SUPPLY-07`, `SUPPLY-01-FIX-02`, `QUALITY-06` | 없음 | `DEMO-OPENING-01` | 통제된 inert 미서명 또는 tag-only ECR fixture와 현재 서명된 HR exact digest를 같은 HR Pod shape·namespace에서 server-side dry-run해 각각 `DENIED`·`ALLOWED`를 한 명령 attack/control/evidence로 판정하고, negative 요청 전후 신규 ReplicaSet·Pod 0건과 기존 `frontend`·`employee-service`·`hr-service` Ready·digest·UID 불변, 합성 identity의 HR 포털 read-only 200, Argo `hr-system` `Synced/Healthy`를 확인한다. 촬영 중 build·replication·GitOps digest 갱신·실제 Deployment patch·악성코드·실데이터·Secret 출력은 0건이고, 실패해도 생성 자원이 없는 reset/no-op 및 기존 온프레미스 fallback 경계를 문서화한다. |
| `DEMO-OPENING-01 BLOCKED` | 세션 1·2·3·5의 온프레미스 흐름과 세션 4의 HR System/EKS 흐름으로 데모 5편의 16:9 오프닝 이미지용 영문 프롬프트를 만들고, 사용자가 chatgpt.com에서 생성한 후보를 함께 보정해 세션별 최종 이미지 한 장을 선정한다 (Notion `데모 시나리오`) | `DEMO-RECOVERY-01`, `DEMO-AWS-HR-01` | 없음 | `DEMO-SCRIPT-01` | 세션을 한 편씩 진행해 영문 prompt v1 → 사용자 생성 후보 → 구성·기술 정확성 검토 → 수정 prompt 순서로 반복하고 사용자가 각 최종본을 승인한다. 프롬프트는 공통 가상 기업의 일관된 dark cinematic security infographic 디자인, 16:9·좌우 안전영역·한눈에 읽히는 단방향 흐름을 유지하되 각 편은 독립적으로 이해되어야 한다. 실제 등장하는 핵심 제품의 공식 SVG/PNG 원본과 공식 출처를 참조자료로 제공하고, 생성 모델이 로고·글자를 다시 그리게 하지 않는다. 모델이 아이콘을 왜곡하면 아이콘 없는 배경과 배치 공간을 생성한 뒤 편집에서 공식 원본을 그대로 합성한다. 각 Notion 페이지에 최종 영문 prompt·금지 조건·아이콘 출처·선정 이미지를 남기며 Secret·실데이터·가짜 UI·읽을 수 없는 생성 텍스트는 0건이다. |
