...
| `CI-01-FIX-02 READY` | Jenkins에 파이프라인 시각화 플러그인 `pipeline-graph-view`와 `pipeline-stage-view`를 추가해 stage 단위 성공·실패를 화면에서 읽히게 한다 (`gitops/apps/jenkins/plugins.txt`, `gitops/apps/jenkins/`) | `CI-01-FIX-01` | `ARGO-ROOT` | Jenkins 화면 가독성, 발표·데모 증거 | 현재 `plugins.txt` 64개에는 시각화 플러그인이 없어 job 화면에 Stage View가 없고 `Pipeline Steps`의 평문 목록만 남는다. 두 플러그인과 전이 의존성 버전을 `plugins.txt`에 exact 고정하고, 기존 pinned 방식과 PVC 재사용 규칙을 깨지 않는다. Blue Ocean은 upstream 유지보수가 끝나 채택하지 않는다. `ARGO-ROOT` 잠금 아래 최신 main SHA로 `jenkins` child를 검증해 `Synced/Healthy`와 controller 재기동 성공, 기존 job 5개와 credential·JCasC 선언 불변, `hr-system-image-build`에서 stage 단위 성공·실패가 화면에 표시됨을 확인한다. 공급망 digest 경계와 Kyverno admission을 완화하지 않으며 라이브 credential·Secret 변경 0건이다. |
