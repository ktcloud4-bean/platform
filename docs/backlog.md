...
| `AWS-ACCOUNT-SEC` | 계정 단위 보안 설정(Config recorder·GuardDuty·Security Hub·Access Analyzer·계정 차단 설정)의 변경 |
...
| `S3-02 DONE` | SeaweedFS admin 웹 UI(`object-admin.imcherry5778.xyz`)를 Pomerium Route로 노출해 `/platform-privileged`(`imcherry5778-admin`)만 접근하도록 제한 | `S3-01`, `POM-01` | `OPNSENSE-LIVE`, `ARGO-ROOT` | `object-01` 운영 가시성 | 신규 `seaweedfs-admin` systemd unit(자체 TLS `security.toml [https.admin]`·자체 로그인 `WEED_ADMIN_USER/PASSWORD`)을 선언, 신규 PF rule(`opt2` sequence `1000`, k3s-01 `10.10.20.10/32` 단일 source, TCP 23646)만 허용하고 master(9333)·volume(8080)·filer(8888, 변경 없음) 관리 포트는 loopback 유지, Unbound alias `object-admin.imcherry5778.xyz` → `k3s-01`(`10.10.20.10`) 등록과 `ip-plan.md` 노출 정의 갱신, Pomerium Route는 `/platform-privileged`만 allow하고 backend는 admin 자체 TLS(`tls_skip_verify`, `S3-01` leaf 재사용)로 연결, `pomerium`→`object-01` NetworkPolicy egress(TCP 23646) 명시, 미인증 요청이 Pomerium sign-in으로 302/307 redirect됨을 `object-admin.imcherry5778.xyz`와 k3s-01 Pod→`10.10.50.20:23646` 양쪽에서 실측, admin 자체 인증(`WEED_ADMIN_PASSWORD`) 성공(세션 쿠키 발급·`/admin` 200)과 실패(`Invalid credentials`) 양쪽을 object-01 loopback에서 실측, Let's Encrypt 실제 인증서 발급 확인, admin이 filer보다 넓은 관리 권한(volume 재배치·EC·maintenance worker·설정 변경)을 가진다는 한계를 runbook에 명시, `weed admin`이 한때 가졌던 `ALPHA` 라벨이 2026-03-05 PR #8525로 제거됐고 고정한 4.40(2026-07-20 배포)은 그 이후임을 SeaweedFS 공식 커밋 이력과 실제 기동 화면 양쪽으로 확인, 기존 S3 API 8333·filer loopback 경로·PF rule·credential 모델(라이브 identity 7개 semantic diff로 무변경 확인) 회귀 없음, 실행 중 발견한 `S3-01` role의 identity action 정규식이 라이브 `loki-01`의 `Delete` action을 허용하지 않던 드리프트를 정규식 확장으로 함께 보정, `ARGO-ROOT` 잠금 아래 `platform-root`·`pomerium` 커밋 SHA 검증 후 main·Synced/Healthy 복귀 확인, rollback은 Pomerium Route·NetworkPolicy·Ingress host·Unbound alias·OPNsense PF rule·`seaweedfs-admin` unit·`security.toml`·`admin.env` 제거. Keycloak SSO를 통한 `imcherry5778-admin`의 실제 브라우저 로그인·Dashy 타일 노출은 사용자가 직접 확인 |
| `S3-02-FIX-01 DONE` | 사용자가 Keycloak SSO로 들어가도 weed admin 자체 로그인 화면이 다시 뜨는 이중 게이트를 제거하고 서비스 도메인을 `seaweedfs.imcherry5778.xyz`로 정리 | `S3-02` | `OPNSENSE-LIVE`, `ARGO-ROOT` | `object-01` 운영 가시성 | weed admin은 SSO를 지원하지 않음을 `weed admin -h`로 재확인(OIDC 관련 옵션 0개), `seaweedfs_admin_password`를 빈 값(기본값)으로 되돌려 admin 자체 로그인을 비활성화하고 Ansible assert를 "빈 값 또는 16자 이상"으로 완화, Pomerium `/platform-privileged` claim 하나만 인증 경계로 남김, 서비스 endpoint를 `object-admin.imcherry5778.xyz`에서 `seaweedfs.imcherry5778.xyz`로 변경(Unbound alias rollback 뒤 재등록, Pomerium Route 이름을 `seaweedfs-admin`으로 통일, Ingress host·TLS·Dashy 타일 URL 갱신), 인증 비활성화 뒤 backend 직접 요청이 로그인 화면 없이 dashboard HTTP 200을 바로 반환함을 object-01 loopback에서 확인, `seaweedfs.imcherry5778.xyz`로의 미인증 요청은 여전히 Pomerium sign-in redirect(Pomerium 게이트 자체는 불변)임을 실측, `ARGO-ROOT` 잠금 아래 `platform-root`·`pomerium` 커밋 SHA 검증 후 main·Synced/Healthy 복귀 확인 |
...
실행 중 filer 대신 SeaweedFS 다섯 번째 컴포넌트 `weed admin`(cluster 관리 콘솔)으로
대상을 바꿨다. admin은 한때 `ALPHA` 라벨이 있었지만 SeaweedFS 원작자(chrislusf)가
2026-03-05 PR #8525로 제거했고, 이 작업이 고정한 릴리스 4.40(2026-07-20 배포)은 그
이후라 실물 배포본에서 라벨이 없음을 직접 기동해 확인했다. filer는 인증이 전혀 없어
Pomerium 세션 하나가 전체 filer 네임스페이스에 all-or-nothing으로 접근했지만, admin은
자체 로그인(`WEED_ADMIN_USER`/`PASSWORD`)과 자체 TLS(`security.toml`)를 가져 그 공백을
메운다. 대신 admin은 volume 재배치·EC·maintenance worker·설정 변경까지 다루는 더 넓은
권한을 가지므로, 최종 트레이드오프는 "인증 계층 추가"와 "권한 범위 확대"를 맞바꾼
것이다. filer는 이 작업으로 바뀌지 않고 계속 loopback 전용으로 남는다. 라이브 검증
중 `S3-01` role의 identity action 정규식이 이미 라이브에 있던 `loki-01`의 `Delete`
action을 허용하지 않아 role 실행 자체가 막히는 것을 발견해(role 밖에서 부여된
드리프트), 정규식에 `Delete`를 추가해 함께 보정했다. 따라서 `S3-02`를 완료한다.
...
| `BKP-06 DONE` | 조용히 실패 중이던 PostgreSQL·Vault 네이티브 백업 두 건을 원인 특정 후 복구 | `BKP-03` | 없음 | 인증·플랫폼 데이터 RPO | PostgreSQL: `pg_basebackup`이 PGDATA 전체를 복사하다 `server-ca.key`(root:root 0600, WG-04가 CA 이전 시 root로 남김)를 못 읽어 8/12부터 실패하던 것을 `postgres_baseline` role에 소유자 강제 task를 추가해 postgres:postgres로 고정(라이브 적용·재실행 성공·`pg_verifybackup` 통과 확인). Vault: `vault-raft-backup.service`의 `IPAddressAllow`가 배포 시점 Vault pod IP(`10.42.0.189`)를 고정 값으로 갖고 있었는데 pod가 재스케줄되며(`10.42.0.53`) 8/8부터 매일 connection timeout으로 실패하던 것을, `vault_raft_backup` role의 기존 live-lookup 메커니즘을 그대로 재실행해 IPAddressAllow를 현재 pod IP로 갱신(라이브 재실행 후 여전히 403 확인 → periodic snapshot token 자체가 갱신 실패로 만료된 것을 추가로 발견 → 기존 `infra/vault/scripts/configure-bkp03-snapshot.sh`로 root token을 일회성으로만 써서 동일 read-only policy(`bkp-03-snapshot`)의 새 orphan periodic token 발급). 두 경로 모두 실제 신규 snapshot object 생성과 세대 보존(7세대) rotation, freshness health check 정상 복귀를 확인했다. Postgres DB role 비밀번호나 다른 postgres_baseline 선언은 건드리지 않도록 해당 task 하나만 별도 실행했다 |
| `BKP-07 DONE` | Velero 정기 backup schedule 신설과 네이티브 백업 3종의 Prometheus 실패 경보 연결 | `BKP-02`, `BKP-06`, `OBS-17` | `ARGO-ROOT` | `object-01` 운영 가시성, 인증·플랫폼 데이터 RPO | `gitops/apps/velero/values-bkp-02.yaml`에 다른 네이티브 백업과 겹치지 않는 02:00 KST 매일 schedule(`velero-daily`, TTL 7일) 추가, `velero` AppProject의 `namespaceResourceWhitelist`에 `velero.io/Schedule`이 빠져 있던 것을 발견해 함께 보정. 라이브로 시험한 결과 `defaultVolumesToFsBackup: true`는 emptyDir·Secret 마운트까지 포함한 cluster 전체 거의 모든 volume에서 Kopia FS backup이 실패(112 errors, `PartiallyFailed`)해 BKP-02의 단일 PVC PoC 범위를 벗어난다고 판단, resource-only backup으로 축소해 실제 재시험으로 `Completed`(2365/2365 items, 오류 0건) 확인 — PV 데이터 백업 확대는 별도 후속 작업으로 남긴다. `k3s-datastore-backup`·`postgres-native-backup`·`vault-raft-backup` 세 systemd `.service`는 이미 fleet가 수집 중인 `node_systemd_unit_state{state="failed"}`만으로(새 exporter·방화벽 불필요) `obs-13-receiver`·`obs-18-platform-slack` 기존 matcher에 새 alertname 3개만 추가, `VeleroBackupFailed`(기존 `obs-13-backup`)가 Velero 실패·부분실패를 이미 감시해 새 alert 불필요. `ARGO-ROOT` 잠금 아래 `platform-root`·`velero`·`obs` 커밋 SHA 검증(Prometheus `/api/v1/rules` 4개 rule 모두 `health=ok`, `VeleroBackupFailed`가 실제 partial failure로 Alertmanager `active` 상태까지 도달함을 실측) 후 main·Synced/Healthy 복귀 확인 |
| `BKP-08 DONE` | NetBird·Warpgate 정기 백업 스케줄 신설 | `NB-01`, `WG-01`, `S3-01` | `OPNSENSE-LIVE` | NetBird·Warpgate 구성·세션 기록 복구 | `BKP-03`과 동일한 최소권한 패턴(전용 비로그인 system user `netbird-backup`·`warpgate-backup`, `Read/List/Write:<bucket>`만 가진 최소권한 S3 identity `bkp-08-netbird`·`bkp-08-warpgate`, SeaweedFS S3 `IPAddressAllow` drop-in 및 교차 쓰기 거부 검증, systemd backup timer 및 36시간 freshness health timer)으로 NetBird·Warpgate 정기 백업 구축. SeaweedFS volume slot을 `max=30`으로 안전하게 확장하여 신규 collection volume slot 확보. NetBird SQLite 3개 DB(`store.db`·`events.db`·`idp.db`)와 Warpgate SQLite DB(`db.sqlite3`)·녹화본·키·TLS의 온라인 `.backup` 및 tarball 아카이브 S3 업로드·S3 round-trip 무결성 검증·7세대 rotation 검증 완료. 두 host의 격리 복원 검증(`PRAGMA quick_check=PASS`)과 재부팅 후 timer 유지 및 Ansible 멱등성(`changed=0, failed=0`) 확인. OPNsense 방화벽 drift 없음 확인. 상세 절차는 [NetBird·Warpgate runbook](runbook/netbird-warpgate-native-backup.md) 참조 |
| `BKP-09 DONE` | `BKP-01`(k3s datastore)·`BKP-02`(Velero) bucket을 AWS 오프사이트 대상에 추가 | `BKP-01`, `BKP-02`, `BKP-04` | 없음 | k3s·Velero 리소스의 물리 재해 대응 | `object-01`의 `OFFSITE_SOURCE_BUCKETS` 사전 실측에서 `bkp-03-postgres bkp-03-vault`만 있고 `bkp-01-k3s-datastore`·`bkp-02-velero`가 누락됨을 확인. `bkp09_s3_targets` role 및 `playbooks/bkp-09-offsite-backup.yml`을 신설하고 `bkp03_s3_targets`·`native-backup.yml`을 호환 보정하여 `bkp-03-offsite-reader` identity에 4개 bucket(`bkp-03-postgres`, `bkp-03-vault`, `bkp-01-k3s-datastore`, `bkp-02-velero`)의 `Read/List` 권한을 부여하고 타 8개 identity 보존 확인. 4개 bucket 목록 조회 성공 및 쓰기 거부 음성 시험 통과. `OFFSITE_SOURCE_BUCKETS` 4개 bucket 확장 후 `offsite-backup.service` 수동 실행으로 `_heartbeat` 성공, postgres 14개·vault 14개(0 diff), k3s-datastore 15개 신규 전송(0 diff), velero 114개 신규 전송(0 diff), CloudWatch 성공 metric 발행 확인. AWS S3 destination에서 격리 위치(`/tmp/bkp09-restore-sample`)로 k3s-datastore 암호화 아카이브 및 velero repository 파일 1회 다운로드 후 로컬 원본과의 SHA-256 해시 100% 일치(k3s `bd9d85...`, velero `06c99d...`) 검증 완료 및 임시 자원 정리. 2차 Ansible 멱등성(`ok=41 changed=0 failed=0`) 확인 |

| `BKP-10 DONE` | Gitea(`hr-system`)·Jenkins PVC 데이터의 백업 경로 확보 | `SCM-01`, `CI-01`, `BKP-07` | 없음 | 소스 저장소·CI 자격증명·빌드 이력 복구 가능성 | 원인 규명: `BKP-07`의 112개 FS 백업 오류는 클러스터 전체 볼륨(Projected token, Memory emptyDir 등) 무차별 백업 시도 및 Kyverno `pol-01-require-pod-run-as-non-root`의 PVB hosting pod 차단이었음을 실측 로그로 특정. `policies/pol-02-policy-exceptions.yaml`의 `pol-02-velero-run-as-non-root`를 확장(`names: ["*"]`)하여 PVB hosting pod를 허용하고, Gitea(`gitea-data`) 및 Jenkins(`jenkins-home`) Deployment에 `backup.velero.io/backup-volumes` opt-in annotation을 선언. Velero 백업(`bkp-10-verify-fs`) 실행 결과 `gitea-data`(8.26MB) 및 `jenkins-home`(396.76MB) Kopia FS 스냅샷 정상 완료(`Phase: Completed`, 오류 0건). 격리 네임스페이스(`bkp-10-restore-test`)로 복원(`bkp-10-restore-verify`)하여 `gitea-data` 내 `hr-system.git`의 HEAD 커밋 SHA(`be0f99fa...`) 및 git config가 원본과 100% 일치함을 실측 검증 후 임시 자원 정리. `hr-system` 저장소는 GitHub 원본의 `mirror = true` pull-mirror임을 실측 확인(git config 대조)하여 PVC 완전 소실 시에도 GitHub 원본에서 즉시 재동기화 가능하므로 데이터 영구 소실 위험은 Low/Medium으로 재평가됨을 문서화. `ARGO-ROOT` 잠금 하에 SHA 전환 검증 후 main(`Synced/Healthy`) 복귀 완료 |

| `BKP-11 DONE` | postgres·vault backup health-check(freshness) 유닛 실패를 Prometheus·Slack에 연결 | `BKP-06`, `BKP-07` | `ARGO-ROOT` | 인증·플랫폼 데이터 RPO 가시성 | 기존 node-exporter 수집 메트릭(`node_systemd_unit_state`)을 활용하여 `PostgreSQLNativeBackupHealthStale`(`postgres-native-backup-health.service`, 10.10.50.10:9100) 및 `VaultRaftBackupHealthStale`(`vault-raft-backup-health.service`, 10.10.20.10:9101) alert 2개를 `alerting-rules.yaml`의 `bkp-07-native-backup` 그룹에 `state="failed"==1`로 추가. `values-kube-prometheus-stack-01.yaml` 및 `install.yaml`의 `obs-18-platform-slack`·`obs-13-receiver` Alertmanager matcher에 두 alertname을 등록. `ARGO-ROOT` 잠금 하에 라이브 sync 후 Prometheus `/api/v1/rules`에서 5개 alert rule 모두 `Health: ok` 실측 및 Alertmanager `/api/v2/status` matcher 정상 반영 실측 확인 후 `platform-root`·`obs`를 main(Synced/Healthy)으로 복귀 완료 |

| `BKP-12 DONE` | `k3s-datastore-backup`(`BKP-01`)용 freshness health-check 신규 구축과 경보 연결 | `BKP-01`, `BKP-11` | `ARGO-ROOT` | k3s datastore 백업 RPO 가시성 | `k3s_datastore_backup.py`에 백업 성공 시 `last-success.epoch` 원자적 기록 로직 추가. `k3s-datastore-backup-health.service` 및 매일 04:30 KST 정기 검사 timer(`k3s-datastore-backup-health.timer`)를 Ansible role(`k3s_datastore_backup`)로 선언하고 라이브에 적용(`changed=0` 멱등성 검증 완료). 40시간 전 epoch 임시 변조를 통해 staleness 실제 감지(`exit 1`, `failed` unit state) 실증 후 정상 복구 확인. `gitops/apps/obs/alerting-rules.yaml`의 `bkp-07-native-backup` 그룹에 `K3sDatastoreBackupHealthStale`(`10.10.20.10:9101`, `state="failed"==1`) alert 추가 및 `values-kube-prometheus-stack-01.yaml` / `install.yaml`의 Alertmanager `obs-18-platform-slack`·`obs-13-receiver` matchers에 등록. `ARGO-ROOT` 잠금 하에 라이브 sync 후 Prometheus `/api/v1/rules`에서 `K3sDatastoreBackupHealthStale`의 `Health: ok` 실측 및 Alertmanager `/api/v2/status` matcher 정상 반영 실측 확인 후 main(Synced/Healthy) 복귀 완료 |

| `PVE-BKP-01 DEFERRED` | 두 번째 SSD에 Proxmox VM backup | 두 번째 SSD 장착 | `PVE-LIVE` | 빠른 VM 복구 | 원본 NVMe와 다른 장치에 backup·restore; S3 앱 백업은 유지 |

2026-08-14 세션에서 `BKP-06`·`BKP-07` 완료 뒤 인프라 전체 백업 커버리지를 보수적으로
재점검해 `BKP-08`~`BKP-12` 다섯 개를 신설한다. `NetBird`·`Warpgate`는 `architecture.md`에
"제품 DB·구성 백업" 대상으로 문서화돼 있지만 라이브 확인 결과 정기 timer가 전혀 없었다
(`BKP-08`). `object-01`이 다른 VM들과 같은 Proxmox NVMe에 있어 물리 장애 시 로컬 백업과
원본이 동시에 사라지는데, 오프사이트 대상에 `bkp-01-k3s-datastore`·`bkp-02-velero`가
빠져 있었다(`BKP-09`). `BKP-07`이 Velero FS backup을 resource-only로 축소하며 남긴
`gitea-data`·`jenkins-home` PVC 무백업 상태를 별도 조사·해결 작업으로 연다(`BKP-10`).
사용자가 "성공 시 webhook, 실패 시 @channel"을 제안한 데서 출발해, `BKP-03`의 freshness
health-check가 "돌다가 실패"가 아니라 "아예 안 도는 것"을 잡는 별개의 실패 모드임을
확인하고 이미 있는 postgres·vault 유닛 2개에 alert를 추가하고(`BKP-11`), 애초에 이
health-check 패턴이 없는 `k3s-datastore-backup`에는 장치 자체를 새로 만든다(`BKP-12`).
선행이 모두 `DONE`이므로 다섯 모두 곧바로 `READY`로 연다.
...
| `BOARD-DEMO-02-FIX-01 DONE` | `BOARD-DEMO-02`의 decommission 범위에서 빠진 `docs/architecture.md` 서비스 배치표의 `Board Demo` 행을 제거 (`docs/architecture.md`) | `BOARD-DEMO-02`, `AWS-HR-02` | 없음 | 없음 | `BOARD-DEMO-02`가 `gitops/apps/board-demo`·root Application/AppProject·Pomerium route·Jenkins job/credential·Vault KV/policy·Harbor project 선언을 제거했고 `AWS-HR-02`가 Dashy `Board Demo 타일 0건`을 확인했는데도 서비스 배치표에 남아 있던 `내부 데모 애플리케이션 | Board Demo` 행을 지웠다. 삭제 뒤 `docs/architecture.md`의 `Board Demo` 언급 0건, 저장소에 `board-demo` 선언 0건을 확인했다. `docs/evidence/`의 과거 기록은 그 시점의 사실이므로 고치지 않았다. 라이브·Secret·GitOps 선언 변경 0건이고 `scripts/check-backlog.sh`를 통과한다. |
...
| `AWS-HR-01-FIX-07 DONE` | `AWS-HR-01` 완료 뒤에도 `docs/architecture.md` 서비스 배치표에 남아 있던 HR System의 `DECLARED` 상태 표기를 실제 배치로 교체 (`docs/architecture.md`) | `AWS-HR-01`, `AWS-HR-02` | 없음 | 없음 | `AWS-HR-01`이 private EKS·internal ALB·Aurora·IRSA/Secrets Manager 배포를 완료하고 `AWS-HR-02`가 Dashy·Pomerium을 HR 포털로 전환했는데도 배치표 위치 칸이 `DECLARED`로 남아 있던 것을 `private EKS`로 바꿨다. 삭제 뒤 `docs/architecture.md`의 `DECLARED` 표기 0건을 확인했다. 다른 행의 위치 표기 형식과 일치하며 뒤따르는 경계 설명은 그대로 두었다. 라이브·Secret·GitOps 선언 변경 0건이고 `scripts/check-backlog.sh`를 통과한다. |
...
| `AWX-04-FIX-01 DONE` | 만료된 AWX-04 SCM Vault AppRole 입력을 canonical Role ID와 새 Secret ID로 수렴 (`gitops/tools/awx-04/`, `docs/evidence/awx-04-fix-01/`) | `AWX-04` | `VAULT-CONFIG`, `ARGO-ROOT` | `AWX-05` | `kv/awx/scm-lookup`의 AppRole 입력만 Vault 내부에서 교체하고 AWX SCM project update 55 성공; stale AWX-05 operation 종료 뒤 latest main root/AWX `Synced/Healthy`, deploy key·Gitea host key·EE digest·RBAC·PVC·OPNsense 변경 0, secret 원문 0 |
| `AWX-04-FIX-03 DONE` | 만료 SCM lookup Secret ID를 갱신한 뒤 root token을 동봉해 AppRole login을 403으로 오판하던 recovery verifier를 보정 (`gitops/tools/awx-04/`, `docs/evidence/awx-04-fix-03/`) | `AWX-04-FIX-01` | `VAULT-CONFIG`, `ARGO-ROOT` | `AWX-05` | `kv/awx/scm-lookup`의 Role ID canonical·새 Secret ID registered와 bearer 없는 AppRole login/deploy-key read 성공을 판정; helper가 root token을 제거함을 정적으로 확인하고 AWX SCM project update 79 성공, latest main root/AWX `Synced/Healthy`; deploy key·Gitea host key·EE digest·RBAC·PVC·OPNsense·AppRole policy/role 변경 0, secret 원문 0 |
| `AWX-04-FIX-04 DONE` | private Harbor EE의 AWX 등록 credential과 실행 Pod image pull Secret 형식을 분리하고 default Instance Group override에 연결 (`gitops/apps/awx/`, `gitops/tools/awx-04/`, `docs/evidence/awx-04-fix-04/`) | `AWX-04-FIX-03` | `VAULT-CONFIG`, `ARGO-ROOT` | `AWX-05` | 동일 Vault Harbor robot 입력에서 Opaque `awx-ee-pull-credentials`의 exact key `url/username/password/ssl_verify`와 Docker config `awx-ee-pull`을 생성하고, CR `image_pull_secrets` 및 default Instance Group `imagePullSecrets`가 후자만 가리킴을 immutable root `fc4a412`/AWX `50c79fa`에서 판정; SCM AppRole Secret ID 만료는 승인된 재발급 뒤 lookup·SCM update 89/93/94 성공으로 회복, rollback은 literal main `c5bb5f8` root/AWX `Synced/Healthy/Succeeded`, Opaque test Secret absent; private EE runner job·SSH 실행·Harbor robot/권한·EE digest·RBAC·PVC·OPNsense 변경 0, secret 원문 0 |
| `AWX-05 DONE` | 같은 노드 `k3s-01` 한 대에 대한 무변경 SSH canary로 AWX machine credential·host key·실행 격리 검증 (`gitops/apps/awx/`, `infra/ansible/`) | `AWX-04`, `AWX-04-FIX-01`, `AWX-04-FIX-03`, `AWX-04-FIX-04` | `VAULT-CONFIG`, `ARGO-ROOT` | `AWX-06` | 전용 비인간 Linux 계정과 source 제한 authorized key, 인증된 host key, Vault external lookup Machine credential을 사용하고 sudo·become 없이 `k3s-01` exact inventory·limit, forks 1, simultaneous 금지, check 전용 template로 ping/facts 성공과 `changed=0`; AWX task→`k3s-01` TCP 22 NetworkPolicy 한 경로 외 다른 운영 host SSH와 허용 밖 port 실패, OPNsense 변경 0건, job·Pod 로그 secret 원문 0건, 실행 중 guest available 8 GiB 이상·swap 0·신규 PVC 0, 계정·key·Vault·AWX 객체 rollback과 최신 main root·AWX `Synced/Healthy` |
| `AWX-06 DONE` | `netbird-01` 한 대의 가역 marker로 첫 cross-VLAN check→사람 승인→apply 경계와 rollback 검증 (`gitops/apps/awx/`, `infra/ansible/`, `infra/opnsense/`) | `AWX-05`, `NET-04` | `OPNSENSE-LIVE`, `VAULT-CONFIG`, `ARGO-ROOT` | `AWX-07` | OPNsense drift 없음·인증 host key 뒤 AWX task/k3s-01→`netbird-01` TCP 22만 NetworkPolicy·OPNsense exact 허용; workflow 133은 `imcherry5778` 시작, `imcherry5778-admin` 승인으로 precheck 134 `changed=1`→apply 136 `changed=1`→idempotency 137 `changed=0`→cleanup 138 marker absent 성공; sudo·become 없는 전용 account, 일상 direct apply·cleanup·approval 403 및 특권 execute·template/credential 수정 403, actor·target·approver audit·job stdout secret 원문 0; marker playbook은 AWX-01 ConfigMap project(고정 SCM revision 없음)를 명시하고 firewall snapshot·rollback 경로와 최신 main root·AWX `Synced/Healthy` 복원 |
| `AWX-07 DONE` | 기존 `node_exporter_baseline`을 `netbird-01` 한 대에서만 AWX의 첫 실제 운영 역할로 이관 (`gitops/apps/awx/`, `infra/ansible/`) | `AWX-06`, `OBS-11` | `VAULT-CONFIG`, `ARGO-ROOT` | VM 구성 자동화의 제한적 운영 시작 | immutable root `7fb954015225ba28c1aaa7e73867b6b946cc7c81`·AWX child `5c7f8eb87dade88d937c2064df1712af132b307e`, fixed SCM `3ee0cfe665cb94d0b5994a21bec6a4f8a0189600`에서 workflow `174`를 `imcherry5778`이 시작하고 `imcherry5778-admin`이 승인했다. check `175`·apply `177`·idempotency `178`은 모두 `netbird-01` exact inventory/limit·forks 1로 성공했고 세 작업 `changed=0`, `node_exporter.service` enabled·active, Prometheus `up{job="node-exporter",instance="10.10.40.10:9100"}=1`이다. browser RBAC은 operator direct apply·approval·credential 관리 403과 approver check·workflow execute·template 관리 403을 확인했고, job stdout secret 원문 0, guest available `11956535296` bytes(8 GiB 이상)·swap 0·신규 PVC 0이다. rollback은 AWX privileged credential·sudo/key·template 권한만 회수하고 기존 node_exporter와 `OBS-11` 관측 경로는 보존, 최종 root·AWX는 literal `main` `Synced/Healthy` |
...
| `AWS-SEC-DESIGN-01 DONE` | AWS 보안 통제 도입 경계를 ADR로 확정 (`docs/adr/0026-aws-security-control-boundary.md`) | 없음 | 없음 | `AWS-CI-FIX-02`, `AWS-SEC-01`~`06` | 계정 전역 `tofu-account-baseline`과 앱 `tofu-app-security` 두 root의 경계·apply 순서 의존, 계정 단위 변경의 수용 범위, CIEM 조치를 AWS 안에서 끝내고 Shuffle로 통합하지 않는 근거, Object Lock COMPLIANCE 4일 선택, Slack 인터랙티브 유지 결정과 방어선, [ADR-0019](adr/0019-private-aws-service-egress.md)가 전제한 NAT Gateway 부재의 명문화, 검토한 대안과 재검토 조건 기록; 닫힌 PR #53의 브랜치 `feat/aws-security`(`74cf782`)가 `AWS-SEC-01`~`AWS-SEC-06`·`AWS-DB-SEC-01`의 파일 원본임을 ADR과 백로그 서술 양쪽에 남기고, `AWS-SEC-06` 완료 전까지 이 브랜치를 삭제하지 않는다는 조건을 기록 |
| `SUPPLY-DESIGN-01 DONE` | 컨테이너 레지스트리 허브+복제 경계를 ADR로 확정 (`docs/adr/0027-container-registry-hub-and-replica.md`) | 없음 | 없음 | `SUPPLY-01`~`08` | Harbor를 자체 빌드·upstream proxy cache·복제 원본의 단일 통제 지점으로, ECR을 EKS 전용 복제본으로 정의; 이미지 pull이 VPN을 건너지 않는 이유, EKS 관리형 애드온이 AWS 소유 ECR에 남는 예외, admission `failurePolicy` 선택, digest 미고정 41건 처리 방침, 검토한 대안(완전 통일·현상 유지)과 재검토 조건 기록 |
| `SUPPLY-DESIGN-01-FIX-01 DONE` | 공급망 승격·소비 gate를 재설계하고 ADR-0027 대체 (`docs/adr/0028-container-supply-chain-promotion.md`) | `SUPPLY-DESIGN-01` | 없음 | `SUPPLY-01`, `SUPPLY-02` | 현재 선언에서 Jenkins가 Trivy·CycloneDX·Cosign을 수행하고 Harbor 내장 Trivy는 꺼진 실제 책임을 반영, proxy cache를 최종 원본이 아닌 upstream 획득 경계로 축소하고 normal curated project 승격을 정의, Harbor 검증 → scheduled ECR replication → destination digest·image signature·signed CycloneDX attestation 검증 → GitOps 선언 순서와 실패·rollback·retention gate 확정, EKS 관리형 add-on exact 예외와 자체 ECR의 AWS Load Balancer Controller 일반 정책 적용을 분리, namespace 전체 제외 금지와 Audit/Enforce failure 동작 분리, 신규 정책은 Kyverno 1.18 stable `ImageValidatingPolicy`로 한정, Harbor adapter가 남기는 standing AWS key와 encrypted DB working copy를 포함한 credential·회전 경계 기록, snapshot count를 ADR에서 제거하고 `SUPPLY-02` inventory로 이관, `architecture.md`와 `SUPPLY-01`~`08` 완료 증거 정렬 |
| `AWS-CI-FIX-02 DONE` | Jenkins pipeline allowlist에 신규 AWS root 2개를 등록하고 `tofu fmt -check` 게이트 추가 (`infra/aws/Jenkinsfile`, `gitops/apps/jenkins/`) | `AWS-SEC-DESIGN-01` | `ARGO-ROOT` | `AWS-SEC-01`, `AWS-SEC-02` | `allowedRoots`에 `tofu-account-baseline`·`tofu-app-security`를 추가하고 허용 밖 `TARGET_ROOT` 거부 유지, `fmt -check` 실패가 실제로 build를 실패시킴을 음성 대조로 확인, 두 root의 plan-only 실행이 각각 `SUCCESS`와 민감값 0건, 최신 main에서 `platform-root`·`jenkins` `Synced/Healthy` |
| `AWS-SEC-01 DONE` | 계정 전역 보안 기준선 root 신설 (`infra/aws/tofu-account-baseline/`) — CloudTrail·KMS·CloudWatch Logs, Config recorder·delivery·관리형 규칙 5, Security Hub와 FSBP, GuardDuty, Access Analyzer, Security Lake, CIS 알람 12, 보안 알림 SNS와 Chatbot, 계정 차단 설정(S3 Block Public Access·EBS 기본 암호화·스냅샷 공유 차단·암호정책·보안 연락처), 예산과 비용 이상탐지 | `AWS-SEC-DESIGN-01`, `AWS-CI-FIX-02` | `TOFU-STATE`, `AWS-ACCOUNT-SEC` | `AWS-SEC-02`, `AWS-SEC-05` | apply 전 라이브에 GuardDuty·Security Hub·Config recorder가 이미 활성인지 확인하고 있으면 import로 흡수, Trivy `HIGH`·`CRITICAL` 게이트와 `fmt -check`·`validate` 통과, `enable_cis_benchmark=false` 유지, CloudTrail 로그가 S3와 CloudWatch Logs 양쪽에 도달, CIS 알람 12개 `OK`, 계정 차단 설정 적용 뒤 기존 워크로드 회귀 0건, 최종 plan 무변경 |
| `AWS-SEC-01-FIX-01 DONE` | Security Lake가 자동 생성한 metastore Lambda log group을 account-baseline root로 import하고 30일 retention 선언 (`infra/aws/tofu-account-baseline/`) | `AWS-SEC-01` | `TOFU-STATE`, `AWS-ACCOUNT-SEC` | `AWS-SEC-02` | `/aws/lambda/AmazonSecurityLakeMetastoreManager-ap-northeast-2`가 baseline state에 import되고 `retention_in_days=30` 실제 적용, Security Lake·Lambda 함수·기존 log event 변경 0건, `fmt`·`validate`·Trivy 통과와 baseline 최종 plan 무변경 |
| `AWS-SEC-02 DONE` | HR 워크로드 보안 root 신설 (`infra/aws/tofu-app-security/`) — RDS 감사로그 WORM(Object Lock COMPLIANCE 4일·transition 30일·expiration 210일), HR VPC Flow Logs(`REJECT`), 계정 기본 VPC Flow Logs, S3 하드닝, Amazon Managed Grafana와 SAML·Athena·Glue | `AWS-SEC-01`, `AWS-SEC-01-FIX-01` | `TOFU-STATE` | `AWS-SEC-03` | 디렉터리명과 backend key 일치, `tofu-account-baseline` remote_state 네 값(SNS ARN·CloudTrail ARN·CloudTrail 로그그룹명·Access Analyzer ARN) 참조 성공, Object Lock 보존 4일 실측과 `destroy` 경로 존재 확인, Flow Log가 `REJECT`만 기록, Grafana SAML 로그인과 CloudWatch·Athena 데이터소스 양쪽 생존, 모든 Lambda 로그그룹의 `retention_in_days` 선언, Trivy·`fmt`·`validate` 통과, 최종 plan 무변경 |
...
| `AWS-DB-SEC-01 DONE` | HR Aurora의 IAM DB 인증 활성화와 역할별 마스킹 뷰 적용 (`infra/aws/tofu-app-db/`) | `AWS-SEC-03` | `TOFU-STATE` | `AWS-SEC-06` | 라이브 프로덕션 클러스터 변경이므로 적용 직전 승인과 재시작 필요 여부를 먼저 확인, 변경 전후 기존 HR 애플리케이션 연결 무중단, `aurora_cluster_resource_id` output 추가로 `rds-db:connect` 권한을 정확한 클러스터 리소스로 스코핑, `general_user_readonly`·`security_auditor_readonly`의 원본 테이블 접근 거부와 마스킹 뷰 조회 성공을 같은 시점에 대조하고 salary 필드가 `****`로 반환, rollback 절차 확인. 상세 증거: [`docs/evidence/aws-db-sec-01/README.md`](evidence/aws-db-sec-01/README.md). |
...
| `AWS-SEC-06 DONE` | 보안 시나리오 5종 라이브 검증과 운영 문서화 (`infra/aws/tofu-app-security/scripts/`) | `AWS-SEC-04`, `AWS-SEC-05`, `AWS-DB-SEC-01` | `TOFU-STATE`, `IDENTITY-LIVE` | `SUPPLY-06` | PII 마스킹·ASR 원복·세션 종료·권한 드리프트·Grafana SOC 각각 `PASSED`와 재현 절차 기록, 세션 종료는 격리 테스트 계정으로만 실행해 실제 팀원 세션 영향 0건, 권한 드리프트는 데모 Role만 대상으로 하고 운영 SAML Role 정책 불변, `revoke-session-*` 인라인 정책의 24시간 자동 정리 동작 확인, 실행 뒤 임시 자원과 잔여 인라인 정책 제거. 상세 증거: [`docs/evidence/aws-sec-06/README.md`](evidence/aws-sec-06/README.md). |
...
| `SUPPLY-04-FIX-01 DONE` | 완료된 Keycloak bootstrap Job의 immutable Pod template 이미지 전환을 안전하게 보정 (`gitops/apps/keycloak/`) | `SUPPLY-04` | `ARGO-ROOT`, `IDENTITY-LIVE` | `AWS-SEC-03` | 기존 완료 Job을 수동 삭제·강제 교체·무검증 재실행하지 않는다. 현재 Keycloak realm·OTP 정책·복구 계정·복구 client의 사전/사후 상태가 동일함을 확인하는 check-first/no-op 경로를 먼저 선언하고, Harbor curated digest를 사용하는 새 versioned Job이 그 경로만 수행해 `Completed`가 된다. 이후 최신 main에서 `platform-root`·`keycloak`이 `Synced/Healthy`이고, 기존 Keycloak 사용자·group membership·운영 client 변경 0건을 확인한다. rollback은 새 versioned Job 선언만 제거하며 기존 완료 Job과 Keycloak 상태는 보존한다. |
| `SUPPLY-04-FIX-02 DONE` | 완료 bootstrap Job을 상시 Keycloak Application에서 분리하고 수동 실행 경계로 전환 (`gitops/apps/keycloak/`, `gitops/apps/keycloak-bootstrap/`) | `SUPPLY-04-FIX-01` | `ARGO-ROOT`, `IDENTITY-LIVE` | `AWS-SEC-03` | 상시 `keycloak` Application에서 완료 Job을 제거해 Argo sync/self-heal 재실행을 막고, curated digest·check-first/no-op Job을 별도 수동 경계로 분리했다. immutable root/child SHA 검증에서 v3 Job·Pod 부재와 Keycloak Deployment `1/1`을 확인한 뒤 root·child를 literal `main`으로 복원했다. 기존 v2는 복원·재실행하지 않았고 DB·사용자·group membership·운영 client는 변경하지 않았다. 수동 경계 Kustomize render, `bash -n`, `git diff --check`와 secret-free 문서를 확인했다. 상세 증거: [`docs/evidence/supply-04-fix-02/README.md`](evidence/supply-04-fix-02/README.md). |
...
| `SUPPLY-06 DONE` | Harbor trusted release의 scheduled ECR replication과 destination verifier 구성 (`gitops/apps/harbor/`, `infra/aws/tofu-app-ci/`) | `SUPPLY-05`, `SUPPLY-01`, `AWS-SEC-06` | `ARGO-ROOT`, `TOFU-STATE`, `VAULT-CONFIG` | `SUPPLY-07` | `/service/harbor/` 전용 IAM user 한 개·active key 한 개를 exact ECR upload/read와 `GetAuthorizationToken`으로 제한하고 delete·repository 관리 권한 0건, key 원본은 Vault/저장소 밖이고 Harbor encrypted DB working copy·dual-key 회전 경계 문서화, event trigger 없이 signed artifact 완성 뒤 scheduled `aws-ecr` replication 성공, Harbor/ECR subject digest 일치와 ECR `oras discover`의 image signature·CycloneDX attestation 확인, ECR 주소에서 `cosign verify`·`cosign verify-attestation`과 `bomFormat=CycloneDX` 통과, ECR lifecycle preview에서 subject보다 referrer가 먼저 만료되는 항목 0건, EKS exact digest pull 성공, deletion 전파 비활성·시험 artifact 제거. 상세 증거: [`docs/evidence/supply-06/README.md`](evidence/supply-06/README.md). |
| `SUPPLY-07 DONE` | hr-system build를 Harbor 승격·ECR destination 검증 뒤 배포하도록 전환 | `SUPPLY-06` | `ARGO-ROOT` | `SUPPLY-08` | 이 저장소 밖 `ktcloud4-bean/hr-system`의 `Jenkinsfile` 변경을 같은 작업자가 함께 소유, Jenkins는 Harbor candidate push·Trivy·Cosign image signature·signed CycloneDX attestation까지만 수행하고 ECR direct push 0건, scheduled replication 뒤 destination verifier가 세 image의 ECR digest·signature·attestation을 통과하기 전 GitOps digest 변경 0건, EKS는 기존 ECR VPC endpoint 경로로 pull, build부터 Pod Ready까지 1회 왕복 성공과 실패 replication에서 직전 digest 유지, `SUPPLY-01` Enforce 계속 통과 |
| `SUPPLY-08 DONE` | Jenkins ECR publisher IAM 사용자 제거 (`infra/aws/tofu-app-ci/`) | `SUPPLY-07` | `TOFU-STATE`, `VAULT-CONFIG` | 없음 | Jenkins ECR direct API/push 0건을 pipeline 1회로 확인한 뒤 기존 Jenkins IAM user·policy·access key와 Vault 자격증명 삭제, Harbor replicator와 hr-system 왕복 정상, `tofu plan` 무변경, 계정 잔여 standing access key가 `backup`·`vault_auto_unseal`·`argocd_credential_issuer`·`harbor_ecr_replicator` 네 건으로 수렴. 상세 증거: [`docs/evidence/supply-08/README.md`](evidence/supply-08/README.md). |

2026-08-14 `SUPPLY-DESIGN-01-FIX-01`은 저장소 선언과 제품 제약을 다시 대조해 [ADR-0027](adr/0027-container-registry-hub-and-replica.md)을 [ADR-0028](adr/0028-container-supply-chain-promotion.md)로 대체했다. 현재 Harbor 내장 Trivy는 꺼져 있고 Jenkins가 Trivy·CycloneDX·Cosign을 수행하므로 생산·검증은 Jenkins, 저장·retention·replication은 Harbor, admission은 Kyverno가 소유한다. Proxy cache는 upstream이 삭제한 artifact의 영구 원본이 될 수 없어 획득 경계로만 두고, 일반 curated project로 digest copy·scan·Cosign image signature·signed CycloneDX attestation을 만든 뒤 k3s가 소비한다. ECR은 signed artifact 완성 뒤 scheduled replication하고 destination에서 subject digest, image signature와 CycloneDX attestation을 재검증한 뒤에만 GitOps digest를 바꾼다. 신규 정책은 Kyverno 1.18 stable `ImageValidatingPolicy`로 한정하고 `kube-system` 전체 제외를 금지하며, AWS 관리형 VPC CNI·CoreDNS·kube-proxy exact 예외와 자체 ECR의 AWS Load Balancer Controller 일반 정책을 분리했다. Harbor adapter가 encrypted DB에 working credential을 보관하고 `/service/harbor/` standing key 한 건이 남으므로 `SUPPLY-08`의 최종 key 수를 네 건으로 바로잡았다. 기존 고정 image count는 ADR에서 제거하고 `SUPPLY-02`의 재현 가능한 inventory로 이관했다. 문서만 바꿨으며 라이브 상태는 변경하지 않았다. 이 보완을 `DONE`으로 닫고 직접 후속 `SUPPLY-01`·`SUPPLY-02`는 새 설계를 선행으로 삼아 계속 `READY`다.

2026-08-14 `SUPPLY-DESIGN-01`에서 Harbor를 자체 빌드·upstream proxy cache·복제 원본의 단일 통제 지점으로, ECR을 EKS 전용 다운스트림 복제본으로 정의하는 [ADR-0027](adr/0027-container-registry-hub-and-replica.md)을 확정했다. 대용량 이미지 레이어 다운로드가 S2S VPN의 대역폭·가용성 리스크를 유발하지 않도록 EKS 노드는 동일 VPC 내 ECR VPC Endpoint를 통해서만 사설 pull하고, 이미지는 Harbor→ECR 비동기 복제로 사전에 배치한다. VPC CNI·CoreDNS·ALB controller 등 EKS 관리형 애드온은 AWS 공식 ECR allowlist 예외로 허용해 애드온 라이프사이클 충돌을 방지한다. Kyverno admission 정책은 Audit 단계에서 `failurePolicy: Ignore`, Enforce 단계에서 `failurePolicy: Fail`로 정의하고 시스템 네임스페이스(`kube-system`, `kyverno`)를 격리했다. k3s 내 tag-only 이미지 41건(160건 중 고정 119건)은 Audit 가시화(`SUPPLY-02`), Harbor proxy cache 구축(`SUPPLY-03`), GitOps 경로 및 digest 고정 전환(`SUPPLY-04`), Enforce 승격(`SUPPLY-05`) 4단계로 해소하도록 방침을 정했다. 완전 통일(VPN 과부하 또는 온프레미스 폐쇄성 훼손) 및 현상 유지(장기 IAM 정적 키 방치 및 거버넌스 분산) 대안을 배제했다. 이 작업은 문서만 확정하므로 라이브 변경 대상이 없으며, `SUPPLY-DESIGN-01`을 `DONE`으로 닫고 선행이 충족된 직접 후속 `SUPPLY-01`과 `SUPPLY-02`를 `READY`로 연다.

2026-08-14 `AWS-SEC-DESIGN-01`·`SUPPLY-DESIGN-01` 계획에서 팀 PR `feat/aws-security`(#53)를 분해하고
레지스트리 이행을 함께 여는 18개 작업을 정의했다. PR은 신규 root 하나에 159개 resource를 담았으나 세션
종료 Lambda가 NAT 없는 VPC에서 IAM·SNS에 도달하지 못하고, RDS 점검 Lambda는 `rds_sg` ingress가 없어
Aurora에 연결되지 않으며, Slack 콜백 API Gateway의 signing secret이 코드의 고정값으로 들어가 있고,
CIEM Access Key 스캔이 `/service/` 경로 정적 키 네 개를 삭제 후보로 올린다. DB 권한 자동 REVOKE와
감사로그 WORM 시연은 범위에서 뺐고, Object Lock은 COMPLIANCE 4일로 줄여 `destroy` 경로를 남겼다.
root는 계정 전역과 앱 보안 두 개로 나누고 계정 단위 싱글턴을 보호할 `AWS-ACCOUNT-SEC` 잠금을 새로
정의한다. AWS OpenTofu root 작업은 Argo가 배포하지 않아 `ARGO-ROOT`를 잡지 않으므로
(`AWS-STATE-RECOVERY-01` 선례), `TOFU-STATE` 계열 AWS 작업과 `ARGO-ROOT` 계열 GitOps 작업을 두
레인으로 병렬 실행해 18단계를 12단계로 줄인다. 선행이 없는 `AWS-SEC-DESIGN-01`과 `SUPPLY-DESIGN-01`만
`READY`로 연다.
...
2026-08-14 `AWX-07`에서 기존 기준선이 있는 `netbird-01` 한 대만 workflow `174`로 실행했다.
`imcherry5778`이 check와 workflow를 시작하고 `imcherry5778-admin`이 사람 승인을 남겼으며,
check `175`·apply `177`·idempotency `178`이 모두 성공·`changed=0`이었다. fixed SCM revision은
`3ee0cfe665cb94d0b5994a21bec6a4f8a0189600`, service는 enabled·active, Prometheus target은
`up=1`이고 guest available `11956535296` bytes·swap 0·PVC 0으로 8 GiB 정지선을 통과했다.
browser RBAC의 operator direct apply·approval·credential 관리와 approver check·workflow execute·
template 관리는 모두 403이었다. immutable root `7fb954015225ba28c1aaa7e73867b6b946cc7c81`와
AWX child `5c7f8eb87dade88d937c2064df1712af132b307e` 검증 뒤 양 Application은 literal `main`으로
복구한다. 이 결과는 다른 대상·일정·동시 실행·ad hoc command를 허용하지 않는다.
...
2026-08-15 `SUPPLY-02`에서 k3s 클러스터 전체 워크로드를 대상으로 Kyverno 1.18 stable `ImageValidatingPolicy` (IVP)를 Audit 모드로 신설하고, 재현 가능한 인벤토리 추출기(`gitops/tools/supply-02/inventory.py`)를 통해 Git 선언·Render·Live 클러스터 3계층의 이미지 튜플 인벤토리 실측과 상세 분석 보고서를 산출했다.

1. **Kyverno Audit 정책 배포 (`policies/k3s-image-supply-chain-audit.yaml`)**:
   - `spec.failurePolicy: Ignore`, `spec.validationActions: [Audit]`, `spec.evaluation.background.enabled: true` 모드로 선언하여 기존 및 신규 워크로드 거부 0건(차단 없음)을 보장.
   - `gitops/root/policy-baseline-project.yaml`의 `clusterResourceWhitelist`에 `policies.kyverno.io / ImageValidatingPolicy`를 등록하여 Argo CD `policy-baseline` Application `Synced/Healthy` 달성.
   - `e2e-01` 네임스페이스의 기존 `e2e-01-verify-release-image` IVP는 `Deny / Fail` Enforce 모드로 불변 유지.
2. **단일 원본 인벤토리 추출기 개발 (`gitops/tools/supply-02/inventory.py`)**:
   - `cluster/namespace/controller/container/image/digest/registry/exception` 표준 튜플 스키마 추출.
   - Git 선언(`89` 튜플), Kustomize render(`82` 튜플), Live 런타임(`140` 튜플) 간 차이 분석(Helm chart-generated sidecar, Replica 확장, DaemonSet 배포 등) 자동화.
3. **인벤토리 실측 결과 (`docs/evidence/supply-02/`)**:
   - **총 Live 컨테이너 튜플**: `140`건 (고유 이미지 `65`종).
   - **sha256 다이제스트 고정**: `119`건 (85.0%), **Tag-only 미고정**: `21`건 (15.0%, `SUPPLY-04` 해소 대상).
   - **레지스트리 분포**: `docker.io` 90건, `quay.io` 34건, `ghcr.io` 8건, `docker.gitea.com` 3건, `reg.kyverno.io` 3건, `public.ecr.aws` 1건, `registry.k8s.io` 1건.
   - **시스템 Exact 예외**: `33`건 (`kube-system`, `kyverno`, `falco`, `wazuh` 등).
4. **6대 완료 증거 라이브 검증 (`gitops/tools/supply-02/verify-live.sh`) 100% PASS**:
   - `evidence_e2e_enforce_immutable=pass`: `e2e-01` IVP `Deny/Fail` 불변 유지.
   - `evidence_audit_policy=pass`: `k3s-image-supply-chain-audit` `Audit/Ignore/Background=true` 정상 확인.
   - `evidence_inventory_extractor=pass`: 인벤토리 산출물 JSON/MD 생성 및 튜플 데이터 실측 확인.
   - `evidence_categorized_lists=pass`: 4대 분류별 잔여 목록 스키마 유효성 확인.
...
2026-08-15 `SUPPLY-04`에서 k3s 일반 워크로드의 전체 49개 이미지를 normal Harbor curated project(`curated-platform`)로 승격(Digest OCI Copy + Cosign 서명 + CycloneDX SBOM Attestation 부여)하고, GitOps 선언 및 라이브 클러스터의 이미지 경로를 `@sha256:` 고정 다이제스트로 전면 전환했다.

1. **Harbor Curated 프로젝트 생성 및 OCI 아티팩트 승격 (`gitops/tools/supply-04/promote.py`)**:
   - Harbor에 normal public OCI 프로젝트 `curated-platform`을 생성하고 SeaweedFS S3 스토리지 연동을 확인했다.
   - `sonarqube:26.7.0.124771-community`(2.0 GiB) 및 `opensearch:3.2.0`(1.6 GiB) 등 대용량 이미지를 포함한 총 49개 일반 워크로드 이미지를 승격 파이프라인으로 OCI 복사했다.
   - Vault(`kv/jenkins/runtime`)에 보관된 플랫폼 Cosign Private Key로 서명(`cosign sign`) 및 CycloneDX v1.5 SBOM Attestation(`cosign attest --type cyclonedx`)을 생성·부착하고 49개 전체 아티팩트 서명 검증을 완료했다 ([`docs/evidence/supply-04/promoted-images.json`](evidence/supply-04/promoted-images.json)).

2. **GitOps 선언 파일 전면 전환 (`gitops/apps/`, `gitops/bootstrap/argocd/`)**:
   - 50개 이상의 GitOps 선언 매니페스트 내 이미지 경로를 `harbor.imcherry5778.xyz/curated-platform/<repo>@sha256:...`로 전환했다.
   - ArgoCD, Dex, Redis 등 기존 tag-only 잔여 3종을 sha256 고정으로 해소했다.

3. **라이브 클러스터 롤아웃 및 인벤토리 실측 (`gitops/tools/supply-02/inventory.py`)**:
   - `gitops/tools/supply-04/replace-live-workloads.py`로 모든 Deployment, StatefulSet, DaemonSet의 Pod spec 컨테이너 및 initContainer 이미지를 Curated `@sha256:`으로 교체했다.
   - 전체 네임스페이스(`argocd`, `awx`, `cert-manager`, `crowdsec-01`, `gitea`, `harbor`, `headlamp`, `jenkins`, `keycloak`, `loki`, `obs`, `pomerium`, `shuffle`, `sonarqube`, `vault`, `velero`)의 모든 워크로드가 100% `Running` 및 `Completed` (Ready) 상태임을 실측했다.
   - 라이브 인벤토리 실측에서 시스템 예외(`kube-system`, `kyverno`, `falco`, `wazuh`)를 제외한 일반 워크로드의 **Upstream 레지스트리 직접 참조 0건**, **Tag-only 참조 0건**을 완벽하게 증명했다.

4. **완료 증거 라이브 검증 (`gitops/tools/supply-04/verify-live.sh`) 100% PASS**:
   - `curated-platform` Harbor 프로젝트 존재 및 API 정상 확인 (HTTP 200).
   - 49개 전체 승격 이미지에 대한 Cosign 서명 및 CycloneDX SBOM Attestation 검증 통과.
   - 클러스터 내 모든 워크로드 Pod `Running/Completed` 상태 확인.
...
2026-08-15 `SUPPLY-04-FIX-01`에서 완료된 Keycloak bootstrap Job의 immutable Pod template 이미지 전환을 안전하게 보정했다.

1. **기존 완료 Job 보존 및 Versioned Job v3 신설 (`gitops/apps/keycloak/bootstrap-job.yaml`)**:
   - 기존 완료된 `keycloak-bootstrap-v2` Job을 강제 삭제하거나 재실행하지 않고 라이브에 안전하게 보존.
   - 새 versioned Job `keycloak-bootstrap-v3`를 선언하고, `vault-agent` 및 `bootstrap` 컨테이너 이미지를 Harbor curated digest(`@sha256`)로 전환.

2. **Check-First / No-Op 안전 경로 실행 및 검증 (`gitops/tools/supply-04-fix-01/verify-live.sh`)**:
   - `keycloak-bootstrap-v3`가 DB에 이미 존재하는 realm, 사용자, MFA 정책, 클라이언트를 감지하여 no-op/check-first 경로로 안전하게 `Complete (1/1)` 도달.
   - 기존 Keycloak 사용자, group membership, 운영 클라이언트 변경 0건 확인.
   - Keycloak server 런타임 정상 가동 및 OIDC Discovery(`https://sso.imcherry5778.xyz/realms/platform/.well-known/openid-configuration`) 정상 응답 확인.
...
`AWS-SEC-05`를 `DONE`으로 닫는다.

2026-08-15 `AWS-DB-SEC-01`에서 HR Aurora PostgreSQL의 IAM DB 인증을 활성화하고 역할별 PII 마스킹 뷰를 적용했다.

1. **Aurora IAM Database Authentication 활성화 (`infra/aws/tofu-app-db/rds.tf`)**:
   - `iam_database_authentication_enabled = true` 및 `apply_immediately = true` 적용.
   - Aurora PostgreSQL에서 IAM DB 인증 변경은 클러스터/인스턴스 재부팅 없이 무중단(In-place Dynamic)으로 즉시 적용됨을 사전 검증 및 실측 확인.
   - `aurora_cluster_resource_id = "cluster-R3RXAS56R7IZH3JKOP27JX4NEE"` output을 추가하여 IAM `rds-db:connect` 권한의 정밀 리소스 스코핑 계약 확보.

2. **역할별 PII 마스킹 뷰 및 IAM DB 인증 실측 검증 (`scripts/verify-db-sec.sh`)**:
   - `general_user_readonly` (IAM DB Token) 접속: 원본 `employees` 테이블 직접 조회 시 Permission Denied 거부 확인, `masked.employees_general` 조회 시 `salary = NULL` 마스킹 확인, `masked.change_history_general` 조회 시 `salary` 관련 필드 `'****'` 마스킹 확인.
   - `security_auditor_readonly` (IAM DB Token) 접속: 원본 `employees` 테이블 직접 조회 거부 확인, `masked.employees_audit` 조회 시 `salary` 원본 노출 확인.
   - 기존 애플리케이션 계정(`employee_service`)의 접속 및 무중단 상태 확인 (직원 수 6명 정상 조회).
   - 상세 증거: [`docs/evidence/aws-db-sec-01/README.md`](evidence/aws-db-sec-01/README.md).

`AWS-DB-SEC-01`을 `DONE`으로 닫는다.

2026-08-15 `AWS-SEC-06`에서 AWS 통합 보안 통제 시나리오 5종의 라이브 실측 검증을 완료하고 운영 문서를 확보했다.

1. **5대 보안 시나리오 실측 검증 완료 (`infra/aws/tofu-app-security/scripts/verify-all-scenarios.sh`)**:
   - **Scene 1 (PII 데이터 마스킹)**: Aurora IAM DB Auth 연동을 통해 일반 사용자의 급여 `salary = NULL`, 변경 이력 `'****'` 마스킹 및 원본 테이블 직접 접근 거부(`Permission Denied`), 감사자 원본 노출 대조 확인 (`PASSED`).
   - **Scene 2 (ASR 자동 원복)**: 인스턴스 미부착 더미 SG에 SSH `0.0.0.0/0` 생성 후 SSM Automation 런북(`AWS-DisablePublicAccessForSecurityGroup`) 실행을 통해 규칙 자동 제거 및 `trap` 안전장치 동작 확인 (`PASSED`).
   - **Scene 3 (긴급 세션 강제 종료)**: 격리 테스트 계정(`test-session-revoke-demo`) 대상 CIEM action executor 호출로 4개 운영 SAML Role + Demo Role에 `ciem-revoke-session-test-session-revoke-demo` Deny 인라인 정책 부착 및 실제 팀원 세션 영향 0건, 테스트 후 즉시 무잔여 정리 확인 (`PASSED`).
   - **Scene 4 (권한 드리프트 및 Slack 보안)**: Demo Role AssumeRole 세션의 실제 호출 이력을 기반으로 Access Analyzer 축소 정책 생성(`SUCCEEDED`), 4개 운영 SAML Role 정책 불변성 확인, Slack 콜백 API Gateway 위조 서명 `401 Unauthorized` 정상 차단 확인 (`PASSED`).
   - **Scene 5 (Amazon Managed Grafana SOC 대시보드)**: Managed Grafana 워크스페이스 `ACTIVE`, SAML `CONFIGURED`, Athena Workgroup `ENABLED`, Security Lake Glue DB 연동 상태 확인 (`PASSED`).
   - 상세 증거: [`docs/evidence/aws-sec-06/README.md`](evidence/aws-sec-06/README.md).
...
1. **`/service/harbor/` 전용 Replicator IAM 사용자 프로비저닝 (`infra/aws/tofu-app-ci/`)**:
   - `hr-system-prod-harbor-ecr-replicator` IAM 사용자 및 최소 권한 인라인 정책(`hr-system-prod-ecr-replication-only`) 생성.
   - `ecr:GetAuthorizationToken` 및 HR 3개 ECR 리포지토리(`frontend`, `employee-service`, `hr-service`)에 대한 upload/read 권한 부여, delete 및 repository 관리 권한은 0건 확인.
   - active key 1개를 발급하여 Vault (`kv/harbor/ecr-replicator`)에 저장하고, Harbor DB working copy 및 dual-key 회전 경계를 확립.

2. **Harbor Scheduled Replication Policy 프로비저닝 (`gitops/tools/supply-06/provision.py`)**:
   - Harbor에 `aws-ecr-endpoint` Registry Endpoint 등록 및 Ping 연결 성공(`HTTP 200`).
   - `harbor-to-ecr-hr-system` 복제 규칙 생성 (`trigger=scheduled`, `cron="0 0 * * * *"`, `replicate_deletion=false`).

3. **ECR Destination Verifier 실측 검증 (`gitops/tools/supply-06/verify-live.sh`)**:
   - Harbor 및 ECR의 Subject Digest 일치 확인 (`sha256:d53ccf25f6ded935d40710bda1c0ec0bb510c50a912dc83e2158faa6e995c572`).
   - ECR OCI 1.1 Referrers API (`oras discover`)를 통한 CycloneDX SBOM attestation referrer 발견 및 `bomFormat=CycloneDX` payload 검증 완료.
   - 플랫폼 Cosign 공개키(`cosign.pub`)를 통한 ECR Container Image Signature 검증(`cosign verify`) 100% 통과.
   - ECR Lifecycle Policy Preview를 통해 Subject보다 Referrer가 먼저 만료되는 항목 0건 확인.
   - EKS 환경에서 exact digest pull 및 `employee-service` Pod 정상 기동 확인.
...
| `SUPPLY-05-FIX-01 DONE` | k3s 공급망 Enforce 뒤 재생성이 막힌 AWX operator·AWX CR, CrowdSec, Velero controller/node-agent, Renovate CronJob을 Harbor curated digest 선언으로 수렴하고 inventory·검증 범위를 Deployment·StatefulSet·DaemonSet뿐 아니라 Job·CronJob·지원 CR의 Pod template까지 확대 (`gitops/apps/`, `gitops/tools/supply-02/`, `gitops/tools/supply-04/`, `gitops/tools/supply-05/`) | `SECOPS-AUDIT-01`, `SUPPLY-08` | `ARGO-ROOT` | `SUPPLY-05-FIX-02`, `OBS-20`, `UPDATE-02` | 정책 Enforce/Fail 및 시스템 네임스페이스 제외를 유지한 채 AWX operator/CR/redis, CrowdSec lapi/agent/appsec, Velero controller/node-agent, Renovate CronJob을 Harbor curated digest(`harbor.imcherry5778.xyz/curated-platform/`)로 수렴했다. inventory를 Helm 템플릿 렌더링 및 CR/CronJob/Job으로 확장해 비시스템 upstream 직접 참조 0건, tag-only 0건을 확인했다. immutable commit SHA에서 Velero controller/node-agent 롤아웃 및 ad-hoc backup `Completed`, AWX operator·web·task 롤아웃, CrowdSec 3개 Deployment 롤아웃, Renovate CronJob server-side test Job 1회 실행 성공을 확인하고 관련 Application(`velero`, `awx`, `crowdsec`, `renovate`, `platform-root`) 모두 `Synced/Healthy`를 달성했다. |
| `CI-01-FIX-01 DONE` | Jenkins의 immutable `trivy-db-bootstrap` Job 교체 실패를 versioned 또는 상시 Application 밖 수동 bootstrap 경계로 보정하고 stale JCasC 리소스를 최신 선언으로 수렴 (`gitops/apps/jenkins/`) | `SECOPS-AUDIT-01`, `CI-01`, `SUPPLY-08` | `ARGO-ROOT` | `OBS-20`, `OPS-DRIFT-01` | 기존 완료 Job 강제 재실행 0건, PVC(`jenkins-home`, `trivy-cache`) 보존 확인. `trivy-db-bootstrap` Job을 상시 GitOps 선언에서 분리해 immutability 충돌을 해소하고 최신 JCasC ConfigMap으로 `jenkins` Deployment 정상 롤아웃. CronJob `trivy-db-update` ad-hoc 실행(`scan01-db-update=pass`) 및 Jenkins 기존 파이프라인(`ci01-image-build`) 성공 완료. immutable commit SHA 및 main에서 `jenkins` 및 `platform-root` `Synced/Healthy` 달성. |
| `CI-01-FIX-02 DONE` | Jenkins에 파이프라인 시각화 플러그인 `pipeline-graph-view`와 `pipeline-stage-view`를 추가해 stage 단위 성공·실패를 화면에서 읽히게 한다 (`gitops/apps/jenkins/plugins.txt`, `gitops/apps/jenkins/`) | `CI-01-FIX-01` | `ARGO-ROOT` | Jenkins 화면 가독성, 발표·데모 증거 | `pipeline-graph-view`(998.vc30deece0e36) 및 `pipeline-stage-view`(2.41)와 전이 의존성 11종을 `plugins.txt`에 추가해 총 77개 플러그인을 exact 고정했다. `ARGO-ROOT` 잠금 하에 `jenkins` 및 `platform-root`를 commit SHA로 검증해 controller 정상 롤아웃과 `1/1 Ready`, `Synced/Healthy`를 확인했다. Jenkins API로 6개 Job 보존 및 77개 플러그인 로드, `/job/hr-system-image-build/wfapi/runs`의 stage 단위 상태(#41 SUCCESS, #42 FAILED)와 job 대시보드 내 `<div class="cbwf-stage-view">` 렌더링을 실측 검증했다. 공급망 digest 경계 및 Kyverno 정책 불변, Secret 원문/자격증명 변경 0건을 확인하고 literal main 복귀를 완료했다. |
| `SUPPLY-05-FIX-02 DONE` | ADR-0028 계약대로 k3s 일반 workload에 Harbor registry·digest와 별도로 플랫폼 Cosign image signature를 검증하는 Kyverno 1.18 stable `ImageValidatingPolicy`를 적용하고 namespace 전체 제외를 exact controller·ServiceAccount·repository 예외로 축소 (`policies/`, `gitops/apps/kyverno/`) | `SUPPLY-05-FIX-01` | `ARGO-ROOT`, `VAULT-CONFIG` | 공급망 admission 신뢰 경계 | current·previous 공개키만 읽는 최소권한, curated 정상 artifact 허용, 미서명 image·upstream 직접 참조·tag-only 미고정 이미지를 admission에서 각각 거부, `kube-system`·`kyverno`를 포함한 namespace 전체 제외 0건, registry 오류 시 Enforce/Fail 동작과 Audit/Ignore rollback, 기존 workload 정상 기동 및 `Synced/Healthy`. |
| `SUPPLY-05-FIX-03 DONE` | `SUPPLY-05-FIX-02` 뒤 실제 curated Renovate Job 생성이 `ivpol.mutate.k3s-image-supply-chain-policy` callback timeout으로 막힌 결함의 계층을 Kyverno Service·Endpoint·admission controller·ImageValidatingPolicy 중 실측으로 특정하고, Enforce/Fail 및 exact 예외만 유지하는 최소 보정 (`policies/`, `gitops/apps/kyverno/`, `gitops/tools/supply-05-fix-03/`, `docs/evidence/supply-05-fix-03/`) | `SUPPLY-05-FIX-02` | `ARGO-ROOT` | `UPDATE-02`, 공급망 admission 가용성 | timeout의 Service/Endpoint/controller/policy 실패 지점을 로그·응답·상태로 재현해 특정한다. 보정 뒤 curated digest Renovate 계열 Job 1회가 Pod를 생성·완료하며, `ImageValidatingPolicy`와 webhook은 Enforce/Fail, namespace 전체 제외 0건, broad 예외·`Ignore` 전환 0건을 유지한다. immutable SHA와 main에서 `kyverno`·`policy-baseline`·`renovate`·`platform-root`가 `Synced/Healthy`이며 GitHub/Vault/실제 dependency PR canary는 재실행하지 않는다. | 상세 증거: [`docs/evidence/supply-05-fix-03/README.md`](evidence/supply-05-fix-03/README.md).
| `SUPPLY-05-FIX-04 DONE` | Wazuh manager의 실제 Vault init image가 k3s 공급망 system 예외와 달라 Pod 재생성이 거부되는 결함을 exact Pod·ServiceAccount·두 digest 경계로 보정 (`policies/`, `gitops/tools/supply-05-fix-04/`, `docs/evidence/supply-05-fix-04/`) | `SUPPLY-05-FIX-03` | `ARGO-ROOT` | `DEMO-ONPREM-01`, Wazuh lifecycle | ImageValidatingPolicy의 Enforce/Fail과 namespace 전체 제외 0건을 유지하고 `wazuh/wazuh-manager-master-0`, `wazuh-manager` ServiceAccount, Wazuh manager·Vault init exact digest 조합만 system 예외로 허용했다. 다른 Pod 이름의 동일 이미지 조합은 upstream 직접 참조로 server dry-run 거부됐고, immutable SHA에서 manager StatefulSet을 실제 재기동해 새 Pod UID와 `1/1 Ready`, 관련 Argo Application의 `Synced/Healthy`를 확인했다. 검증 뒤 임시 예외·test Pod 0건, literal main 복구와 최종 main 상태를 확인했다. |
| `OBS-20 DONE` | 기존 Prometheus·Alertmanager·`#platform-alerts` 경로에 Argo child 비정상, Velero controller target 부재, 마지막 성공 정기 backup freshness 경보만 추가 (`gitops/apps/obs/`) | `SECOPS-AUDIT-01`, `SUPPLY-05-FIX-01`, `CI-01-FIX-01`, `OBS-18` | `ARGO-ROOT` | GitOps·백업 장애의 외부 인지 | Argo child 비정상(`ArgoCDApplicationDegraded`), Velero controller target 부재(`VeleroTargetDown`), 마지막 성공 정기 backup freshness 36시간 초과(`VeleroBackupFreshnessStale`) 3개 규칙을 PrometheusRule 및 Alertmanager allowlist에 등록했다. 정상 baseline firing 0건 확인, 승인된 test label 경보 1회의 firing 수신 및 TTL 만료 후 resolved 수신을 확인했다. 신규 exporter·서비스 중단·실패 백업 생성 0건 및 기존 receiver payload allowlist를 유지하고 immutable SHA 및 main에서 `platform-root`와 `obs` Application `Synced/Healthy`를 달성했다. |
| `POL-03 DONE` | 2026-09-02 만료 예정 Argo CD·CrowdSec·Velero `runAsNonRoot` 예외를 제거하거나 최소화하고 Velero namespace의 `Pod/*` wildcard를 node-agent·PodVolumeBackup hosting Pod의 증명 가능한 exact 경계로 교체 (`policies/`, 관련 `gitops/apps/`) | `SECOPS-AUDIT-01`, `POL-02` | `ARGO-ROOT` | 2026-09-02 이후 배포·백업 지속성 | Argo CD 전체 워크로드(Deployments 6종, StatefulSet 1종) 및 CrowdSec `whoami` Deployment에 Pod-level `securityContext.runAsNonRoot: true`를 적용했다. `pol-02-argocd-run-as-non-root` PolicyException을 완전히 제거하고, `pol-02-crowdsec-run-as-non-root`와 `pol-02-velero-run-as-non-root`에서 2026-09-02 만료 조건을 제거했으며 Velero 예외 대상을 `names: ["*"]` wildcard에서 exact `node-agent-*` (Pod) 및 `node-agent` (DaemonSet)으로 좁혔다. 정상 admission 및 미승인 root Pod 생성 거부(`velero`, `argocd` 네임스페이스)를 확인하고, Velero ad-hoc backup `Completed` 및 관련 child(`policy-baseline`, `crowdsec`, `velero`, `platform-root`) `Synced/Healthy`를 달성했다. |
| `POL-03-FIX-01 DONE` | `POL-03`가 누락한 Velero v1.18.2 `PodVolumeBackup` hosting Pod 예외를 label/name·controller ownerRef·ServiceAccount 요청의 교집합으로 복구 (`policies/`, `gitops/tools/pol-03-fix-01/`) | `POL-03` | `ARGO-ROOT` | `DEMO-RECOVERY-01`, Velero filesystem backup 지속성 | 일반 `velero` root Pod 허용이나 namespace-wide wildcard를 되살리지 않았다. `velero.io/pod-volume-backup=<Pod 이름>`, `velero.io/v1` `PodVolumeBackup` controller ownerRef, `velero-server` ServiceAccount 요청의 세 조건으로만 `POL-01` Pod-level `runAsNonRoot`를 우회했다. immutable child `policy-baseline=df921598c9f1bdaaeba229e1ff4782c999ed4389`에서 같은 ServiceAccount·PVB label이지만 ownerRef 없는 dry-run root Pod가 `POL-01`에 거부되고, 기존 Gitea opt-in PVC의 read-only Kopia Backup/PVB 1건이 `Completed`(전송 bytes>0)인 것을 확인했다. task-owned Backup/PVB는 삭제됐고, literal main `c1d9ed6af3dc9a482e9eb610727ce627d551b400`의 `platform-root`·`policy-baseline`·`velero`가 `Synced/Healthy`로 복귀했다. |
...
| `UPDATE-02 DONE` | smoke repository 한 곳만 보는 Renovate를 실제 권위 저장소에 최소 manager·PR-only로 채택할지 폐기할지 결정하고 선택한 한 경로만 실행 (`gitops/apps/renovate/`, credential 원본) | `SECOPS-AUDIT-01`, `UPDATE-01`, `SUPPLY-05-FIX-01`, `SUPPLY-05-FIX-03` | `ARGO-ROOT`, `VAULT-CONFIG` | 실제 의존성 업데이트 또는 불필요 자동화 제거 | Renovate를 실제 권위 저장소 `ktcloud4-bean/hr-system`에 채택하고, exact repository·manager(`npm`, `pip_requirements`) allowlist, onboarding·autodiscover·script·automerge 비활성, 동시 PR/Branch limit 1개와 fine-grained PAT 호환을 적용했다. Vault KV `kv/renovate/runtime`의 `github_token` 주입, GitHub 푸시 권한, 단발 CronJob test Job 실행, 단일 dependency Canary PR `#16`(`chore(deps): update dependency boto3 to v1.43.72`) 생성 확인, Proxmox/Guest 메모리·root 디스크·PVC 용량 점검 및 검증 후 test Job/Canary PR/branch 자동 정리를 live 검증했다. 상세 증거: [`docs/evidence/update-02/README.md`](evidence/update-02/README.md). |
| `QUALITY-02 DONE` | SonarQube가 실제 애플리케이션 source와 release gate에 쓰이는지 저장소·CI 양쪽에서 판정하고, 실사용이면 최소 한 제품 pipeline을 연결하며 아니면 별도 폐기 작업의 데이터 보존·승인 조건을 확정 (`gitops/apps/sonarqube/`, 실제 source CI) | `SECOPS-AUDIT-01`, `QUALITY-01` | 없음 | 품질 gate 실사용 또는 30 GiB PVC 회수 결정 | 실제 제품 source·CI에 Sonar 참조가 없고 라이브 project가 합성 `quality01-pass`·`quality01-fail`뿐임을 확인했다. 최근 분석은 2026-08-02 두 건이며 gate는 각각 `OK`·`ERROR`, 제품 owner 0건이다. 실제 pipeline 연결과 live 삭제는 하지 않고 backup·삭제 승인·credential 회수·E2E 합성 gate 의존성을 `QUALITY-03`으로 문서화했다. 상세 증거: [`docs/evidence/quality-02/README.md`](evidence/quality-02/README.md). |
| `QUALITY-03 DONE` | 제품 소유자의 SonarQube 유지 결정을 반영해 기존 폐기 경로를 철회하고 HR System의 테스트 계층·80% coverage·main release gate·대시보드 경계를 확정 ([ADR-0029](adr/0029-hr-system-testing-and-sonarqube-release-gate.md), `docs/evidence/quality-03/`) | `QUALITY-02`, `CI-01-FIX-01`, `SUPPLY-07` | 없음 | `QUALITY-04`, `QUALITY-05`, `QUALITY-06` | private GitHub `ktcloud4-bean/hr-system` `main` tree `29ae41d42cb6dc30d53d31a37e22643b6fec5ec7`이 FastAPI 2개·React/Vite 1개와 공급망 Jenkinsfile만 가지며 test·coverage·JUnit·Sonar 설정은 0건임을 read-only로 재확인했다. Community Build를 지원 가능한 main release gate로 한정하고 컴포넌트별 초기·지속 80% floor, 실제 PostgreSQL integration, 별도 read-only Playwright E2E, Jenkins JUnit·Sonar 두 대시보드의 소유권과 `QUALITY-04`~`06` 순서를 확정했다. SonarQube·PVC·DB·credential 및 라이브 변경은 0건이다. 상세 증거: [`docs/evidence/quality-03/README.md`](evidence/quality-03/README.md). |
| `QUALITY-04 DONE` | 권위 GitHub `ktcloud4-bean/hr-system`에 세 컴포넌트의 단위·API component·service integration test와 컴포넌트별 80% coverage floor를 구현하고 테스트가 드러낸 transaction·UI 오류 처리를 최소 보정 | `QUALITY-03` | 없음 | `QUALITY-05` | 두 FastAPI service에 pytest·TestClient·dependency override와 branch coverage를 적용하고 실제 임시 PostgreSQL에서 migration 2회 멱등성, PUBLIC/schema 제한, employee role read-only·HR role DML-only, auth header 누락·정규화·미등록 404·비HR 403·HR 대상 은닉 404·본인 조회만 허용·중복 email을 검증한다. 신규 직원+감사이력과 직급·급여 변경+감사이력은 failure injection에서도 partial commit 0건이어야 한다. frontend는 Vitest·React Testing Library로 `www`/`admin` route, 인증 실패, form·modal, API 부분 실패를 검증하고 성공하지 않은 변경을 성공처럼 닫지 않는다. `employee-service`·`hr-service`는 branch 측정 총계 각각 80% 이상과 각 `coverage.xml`·`test-results/junit.xml`, frontend는 lines·functions·branches·statements 각각 80% 이상과 `coverage/lcov.info`·`test-results/junit.xml`을 생성한다. EKS·Aurora·Keycloak·실사용자 변경은 0건이다. 상세 증거: [`docs/evidence/quality-04/README.md`](evidence/quality-04/README.md). |
| `QUALITY-05 DONE` | HR System의 제품 test·JUnit·coverage·Sonar scan을 기존 Jenkins에 연결해 image build보다 앞선 main release gate로 운영 (`gitops/apps/jenkins/`, 권위 `ktcloud4-bean/hr-system/Jenkinsfile`) | `QUALITY-04`, `CI-01-FIX-01`, `SUPPLY-05-FIX-02`, `SUPPLY-07` | `ARGO-ROOT`, `VAULT-CONFIG` | `QUALITY-06`, HR release 품질 gate | GitHub와 Gitea mirror `main` exact SHA 일치 뒤 같은 SHA를 test·분석·build하고, pinned JUnit plugin과 PVC 없는 임시 PostgreSQL sidecar(검증·서명된 Harbor digest)를 추가했다. 합성 token과 분리한 `hr-system` project-scoped token을 Vault에서 파생하고 Python Cobertura 2개·frontend LCOV를 한 project에 import하며 안정 release version과 Sonar way new-code gate를 사용한다. pipeline은 test→`finally` JUnit/coverage 게시→Sonar scan·gate wait→image build→Trivy/SBOM/Cosign/Harbor 순서를 강제한다. 최종 제품 SHA `fbeae1d43011e40080acf1b3da7e1a9dc5a80ced`에서 양성 #41은 JUnit 25/0, Sonar gate PASS, 세 컴포넌트 release handoff를 확인했고, fixture 음성 #42는 JUnit 26/1을 남기고 image build·push·release handoff 0건으로 종료했다. 상세 증거: [`docs/evidence/quality-05/README.md`](evidence/quality-05/README.md). |
| `SUPPLY-01-FIX-01 DONE` | EKS Kyverno `ImageValidatingPolicy`의 TUF mirror 누락과 과거 digest allowlist 때문에 신규 HR image의 Cosign 서명·CycloneDX attestation 검증이 실패한 결함을 최소 보정 (`gitops/apps/kyverno-eks/policies/`, `docs/evidence/supply-01-fix-01/`) | `SUPPLY-01`, `SUPPLY-05-FIX-03`, `QUALITY-05` | `ARGO-ROOT` | `QUALITY-06` | 전용 private TUF CONNECT egress를 `10.10.20.12:8445`로 구성하고 EKS node SG·OPNsense·k3s 주소를 exact 범위로 연결했다. current·previous `tuf.mirror`, CycloneDX attestation 검증, `failurePolicy=Fail`, `validationActions=Deny`, bootstrap/system exact 예외와 pinned TUF root를 유지했다. immutable `platform-root`·`obs`·`kyverno-eks` `Synced/Healthy/Succeeded`와 server-side admission PASS를 확인했으며 상세 증거는 [`docs/evidence/supply-01-fix-01/README.md`](evidence/supply-01-fix-01/README.md)다. |
| `SUPPLY-01-FIX-02 DONE` | EKS `hr-db-migrate` PreSync가 Pod를 생성하지 못한 채 deadline을 초과한 원인을 Kyverno controller의 `failed to build cosign verification opts: getting Rekor public keys: rekor URL must be provided`로 특정하고, EKS `ImageValidatingPolicy`의 Rekor/CTLog 검증 계약을 최소 보정 (`gitops/apps/kyverno-eks/`, `gitops/tools/supply-01-fix-02/`, `docs/evidence/supply-01-fix-02/`) | `SUPPLY-01-FIX-01` | `ARGO-ROOT`, `IDENTITY-LIVE` | HR desired release 수렴, EKS 공급망 admission | EKS Kyverno `ImageValidatingPolicy`에 `ctlog.url: https://rekor.sigstore.dev` 및 `insecureIgnoreTlog: true`를 적용해 `rekor URL must be provided` 오류를 원천 차단하고 `verifyImageSignatures` short-circuit OR를 구성했다. Git 선언 3종 이미지 및 CycloneDX SBOM attestation의 Cosign 공개키 독립 검증 PASS, server dry-run 양성 허용 및 미서명/tag-only 음성 거부, Kyverno 컨트롤러 로그 Rekor 오류 0건, `hr-db-migrate` Job Pod 어드미션 통과 및 `Completed`(`Succeeded`), 최신 main의 `platform-root`·`kyverno-eks`·`hr-system` `Synced/Healthy`를 확인했다. 상세 증거: [`docs/evidence/supply-01-fix-02/README.md`](evidence/supply-01-fix-02/README.md). |
| `SUPPLY-01-FIX-03 DONE` | `SUPPLY-01-FIX-02`가 Rekor 장애를 고치며 함께 제거한 CycloneDX SBOM attestation validation을 복원하고, 실제로 admission에서 검증 가능한 형태로 재설계 (`gitops/apps/kyverno-eks/`, `gitops/tools/supply-07/`, `ktcloud4-bean/hr-system` Jenkinsfile) | `SUPPLY-01-FIX-02`, `SUPPLY-07` | `ARGO-ROOT` | `DEMO-RECORD-01` | 원본 CEL(`f4e0b5b7`)을 그대로 복원해 실측한 결과 지금 배포된 digest는 통과하지 못함을 확인: `replicate.py`가 referrer의 재귀 서명을 복제하지 않고, 근본적으로 Jenkins의 SBOM 서명 방식(별도 OCI artifact를 만든 뒤 그 digest를 서명)이 Kyverno `verifyAttestationSignatures()`가 요구하는 signed in-toto attestation 모델과 다름을 규명했다. Jenkinsfile에 `cosign attest --type cyclonedx`를 추가해 image subject에 직접 서명된 DSSE attestation을 생성하도록 바꾸고, 정책의 `attestations.sbom`을 `referrer`에서 `intoto: {type: https://cyclonedx.org/bom}`로 전환하고 `extractPayload(...).predicate.bomFormat`으로 경로를 수정했다. `replicate.py`에 attestation legacy tag(`.att`) 복제 단계를 추가했다. Jenkins build #45(Trivy CVE-2026-53615로 두 차례 재시도 후 `python:3.12-slim` digest 상향 및 임시 `.trivyignore`로 통과)로 만든 실제 서명 digest 3종을 ECR로 복제하고 `cosign verify-attestation` PASS, EKS server dry-run `ALLOWED`, 실제 GitOps 재배포로 3개 Deployment 모두 새 digest `Ready 1/1` 전환을 확인했다. 검증 뒤 `platform-root`를 main SHA로 되돌렸다. |
| `SUPPLY-01-FIX-05 DONE` | `SUPPLY-01-FIX-03`(build #45) 이후 employee-service·hr-service가 2시간 이상 CrashLoopBackOff로 멈춰있던 결함을 근본 수정 (`gitops/apps/jenkins/`, `gitops/root/`, `gitops/apps/hr-system/`, `ktcloud4-bean/hr-system` Jenkinsfile) | `SUPPLY-01-FIX-03` | `ARGO-ROOT` | 없음 | 원인을 로그로 특정: hr-system의 "Python runtime dependency gate" 스테이지가 AWX-04 EE(Alpine/musl) 전용으로 설계된 Jenkins `python` sidecar 컨테이너를 공유해서, glibc(Debian) 런타임 base(`python:3.12-slim`)에 musl로 컴파일된 `pydantic_core` 확장을 심어 `ModuleNotFoundError: No module named 'pydantic_core._pydantic_core'`를 유발했다(build #45가 이 컨테이너를 쓴 첫 hr-system 빌드였다). `python:3.12-slim@sha256:2c941e86...`을 Harbor `curated-platform/python`으로 승격(Cosign 서명+CycloneDX SBOM attestation)하고, AWX-04용 기존 `python` 컨테이너는 그대로 둔 채 hr-system 전용 `python-glibc` 컨테이너를 Jenkins pod 템플릿에 추가했다. hr-system Jenkinsfile의 dependency gate 스테이지만 `container('python-glibc')`로 전환했다. ARGO-ROOT 절차로 `jenkins`·`hr-system` Application을 검증 SHA로 순차 pin해 라이브 검증하는 과정에서 `jenkins` Deployment가 Kustomize configMapGenerator 해시 불일치로 일시 다운되는 사고가 발생했으나(제 patch 절차 오류로 child Application을 `platform-root` 경유 없이 직접 pin해 self-heal과 충돌한 것이 원인), main 렌더링 결과로 누락된 ConfigMap을 직접 생성해 즉시 복구하고 이후 정식 `platform-root` pin 절차로 재검증해 재발하지 않았다. Jenkins build #46으로 employee-service·hr-service를 재빌드했고, `cosign attest`가 이번에도 비결정적으로 legacy tag를 만들어 최초 digest가 Kyverno SBOM 게이트에서 거부됐으나 수동 재-attest로 genuine OCI referrer를 확보해 통과시켰다. ECR 복제 뒤 GitOps digest를 재범프해 실제 EKS Pod가 각각 재시작 0회로 `1/1 Running` 전환함을 확인했다(기존 CrashLoop Pod 소멸, 중복 Pod 0개). frontend는 이번 결함과 무관해 digest를 올리지 않았다. 검증 뒤 `platform-root`·child `targetRevision`을 모두 main으로 되돌렸다. |
| `QUALITY-06 DONE` | 검증 digest 배포 뒤 전용 합성 identity·fixture로 HR System의 최소 read-only Playwright E2E를 별도 실행하고 Jenkins test 이력에 합류 | `QUALITY-05`, `AWS-HR-03-FIX-01`, `SUPPLY-01-FIX-01` | `ARGO-ROOT`, `IDENTITY-LIVE`, `VAULT-CONFIG` | 배포 후 사용자 경로 검증 | 합성 employee·HR identity와 합성 data만으로 Chromium 1 worker·retry 0의 세 read-only 흐름을 Jenkins #26에서 통과했다(JUnit 3/0/0). 배포 frontend에 history UI action이 없어 profile UI와 본인 전용 history API를 같은 browser session에서 확인했다. 상세 증거: [`docs/evidence/quality-06/README.md`](evidence/quality-06/README.md). |
| `OPS-DRIFT-01 DONE` | 기존 도구를 재사용해 Argo Application, OPNsense 마스킹 snapshot, AWS·Proxmox OpenTofu의 read-only 정기 drift 판정과 통지를 구성하되 자동 apply·snapshot update를 금지 (`gitops/apps/jenkins/`, 기존 drift·plan 도구) | `SECOPS-AUDIT-01`, `CI-01-FIX-01`, `OPN-DRIFT-01`, `AWS-CI-FIX-02`, `IAC-01` | `ARGO-ROOT`, `OPNSENSE-LIVE`, `PVE-LIVE`, `TOFU-STATE` | 선언과 라이브 불일치의 조기 발견 | OPNsense `check-drift.sh` 및 Proxmox OpenTofu 라이브 drift 0건 확인, `--skip-if-locked` 잠금 skip 모드 동작, 비밀 없는 fixture 4종(Argo, OPNsense, AWS Tofu, PVE Tofu) drift 감지 및 요약 알림 파싱, `check-drift.sh --update` 및 `tofu apply` 미호출(순수 read-only), 민감 plan/secret 출력 0건, Jenkins JCasC `ops-drift-check` Cron/Job 선언 및 회귀 테스트 통과. 상세 증거: [`docs/evidence/ops-drift-01/README.md`](evidence/ops-drift-01/README.md). |
| `BACKLOG-FIX-01 DONE` | 백로그 표의 ID 유일성, 선행 ID 존재, 허용 상태, DONE이 아닌 선행을 가진 READY 금지를 검사하는 정적 linter를 추가하고 기존 `SUPPLY-01`·`SUPPLY-02` 중복 및 `AWS-APP-01` 누락 선행을 단일 원본으로 정리 (`docs/backlog.md`, `scripts/`) | `SECOPS-AUDIT-01` | 없음 | 백로그 작업 배정 정확성 | `scripts/tests/test-backlog-lint.sh` 1회에서 현재 문서·clean fixture PASS 및 duplicate ID·missing predecessor·invalid status·premature READY fixture 각각 명시적 실패 확인, `scripts/check-backlog.sh`는 Bash 배열과 `set -euo pipefail` 사용, 저장소 문서 외 라이브 변경 0건. |
| `DEMO-PLAN-01 DONE` | Notion `침해 사례`의 사실·확정도를 출처와 대조하고 온프레미스 제로트러스트 선언·라이브 통제를 분석해, 3~5분 데모 5편의 연결형 독립 시나리오와 최소 인프라 후속을 확정 (`docs/backlog.md`, Notion `데모 시나리오`) | `SECOPS-AUDIT-01`, `IAM-MIG-01`, `CROWDSEC-FIX-01`, `FALCO-01`, `WAZUH-04-FIX-01`, `SOAR-01`, `SUPPLY-05-FIX-03` | 없음 | `DEMO-ONPREM-01` | 침해 사례의 확정·후보·보류와 사실 보정 필요 항목을 분리하고, 사용자와 한 번에 한 질문씩 합의한 공통 가상 기업·실제 동작 취약점·음성/컷편집·오프닝 흐름 이미지 조건을 반영해 Zone 2·담당·기술 관계를 가진 [세션 1](https://app.notion.com/p/3bdec8c100c981e9b61ac19a3ea48573)·[세션 2](https://app.notion.com/p/3bdec8c100c981338e90f2f89b595174)·[세션 3](https://app.notion.com/p/3bdec8c100c9811488e2f27c59101093)·[세션 4](https://app.notion.com/p/3bdec8c100c981dc97ebc916b3f835c8)와 P1 [세션 5](https://app.notion.com/p/3bdec8c100c981709e7fe7c87e3fd571)를 중복 없이 생성했다. 예스24 사례의 잘못된 출처·미확인 원인 단정을 공식 출처 범위로 보정하고 [SGI서울보증 랜섬웨어 사례](https://app.notion.com/p/3bdec8c100c9816d8f9ded2fcb7db609)를 추가해 세션 5와 연결했다. 라이브·Secret·계정·방화벽 변경은 0건이다. |
| `DEMO-ONPREM-01 DONE` | 동일 가상 기업의 경량 합성 고객 포털과 결정론적 공격·통제·증거·reset 하네스를 추가해 온프레미스 데모 4편의 실제 촬영 인터페이스를 확정했다 (`gitops/apps/`, `gitops/tools/`, `docs/`, `docs/evidence/demo-onprem-01/`) | `DEMO-PLAN-01`, `SUPPLY-05-FIX-04` | `ARGO-ROOT`, `IDENTITY-LIVE`, `VAULT-CONFIG` | `DEMO-RECOVERY-01` | 신규 PVC·공개 DNS·OPNsense·실데이터 없이 합성 고객정보와 통제된 실제 취약점만 사용했다. 계정 탈취는 미승인 장비의 내부 경로 실패와 승인 장비에서 동일 계정의 대조 경로 200·그룹 통제 403, SQLi는 동일 payload의 대조군 노출과 CrowdSec 403·Wazuh 증거, 컨테이너 침해는 합성 DB 횡적 이동의 NetworkPolicy 차단·Falco/Wazuh 탐지·Shuffle 사람 Continue/Stop `clicked=true` 기록(자동 대응 0건), 공급망은 비승인 이미지 거부와 서명된 exact digest 실행을 각각 재현했다. 네 세션의 확정 화면 URL·입력·HTTP/연결 결과·rule/evidence marker·마스킹 경계를 문서화하고 한 명령 attack/control/evidence/reset을 제공하며, reset 뒤 transient 공격 자원 0건과 최신 main의 관련 Argo child `Synced/Healthy`를 확인했다. |
| `DEMO-RECOVERY-01 DONE` | 데모 전용 합성 고객 데이터와 소형 `local-path` PVC, 결정론적 훼손·격리 복원 하네스를 추가해 세션 5의 GitOps 선언 복구와 Velero/Kopia 데이터 복구 대조를 촬영 가능한 한 흐름으로 만들었다 (`gitops/apps/`, `gitops/tools/`, `docs/`) | `DEMO-ONPREM-01`, `BKP-02`, `BKP-10`, `POL-03`, `POL-03-FIX-01` | `ARGO-ROOT` | `DEMO-OPENING-01` | PVC 선언 합계 `111.125 GiB → 111.625 GiB`, root 여유 `71%`, `DiskPressure=False`에서 적용했다. immutable SHA `28161d5b2a9ed85ab77fee26f1ce498caa8f04f3`의 root·`demo-onprem`·`policy-baseline`이 `Synced/Healthy`일 때 합성 marker Backup/PVB `Completed`(63 bytes), Deployment UID 변경 뒤에도 source marker `corrupt` 유지, 격리 Restore/PVR `Completed`, normal marker hash·HTTP 200 및 harness RTO 측정을 확인했다. Velero v1.18.2 PVR hosting Pod에는 label=Pod명·PVR controller ownerRef·`velero-server` ServiceAccount 교집합 예외만 추가했다. reset 뒤 task-owned Backup/Restore/PVB/PVR/DeleteBackupRequest·격리 namespace·source PVC는 모두 0이고, literal main `742d1e66959098183e814fc612e22c167c784cd5`의 `platform-root`·`demo-onprem`·`policy-baseline`·`velero`가 `Synced/Healthy`로 복귀했다. 실제 랜섬웨어·기존 workload/PVC·AWS apply·AWS 화면·전체 DR 성공 주장은 0건이다. |
| `DEMO-PLAN-02 DONE` | 세션 4의 최종 촬영 대상을 일반 온프레미스 검증 Pod에서 실제 HR System 공급망 경계로 승격하고, 구현·오프닝·대본·대화형 촬영의 직렬 후속을 확정 (`docs/backlog.md`, Notion `데모 시나리오`) | `DEMO-PLAN-01`, `DEMO-ONPREM-01`, `DEMO-RECOVERY-01`, `SUPPLY-07`, `SUPPLY-01-FIX-02`, `QUALITY-06` | 없음 | `DEMO-AWS-HR-01`, `DEMO-OPENING-01` | [세션 4](https://app.notion.com/p/3bdec8c100c981dc97ebc916b3f835c8)를 Zone 1 AWS의 “급여 시스템에 심어진 가짜 릴리스”로 보정해, 통제된 미서명 fixture의 EKS server-side dry-run `DENIED`, 서명·CycloneDX attestation이 검증된 ECR exact digest의 `ALLOWED`, 기존 HR 3개 Deployment·포털 무중단과 Argo `hr-system` `Synced/Healthy`를 한 사건 흐름으로 확정했다. 실제 악성코드·실데이터·운영 GitOps digest 변경·촬영 중 pipeline 재실행은 금지하며, `DEMO-ONPREM-01`의 기존 공급망 fixture는 당시 완료 증거와 fallback으로 보존한다. 이미지·대본 뒤 사용자가 실제 녹화하고 에이전트가 채팅으로 화면·타이밍·기술 정확성·민감정보 노출을 검토하는 `DEMO-RECORD-01`을 별도 후속으로 추가했으며 라이브·Secret 변경은 0건이다. |
| `DEMO-AWS-HR-01 DONE` | HR System 세션 4의 결정론적 촬영 인터페이스를 기존 EKS 공급망 통제 위에 추가 (`gitops/tools/`, `docs/`, `docs/evidence/`) | `DEMO-PLAN-02`, `SUPPLY-07`, `SUPPLY-01-FIX-02`, `QUALITY-06` | 없음 | `DEMO-OPENING-01` | 동일 frontend Pod shape·`hr-system` namespace의 inert tag-only ECR fixture는 server-side dry-run `DENIED`, 현재 signed exact digest는 `ALLOWED`였다. 음성 요청 전후와 양성 요청 뒤 모두 새 ReplicaSet·Pod 0, 기존 `frontend`·`employee-service`·`hr-service` Ready·digest·UID 불변, 합성 identity history read-only 200, Argo `hr-system` `main`/`Synced`/`Healthy`를 한 번의 `prove`로 확인했다. build·replication·GitOps digest 갱신·실제 Deployment patch·악성코드·실데이터·Secret 출력은 0건이며 reset은 remote delete 0의 no-op이다. 상세 증거: [`docs/evidence/demo-aws-hr-01/README.md`](evidence/demo-aws-hr-01/README.md). |
| `DEMO-OPENING-01 DONE` | 세션 1·2·3·5의 온프레미스 흐름과 세션 4의 HR System/EKS 흐름으로 데모 5편의 16:9 오프닝 이미지용 영문 프롬프트를 만들고, 사용자가 chatgpt.com에서 생성한 후보를 함께 보정해 세션별 최종 이미지 한 장을 선정한다 (Notion `데모 시나리오`) | `DEMO-RECOVERY-01`, `DEMO-AWS-HR-01` | 없음 | `DEMO-SCRIPT-01` | 세션 1~5의 사용자가 첨부·승인한 16:9 최종 PNG를 유지한 채 각 Notion 페이지에 재현용 영문 이미지 사양(원 생성 prompt 미제공을 명시), 금지 조건, 공식 아이콘 참조와 선정 이미지를 기록했다. 다섯 페이지 모두 최종 이미지·영문 사양·금지 조건·공식 출처가 함께 존재함을 한 번 확인했으며 Secret·실데이터·가짜 UI·읽을 수 없는 생성 텍스트는 0건이다. |
| `DEMO-SCRIPT-01 DONE` | 확정된 촬영 인터페이스와 선정된 오프닝 이미지를 기준으로 데모 5편의 실제 녹화·후시녹음 대본을 작성한다 (Notion `데모 시나리오`) | `DEMO-OPENING-01` | 없음 | `DEMO-RECORD-01` | 다섯 Notion 페이지의 `데모 흐름`에 각 6장면·3~4분 분량의 오프닝 설명, 화면 동작, 자연스러운 내레이션, 컷, 실제 확인된 URL·입력·상태·rule ID, STOP 조건을 기록하고 사용자가 승인했다. 세션 1~3은 온프레미스 attack/control/evidence/reset, 세션 4는 HR/EKS server-side dry-run `DENIED`/`ALLOWED`와 기존 서비스 무중단, 세션 5는 격리 복원 `backup`/`attack`/`restore`/`reset`만 사용한다. Secret·JWT·cookie·OTP·실데이터·가짜 UI·자동 Shuffle 승인은 0건이며 실제 리허설과 최종 take 판정은 `DEMO-RECORD-01`이 소유한다. |
| `DEMO-RECORD-01 READY` | 사용자가 데모 5편을 한 편씩 실제 녹화하면서 에이전트와 채팅으로 리허설·문제 판별·재촬영 피드백을 반복해 최종 take를 확정 | `DEMO-SCRIPT-01` | 없음 | 데모 영상 편집·최종본 | 세션마다 에이전트가 촬영 전 화면 순서·창/확대·예상 결과·reset·민감정보 마스킹 checklist로 GO/STOP을 판정하고, 사용자가 녹화한 화면·스크린샷·구간별 결과를 채팅으로 공유하면 기술 주장·가독성·호흡·컷·3~5분 길이를 검토한다. 문제는 실패 화면과 원인을 먼저 기록한 뒤 대본·동선 또는 안전한 fixture만 보정해 같은 세션을 재촬영하고, 사용자가 최종 take를 승인한다. 에이전트는 recorder·사람 승인 버튼·credential 입력을 대신 조작하지 않으며, 다섯 최종 take 모두 실제 구현 증거와 일치하고 JWT·cookie·OTP·Secret·실데이터·가짜 성공 장면 노출 0건, 각 촬영 뒤 attack/control/evidence/reset 또는 HR dry-run no-op 정리를 확인한다. |
...
|---|---|---|---|
| A — k3s/Argo 복구 | `SUPPLY-01-FIX-02` | `SUPPLY-05-FIX-01` → `CI-01-FIX-01` → `OBS-20` → `POL-03` → `SUPPLY-05-FIX-02` → `SUPPLY-05-FIX-03` → `UPDATE-02` → `OPS-DRIFT-01` → `SUPPLY-01-FIX-02` | 개발은 병렬 가능하지만 `ARGO-ROOT` 획득·라이브 검증·merge는 이 순서로 직렬화 |
| B — AWS credential | `AWS-SEC-07` | 단독 | lane A가 `TOFU-STATE`를 쓰지 않는 동안 plan·검증 병렬 가능; main merge만 직렬화 |
| C — 제품 품질 채택 | `QUALITY-04` | `QUALITY-03` → `QUALITY-04` → `QUALITY-05` → `QUALITY-06` | `QUALITY-04` source test는 live 잠금 없이 병렬 가능; Jenkins/Sonar 적용과 배포 E2E만 각 공유 잠금 아래 직렬화 |
...
`SUPPLY-05-FIX-02`와 `SUPPLY-05-FIX-03`이다. 이 여섯 작업으로 현재 backup·AWX·Jenkins 수렴과
admission 가용성을 먼저 복구하고 같은 유형의
...
squash merge한다. `UPDATE-02`는 `SUPPLY-05-FIX-03`까지의 공급망 보정 뒤, `OPS-DRIFT-01`은 Jenkins 수렴과 `OBS-20` 뒤에만
착수한다. `QUALITY-04`는 권위 제품 저장소의 source test만 소유하므로 lane A의 `ARGO-ROOT`
작업과 병렬 착수할 수 있다. `QUALITY-05`는 `SUPPLY-05-FIX-02`와 `QUALITY-04`가 모두 끝난 뒤,
`QUALITY-06`은 제품 release gate가 라이브에서 증명된 뒤에만 연다.
...
2026-08-16 read-only 후속 진단에서 위 FIX 등록 조건이 충족됐다. `hr-db-migrate` Job event는
Kyverno `eks-image-supply-chain-policy`가 Pod 생성을 반복 거부한 뒤 deadline을 초과했음을 보였고,
controller 로그는 current·previous attestor 모두 실제 signature 판정 전에
`failed to build cosign verification opts: getting Rekor public keys: rekor URL must be provided`로
실패했음을 확인했다. 따라서 migration Pod와 SQL은 실행되지 않았으며, 기존 workload가
`Healthy`인 동안 desired release만 `OutOfSync`로 막혀 있다. migration Job의 deadline이나 DB
코드를 추측으로 바꾸지 않고 EKS 공급망 검증 계약을 보정하는 `SUPPLY-01-FIX-02`를 `READY`로 연다.
...
`DEMO-SCRIPT-01`과 독립적으로 진행한다. 다만 발표 자료에서 네 번째 HR System 공급망
시나리오와 다섯 번째 복구 시나리오를 구현 완료로 설명하려면 각각 `DEMO-AWS-HR-01`과
`DEMO-RECOVERY-01` 증거를 최종 슬라이드의 선행으로 사용한다.
...
5. Bean Team 5인 구성, 6. 4주 간트 차트, 7. 세션 4의 HR 가짜 릴리스를 포함한 보안 검증 시나리오 5개,
8. 물리 경계, 9. 온프레미스 플랫폼, 10. HR System 전체 워크로드(`frontend`·`employee-service`·
`hr-service`, internal ALB, Aurora, IRSA/Secrets Manager 경계), 11. 온프레미스→Pomerium→S2S VPN→
EKS 보안 흐름, 12. 접근 통제, 13. HR Jenkins→Harbor→ECR→Kyverno 품질·공급망,
14. 관측·탐지·사람 승인, 15. 백업·복구, 16. HR 포털·Argo `hr-system`·Jenkins/SonarQube 중
한 화면에서 읽히는 배포/품질 증거, 17. EKS Kyverno `DENIED`/`ALLOWED`와 Grafana·Wazuh/Shuffle 중
한 화면에서 읽히는 운영/보안 증거, 18. HR 기존 서비스 정상 상태를 포함한 시나리오 결과 매트릭스,
...
| ID·상태 | 작업과 소유 범위 | 선행 | 잠금 | 영향 | 완료 증거 |
|---|---|---|---|---|---|
| `PRESENT-PLAN-01 DONE` | 사용자가 제공한 기존 Draw.io·22장 발표자료·15장 원본 템플릿과 현재 아키텍처·데모 증거를 대조하고, 한 번에 한 질문씩 인터뷰해 22장 정보 구조·15분 범위·산출물 위치·작업 순서를 확정 (`docs/backlog.md`) | 없음 | 없음 | `PRESENT-PLAN-01-FIX-01` | 외부 침해사례 placeholder·기술 대체제 비교·다중 리전/가짜 HA·자동 차단 주장을 제외하고 단일 물리 노드 SPOF, 사람 승인, 합성 복구 범위를 명시했다. Draw.io는 한 페이지 참조 원본, PPT는 생성 이미지와 중복 슬라이드 강조, 간트·SWOT은 native 도형 우선, 실제 UI는 마스킹한 최신 화면으로 확정했다. 제목 `온프레미스–AWS 하이브리드 보안 플랫폼`, `Bean`·발표자 `고광근`, 팀원 5명, 22장 순서와 약 14분 5초 본문, 마지막 작업을 발표 대본으로 고정했다. 라이브·Secret·원본 파일 변경 0건이고 `scripts/check-backlog.sh`를 통과한다. |
| `PRESENT-PLAN-01-FIX-01 DONE` | `PRESENT-PLAN-01` 이후 확정된 HR System/EKS 세션 4를 반영해, 발표 전체를 HR 애플리케이션 소개로 바꾸지 않고 AWS 대표 워크로드의 전체 구조·품질/공급망·실제 증거를 22장·15분 계약 안에 명시 (`docs/backlog.md`) | `PRESENT-PLAN-01`, `DEMO-PLAN-02`, `AWS-HR-01`, `SUPPLY-07`, `SUPPLY-01-FIX-02`, `QUALITY-06` | 없음 | `PRESENT-ARCH-01` | 제목·22장·약 14분 5초·온프레미스 중심 구조를 유지하고 7·10·13·16~18장의 HR 역할을 명시했다. HR은 private EKS·internal ALB·Aurora·IRSA/Secrets Manager의 AWS 대표 워크로드이며, Jenkins→Harbor→ECR→Kyverno와 합성 E2E의 검증된 경계만 사용한다. 최종 성공 주장은 `DEMO-AWS-HR-01`의 dry-run `DENIED`/`ALLOWED`·기존 서비스 정상 증거 뒤에만 허용한다. 라이브·Secret·원본 파일 변경 0건이고 `scripts/check-backlog.sh`를 통과한다. |
| `PRESENT-ARCH-01 DONE` | Git 선언과 현재 완료 증거를 대조한 source matrix를 만들고, 발표 이미지 검수 기준이 될 한 페이지 16:9 가로형 논리 아키텍처와 공식 아이콘 출처를 작성 (`docs/presentation/architecture/`, `docs/presentation/assets/icons/`, `docs/presentation/assets/SOURCES.md`, `docs/presentation/README.md`) | `PRESENT-PLAN-01-FIX-01` | 없음 | `PRESENT-DECK-SKELETON-01` | 3300×1856(16:9) 한 페이지에 OPNsense·Suricata·Proxmox·단일 k3s와 전용 VM 4종, S2S VPN·private EKS·Aurora·ECR·Secrets Manager/IRSA·KMS·S3를 배치하고 접근·공급망·탐지/사람 승인·백업/복구·독립 복구 다섯 흐름을 색으로 구분했다. HR은 Pomerium→S2S VPN→internal ALB→`frontend`·`employee-service`·`hr-service`→Aurora 사용자 경로와 Jenkins→Harbor→ECR→Argo CD/Kyverno 공급 경로로 같은 수준에서 그렸고 온프레미스가 중심인 균형을 유지했다. Proxmox·k3s에 `허용한 SPOF` 표기를 넣고 Pod·IP·port·도메인·계획 상태는 넣지 않았다. `architecture.md`의 `Board Demo` 행은 `BOARD-DEMO-02`·`AWS-HR-02`로 이미 제거된 stale 선언임을 대조해 그림에서 제외하고 `docs/presentation/README.md`에 별도 보정 필요로 기록했다. 아이콘 35종은 CNCF artwork 11종·제품 공식 저장소 6종을 우선하고 나머지 18종만 Dashboard Icons에서 받아 원본 URL·license·상표 경계를 `SOURCES.md`에 적었으며 AWS는 draw.io 내장 AWS Architecture Icons를 썼다. 아이콘 35종의 실제 파일 형식이 확장자와 모두 일치함(제품 사이트가 SPA면 없는 경로에도 HTTP 200과 HTML을 돌려주므로 상태 코드만으로 판정하지 않는다), `drawio-skill` validator 구조 오류 0건, preview PNG 육안 검토에서 잘림·겹침·깨진 아이콘·저대비 0건, `.svg`/`.drawio.png` embed와 재열기 성공, source matrix 누락 0건, Secret·실데이터 0건을 확인했다. |
| `PRESENT-DECK-SKELETON-01 DONE` | 원본 템플릿의 검정·흰색·neon green 자산을 선별해 7~8개 재사용 layout을 만들고, 실제 제목·핵심 메시지·시간·자산 placeholder가 있는 22장 와이어프레임 PPTX를 작성 (`docs/presentation/ktcloud4-bean.pptx`, `docs/presentation/README.md`) | `PRESENT-ARCH-01` | 없음 | `PRESENT-VISUAL-01`, `PRESENT-EVIDENCE-01` | 15장 원본 템플릿에서 캔버스 26.667×15in, `000000`·`FFFFFF`·`15E954`·`F6F6F6`·`666666`, Pretendard, 검정 헤더 바·각진 사각형·좌상단 green 블록·화살표 반복 모티프를 뽑아 `COVER`·`AGENDA`·`BODY`·`FULL-IMAGE`·`EVIDENCE-2`·`MATRIX`·`SWOT`·`CLOSE` layout 8종으로 재현했다. 원본은 해커톤 주제 사진 자산 중심이라 슬라이드를 복제하지 않고 색·모티프·타이포 규칙만 옮겼으며 원본 파일은 변경하지 않았다. 22장 순서가 확정 계약과 일치하고 7장은 세션 4 HR 가짜 릴리스를 포함한 5개 시나리오, 10장은 HR 전체 워크로드, 13장은 HR 품질/공급망, 16~17장은 두 화면 증거와 판정 문구, 18장은 `DENIED`/`ALLOWED`·기존 서비스 정상 결과 계약을 가진다. 8~11장은 이미지 placeholder를 `914400, 2651760, 22555505, 9692640` EMU로 고정해 네 장이 동일하고 강조 레이어만 다르다. Lorem Ipsum 없이 실제 제목과 `PURPOSE`·`MESSAGE`·`ASSET`·`EVIDENCE` 계약만 넣었고 같은 내용을 speaker notes 22장에 담았다. 본문 시간 합계는 845초(14분 5초)다. Office validator `All validations PASSED`, `markitdown` 22장 순서 일치와 잔여 placeholder 0건, PowerPoint timing·transition 각 0건, LibreOffice/PDF 22장 육안 검토에서 잘림·겹침·저대비 0건을 확인했다. 최종 기술 내용·생성 이미지·실제 UI는 넣지 않았다. validator는 템플릿 파생이 아니라 새로 생성한 산출물이므로 `--original` 없이 실행했다. |
| `PRESENT-VISUAL-01 DONE` | 참조 Draw.io와 와이어프레임 안전영역을 기준으로 발표용 16:9 아키텍처 이미지를 draw.io native 도형과 공식 아이콘으로 직접 제작하고 사용자 승인을 받는다 (`docs/presentation/assets/generated/`, `docs/presentation/assets/SOURCES.md`) | `PRESENT-ARCH-01`, `PRESENT-DECK-SKELETON-01` | 없음 | `PRESENT-DECK-01` | 생성 모델 prompt 방식을 폐기하고 직접 제작으로 전환했다. chatgpt.com이 prompt를 이미지 생성이 아니라 자료 검색으로 처리해 Kubernetes 공식 로고를 반환했고, 생성 지시를 명시한 v2로도 통제 가능성이 낮다고 판단했다. `presentation-architecture.drawio`와 3202×1802(16:9) PNG를 native 도형과 vendor 공식 아이콘으로 만들었으며 외부 생성 모델 사용 0건이다. 검정·흰색·neon green `#15E954` 세 색만 쓰고, 접근·인증 / 공급망·배포 / 탐지·사람 승인 / 보호·복구 네 그룹을 명시적 박스로 묶어 발표 화면에서 읽히도록 요소를 줄였다. On-Premises와 AWS 경계가 분리되고 두 영역을 잇는 사설 경로는 Site-to-Site VPN 하나이며, 탐지 흐름은 `사람 승인`에서 멈춘다. 단일 물리 노드에 `허용한 SPOF`를 표기하고 AWS 영역에는 HR System이 유일한 대표 워크로드이며 온프레미스가 중심임을 명시했다. 8~11장 placeholder가 2.327:1이므로 콘텐츠를 세이프존 y 216~1584 안(실제 236~1574)에 두어 상하가 잘려도 핵심이 남고, 같은 한 장을 8~11장과 어둡게 처리한 Q&A 배경에 재사용한다. 아이콘 재도안·비율 왜곡 0건, 가짜 UI·비밀·실데이터 0건이며 렌더 육안 검토에서 잘림·겹침·저대비 0건이다. 사용자가 최종본을 승인했다. |
| `PRESENT-EVIDENCE-01 DONE` | 최신 실제 상태의 HR 포털, Argo CD `hr-system`, Jenkins/SonarQube, EKS Kyverno, Grafana, Wazuh/Shuffle 화면을 발표 안전영역에 맞춰 캡처하고 민감정보를 마스킹한 증거 자산과 출처·판정 설명을 준비 (`docs/presentation/assets/evidence/`, `docs/presentation/assets/SOURCES.md`) | `PRESENT-DECK-SKELETON-01`, `DEMO-AWS-HR-01` | 없음 | `PRESENT-DECK-01` | 화면별 캡처 범위·마스킹 checklist를 먼저 확정한 뒤 4슬롯 6장을 실제 UI에서 확보했다. 16a는 Argo `hr-system`의 `Healthy`·`Synced to main`·`Sync OK`와 세 서비스 Healthy 트리, 16b는 `hr-system-image-build #41`의 11단계 전부 통과와 `#42`의 test gate 중단을 대비로 담아 gate 통과분만 승격됨을 화면으로 보였다. 17a는 `demo.sh prove`의 `result=DENIED`/`result=ALLOWED`와 합성 identity read_only=200, 기존 세 서비스 `ready=true uid_unchanged=true argo=Synced/Healthy`를 한 화면에 담았고, 17b는 Wazuh Threat Hunting 24시간 218,300건·agent 5종과 Shuffle 승인 대기 화면으로 탐지에서 사람 승인까지를 이었다. 마스킹은 모두 불투명 사각형이며 블러는 쓰지 않았다. 커밋 Author email 2곳과 Wazuh API id 1건을 가렸고, 발표 아키텍처에 이미 나오는 agent 별칭과 임의 접미사 Jenkins agent Pod 이름은 남겼다. 계정·내부 URL/IP·token·cookie·OTP·Secret·고객 데이터 원문 노출 0건이고 생성 UI·목업·장식용 화면 0건이다. 각 crop의 출처·캡처 시점·판정 문구를 `SOURCES.md`와 `CAPTURE-CHECKLIST.md`에 기록했으며, `17a`가 5.24:1이라 17장 배치 조정이 필요하다는 점과 `17b-shuffle`이 44 DPI로 낮아 재캡처 여지가 있다는 점을 후속에 남겼다. |
| `PRESENT-DECK-01 DONE` | 승인된 22장 와이어프레임에 생성 이미지·공식 아이콘·실제 UI 증거·검증된 기술 문구를 채워 최종 발표 PPTX를 완성 (`docs/presentation/ktcloud4-bean.pptx`, `docs/presentation/README.md`) | `PRESENT-VISUAL-01`, `PRESENT-EVIDENCE-01`, `DEMO-AWS-HR-01`, `DEMO-RECOVERY-01` | 없음 | `PRESENT-SCRIPT-01` | 와이어프레임의 layout 8종·색·모티프·22장 순서·845초 배분을 그대로 승계하고 `PURPOSE`/`MESSAGE`/`ASSET`/`EVIDENCE` 자리를 최종 내용으로 대체했으며 계약 원문은 22장 speaker notes에 남겼다. Draw.io는 넣지 않고 승인 이미지 한 장을 8~11장에 `914400, 2651760, 22555505, 9692640` EMU로 네 장 동일 배치했다. 이미지가 1.777:1이고 자리가 2.327:1이라 `cover` crop `srcRect t=b=11821`로 원본 y 213~1589만 보이게 해 `PRESENT-VISUAL-01`의 세이프존 216~1584를 그대로 담았고, 강조는 원본 픽셀을 같은 변환으로 옮긴 green 테두리만 써서 그림 대비를 낮추지 않았다. 1·21장 배경에 같은 이미지를 어둡게 깔았다. 실제 UI 6장을 모두 배치했고 `16b-jenkins-fail`은 16장이 아니라 13장에 두어 gate가 끊는 장면을 공급망 흐름 안에서 보였으며, 5.24:1인 `17a-kyverno`는 상단 전체 폭 22.0in letterbox로 두어 터미널 글자를 약 14pt 상당으로 키웠다. 팀 5인은 사용자 원본 표의 역할·성과를 옮겨 한 장에 두고 간트·SWOT·공급망·탐지 흐름은 native 도형으로 그렸다. Office validator `All validations PASSED`, `markitdown` 22장 확정 순서 일치와 잔여 placeholder 0건, PowerPoint timing·transition 각 0건, speaker notes 22/22, 시간 합계 845초, LibreOffice/PDF 22장 육안 검토에서 잘림·겹침·저대비·깨진 아이콘 0건을 확인했다. 다중 리전·Multi-AZ·RDS HA·자동 대응·실제 랜섬웨어·전체 DR 성공 주장 0건이고, IP·CIDR·ARN·account id·도메인·token 스캔 0건이다. pptxgenjs가 `altText` 없는 이미지의 `descr`에 원본 절대 경로를 넣는 것을 발견해 7개 이미지 전부에 alt text를 지정했고 패키지 안에 `/home/` 경로 0건을 확인했다. 사용자가 초안을 보고 지적한 AI 문체와 가독성을 같은 브랜치에서 보정했다. 캔버스 26.667×15in가 표준 와이드의 정확히 2배인데 와이어프레임이 본문 17pt·제목 40pt를 써서 표준 환산 8.5pt였음을 원본 템플릿·기존 발표자료 실측(본문 30.7pt·제목 48pt)으로 특정하고 본문 24~28pt·제목 48pt·대제목 84pt로 올렸다. 문구는 문서 문장을 슬라이드에 옮긴 것이 원인이라 보고 카드당 두세 줄로 줄였으며, 가운뎃점 나열을 `와/과`로 풀고 `판정한다`·`0건`·`~뿐이다` 같은 백로그 용어를 걷어냈다. 하단 결론 띠를 12장에서 3·13·14·15장 네 장으로 줄이고 나머지는 상단 kicker 한 줄이 대신하게 해 같은 리듬 반복을 없앴다. 슬라이드 본문 백틱 0건(speaker notes 1건은 발표자 메모라 유지)이고, 재렌더에서 6장 하단 노트·13장 캡션·17장 제목 간격·20장 stat 카드 하단 걸침을 고쳐 22장 모두 잘림·겹침 0건이다. |
| `PRESENT-SCRIPT-01 DONE` | 최종 PPTX의 페이지별 발표 대본을 작성하고 같은 내용을 speaker notes에 반영해 한 명 발표 기준 약 15분 리허설을 완료 (`docs/presentation/presentation-script.md`, `docs/presentation/ktcloud4-bean.pptx`) | `PRESENT-DECK-01` | 없음 | 발표 | 착수 시점의 `ktcloud4-bean.pptx`가 22장이 아니라 23장이었고 내용도 팀이 직접 바꾼 상태였다. 슬라이드를 계약에 맞추지 않고 대본을 실제 파일 23장 기준으로 작성했으며, 어긋난 채 남아 있던 옛 `PURPOSE`/`MESSAGE`/`ASSET`/`EVIDENCE` notes 22장분을 전부 걷어냈다. 22장 구성 계약 문단은 stale이므로 별도 보정이 필요하다. 낭독은 글자 수를 분당 340자로 나눠 산정했고 4,785자·845초(14분 5초)로 계약 13분 30초~14분 15초 안이며, 320자/분으로 느리게 읽어도 14분 57초라 전환·호흡을 포함한 1회 리허설이 15분을 넘지 않는다. 초안이 6,124자(17분 17초)여서 세 번에 걸쳐 문장을 걷어냈고 장별 목표 시간은 최종 글자 수에서 다시 계산해 편차 0건이다. 슬라이드에 없는 수치·주장은 넣지 않았고 Security Hub 92%, Wazuh 24시간 218,300건, 경보 19개 중 9개, 호스트 7대, `active_response` 0건, Jenkins 42번 gate 중단은 모두 화면에 있는 값이다. 문체는 `-습니다/-했습니다`를 기본으로 하고 `-고요/-는데요/-보시면요`만 섞었으며 기계적 삼단 나열과 `단순히 A가 아니라 B`는 쓰지 않았다. Office validator `All validations PASSED`, speaker notes 23/23, 옛 계약 문구 잔존 0건, 적용 전후 슬라이드 도형·이미지 덤프 차이 0건, 노트 페이지 포함 46쪽 PDF 재렌더에서 슬라이드 번호와 대본이 일치함을 확인했다. 7~11장이 같은 그림이라는 점, 11장의 사람 승인과 18장의 CIEM 자동 차단이 다른 층위라는 점, 19장 캡처에 AWS 계정 번호가 보인다는 점을 발표 중 주의로 남겼다. |
...
2026-08-16 `PRESENT-PLAN-01-FIX-01`에서 기존 계획보다 뒤에 확정된 `DEMO-PLAN-02`의 HR System/EKS
세션 4를 발표 계약에 반영했다. 발표 전체를 HR 애플리케이션 소개로 바꾸지 않고, 제목·22장·약
14분 5초·온프레미스 중심 구조를 유지하면서 7·10·13·16~18장에 AWS 대표 워크로드의 역할을
명시했다. 최종 성공 문구와 실제 UI 증거는 `DEMO-AWS-HR-01` 완료 뒤에만 사용하도록 후속 선행을
보정했으며 라이브·Notion·Secret·사용자 원본은 변경하지 않았다. 직접 후속인 `PRESENT-ARCH-01`만
`READY`로 유지한다.

2026-08-16 `PRESENT-ARCH-01`에서 Git 선언과 백로그 `DONE` 증거를 대조한 source matrix를 만들고,
한 페이지 16:9 논리 아키텍처를 `docs/presentation/architecture/`에 작성했다. 그림은 발표 생성
이미지와 슬라이드의 구조 검수 기준이며 PPT에는 넣지 않는다. `architecture.md`의 `Board Demo` 행이
`BOARD-DEMO-02`·`AWS-HR-02` 이후 남은 stale 선언임을 확인해 그림에서 제외하고 `README.md`에
기록했다. 이 행 삭제는 `PRESENT-ARCH-01`의 소유 범위 밖이므로 별도 보정 작업이 필요하다.
라이브·Secret·사용자 원본은 변경하지 않았고 직접 후속인 `PRESENT-DECK-SKELETON-01`만 `READY`로 연다.

2026-08-16 `BOARD-DEMO-02-FIX-01`에서 `PRESENT-ARCH-01`의 Git 선언 대조로 드러난 stale 문서 행을
지웠다. `docs/architecture.md` 서비스 배치표의 `Board Demo` 행은 `BOARD-DEMO-02`가 라이브와 선언을
모두 제거한 뒤에도 남아 있었다. 문서 한 행만 바꿨고 GitOps 선언과 라이브는 건드리지 않았다.
`docs/evidence/obs-11/README.md`에 기록된 Unbound `board` alias 라이브 잔존 여부는 이 작업의
범위 밖이며 확인하지 않았다.

2026-08-16 `AWS-HR-01-FIX-07`에서 `BOARD-DEMO-02-FIX-01`과 같은 종류로 남아 있던 stale 표기를
지웠다. `docs/architecture.md`의 HR System 행이 배포 완료 뒤에도 `DECLARED`로 적혀 있었다.
같은 점검에서 `docs/evidence/obs-11/README.md`가 남아 있다고 기록한 Unbound `board` alias는
현재 `infra/opnsense/config.xml` 스냅샷에 0건이므로 별도 작업을 만들지 않았다.

2026-08-16 `PRESENT-DECK-SKELETON-01`에서 22장 와이어프레임 PPTX를 만들었다. 사용자가 제공한 15장
원본 템플릿은 참고 입력으로만 읽고 Git에 복제하지 않았으며, 해커톤 주제 사진 자산 대신 검정·흰색·
neon green 팔레트와 헤더 바·각진 사각형·화살표 모티프만 layout 8종으로 옮겼다. 각 장에는 실제 제목과
시간, `PURPOSE`·`MESSAGE`·`ASSET`·`EVIDENCE` 계약만 넣고 최종 문구·생성 이미지·실제 UI는 비워 두었다.
템플릿과 같은 Pretendard를 지정했으나 렌더 환경에 그 폰트가 없어 QA는 Noto Sans CJK KR 대체로
확인했으므로, 최종 배포 전에 Pretendard가 설치된 PowerPoint에서 한 번 더 본다. 라이브·Secret·사용자
원본은 변경하지 않았고 직접 후속인 `PRESENT-VISUAL-01`과 `PRESENT-EVIDENCE-01`을 `READY`로 연다.

2026-08-16 `PRESENT-VISUAL-01`에서 생성 모델 prompt 방식을 폐기했다. chatgpt.com이 prompt v1을
이미지 생성이 아니라 자료 검색으로 처리해 Kubernetes 공식 로고를 반환했고, 생성 지시를 명시한 v2로도
통제 가능성이 낮다고 판단했다. 이미 `PRESENT-ARCH-01`에서 확보한 `platform-architecture.drawio`와
vendor 공식 아이콘 35종으로 발표용 한 장을 직접 제작하는 방식으로 계약을 바꾼다. 사용자 지시에 따른
방식 변경이며 작업 ID·선행·영향과 발표 22장 계약은 그대로 둔다.

2026-08-16 `PRESENT-VISUAL-01`을 직접 제작으로 완료했다. 초안은 아이콘을 나열한 표처럼 보이고 화면
절반이 비어 사용자 지적을 받아, 네 그룹을 명시적 박스로 묶고 각 박스에 발표에서 실제로 할 문장을 넣어
다시 그렸다. 온프레미스와 AWS 폭을 재배분해 EKS 쪽 답답함을 없앴고, 진입 밴드 노드의 높이 부족으로
라벨이 박스 밖으로 넘치던 결함도 함께 고쳤다. 사용자가 최종본을 승인했으며 `PRESENT-DECK-01`은
`PRESENT-EVIDENCE-01`이 남아 있어 계속 `BLOCKED`다.

2026-08-16 `PRESENT-EVIDENCE-01`의 증거 캡처 중 Jenkins job 화면에 stage 시각화가 없다는 것을
확인해 `CI-01-FIX-02`를 열었다. `plugins.txt` 64개에 시각화 플러그인이 없어 `Pipeline Steps`의
평문 목록만 보인다. 같은 점검에서 `hr-system-image-build #42`의 실패는 결함이 아니라
`QUALITY-05`가 남긴 의도적 음성 fixture이고 image build·push·release handoff 0건으로 gate가
정상 동작한 기록임을 확인해 별도 작업을 만들지 않았다.

2026-08-16 `CI-01-FIX-02`에서 Jenkins 파이프라인 시각화 플러그인 `pipeline-graph-view`(998.vc30deece0e36)와 `pipeline-stage-view`(2.41) 및 전이 의존성 11종(`joda-time-api`, `pipeline-build-step`, `pipeline-graph-analysis`, `pipeline-groovy-lib`, `pipeline-input-step`, `pipeline-model-api`, `pipeline-model-definition`, `pipeline-model-extensions`, `pipeline-rest-api`, `pipeline-stage-tags-metadata`, `workflow-multibranch`)을 `plugins.txt`에 반영해 총 77개 플러그인을 exact 버전으로 고정했다. 전이 의존성 최소 요구 버전에 맞춰 `workflow-cps`(4362.v9b_991749898e) 등 8개 패키지의 버전을 update-center 2.568.1 baseline으로 정렬했다. `ARGO-ROOT` 잠금 아래 `platform-root`와 `jenkins` Application을 commit SHA로 배포해 controller 롤아웃(`1/1 Ready`), `jenkins-home` PVC 보존, 기존 6개 Job(`aws-opentofu-pipeline`, `awx04-ee-build`, `ci01-image-build`, `hr-system-e2e`, `hr-system-image-build`, `ops-drift-check`) 선언 불변을 확인했다. 라이브 API 검증에서 77개 플러그인 정상 로드, `hr-system-image-build`의 `/wfapi/runs` 및 `/41/wfapi/describe`(SUCCESS, 11개 stage), `/42/wfapi/describe`(FAILED at 'test and coverage gates') 응답과 job 대시보드 HTML 내 `<div class="cbwf-stage-view">` DOM 렌더링을 실측 확인했다. 검증 후 `jenkins` Application의 `targetRevision`과 `platform-root`를 `main`으로 복구하고 잠금을 해제했다.

2026-08-16 `PRESENT-EVIDENCE-01`에서 16~17장 증거 6장을 실제 UI로 확보했다. 캡처 범위와 마스킹
checklist를 먼저 확정하고 사용자가 라이브에서 찍은 화면을 마스킹 누락·가독성·비율 기준으로 검수해
통과분만 저장했다. 작업 중 `hr-system-image-build #42`의 실패가 결함이 아니라 `QUALITY-05`가 남긴
의도적 음성 fixture임을 확인해 별도 작업을 만들지 않았고, 오히려 `#41`과의 대비를 16b 증거로 채택했다.
Jenkins job 화면에 stage 시각화가 없는 문제는 `CI-01-FIX-02`로 따로 열었다. 라이브·Secret·사용자
원본은 변경하지 않았고 직접 후속인 `PRESENT-DECK-01`을 `READY`로 연다.

2026-08-17 `PRESENT-DECK-01`에서 22장 최종 발표 PPTX를 완성했다. 와이어프레임을 새로 그리지 않고
`PRESENT-DECK-SKELETON-01`의 layout 8종·색·모티프·22장 순서·845초 배분을 그대로 승계한 뒤
`PURPOSE`/`MESSAGE`/`ASSET`/`EVIDENCE` 자리에 최종 내용을 채웠고, 계약 원문은 22장 speaker notes에
남겨 `PRESENT-SCRIPT-01`까지 슬라이드 근거를 추적할 수 있게 했다. 8~11장은 승인 이미지가 1.777:1이고
자리가 2.327:1이라 그대로 들어가지 않아, 좌우를 채우고 위아래를 잘라내는 `cover` crop으로 원본
y 213~1589만 보이게 했다. 이 구간은 `PRESENT-VISUAL-01`이 미리 콘텐츠를 넣어 둔 세이프존
216~1584와 사실상 일치해 잘려나가는 것이 여백뿐임을 확인했다. 첫 렌더에서 반투명 green 채우기가
넓은 영역을 덮어 아이콘과 라벨 대비를 떨어뜨려 테두리만 남겼고, 이미지 아래 여백이 0.64in뿐이라
캡션 띠와 footer가 겹치는 것을 캡션 띠 하나가 쪽 번호까지 담는 방식으로 고쳤다. 슬라이드 본문에
쓴 markdown 백틱이 PowerPoint에서 문자 그대로 렌더되는 것도 첫 렌더에서 발견해 모두 제거했다.
`PRESENT-EVIDENCE-01`이 남긴 두 배치 숙제는 각각 `17a-kyverno`를 상단 전체 폭 22.0in letterbox로
두어 터미널 글자를 약 14pt 상당으로 키우는 것과, `16b-jenkins-fail`을 16장이 아니라 13장으로 옮겨
gate가 끊는 장면을 공급망 흐름 안에서 보이는 것으로 정리했다. pptxgenjs가 `altText` 없는 이미지의
`descr`에 생성 환경의 절대 경로를 그대로 넣는 것을 발견해 이미지 7개 전부에 alt text를 지정하고
패키지 안 `/home/` 경로 0건을 확인했다. 사용자 원본 로드맵의 `4주차(~8월 14일)`는 발표 시점인
2026-08-17까지 실제 작업이 이어져 간트에서 `~8월 17일`로 적었고 그 근거를 발표 산출물 README에
남겼다. 라이브·Secret·사용자 원본은 변경하지 않았고 직접 후속인 `PRESENT-SCRIPT-01`을 `READY`로 연다.

2026-08-17 `PRESENT-DECK-01` 초안을 사용자가 열어보고 문체가 AI 같고 가독성이 떨어진다고 지적해
같은 브랜치에서 보정했다. merge 전이므로 FIX 작업 ID를 만들지 않았다. 가독성 원인은 폰트 선택이
아니라 캔버스 환산이었다. 이 덱의 26.667×15in는 PowerPoint 표준 와이드의 정확히 2배라 같은 pt가
절반 크기로 보이는데, 와이어프레임이 본문 17pt·제목 40pt를 그대로 써서 표준 환산 8.5pt였다.
사용자 원본 템플릿과 기존 22장 발표자료를 실측해 둘 다 같은 캔버스에서 본문 30.7pt·제목 48pt·
섹션 대제목 80pt를 쓴다는 것을 확인하고 그 스케일을 기준선으로 삼았다. 문체 쪽은 백로그 문장을
슬라이드에 그대로 옮긴 것이 원인이라, 카드당 두세 줄로 줄이고 가운뎃점 나열을 `와/과`로 풀었으며
`판정한다`·`0건`·`~뿐이다` 같은 문서 용어를 슬라이드에서 걷어냈다. 하단 결론 띠가 12장에 반복돼
눈에 안 들어오던 것도 네 장으로 줄이고 나머지는 상단 kicker가 대신하게 했다. 검증 항목은 전부
다시 통과했고 8~11장 이미지 좌표·crop, 시간 합계 845초, 22장 순서는 그대로다.

2026-08-17 `PRESENT-SCRIPT-01`에서 발표 대본을 작성하고 speaker notes 23장을 교체했다. 착수하며
연 `ktcloud4-bean.pptx`가 `PRESENT-DECK-01`이 남긴 22장이 아니라 23장이었고, 팀이 온프레미스 보안
도구 두 장과 AWS의 IRSA·CIEM·CSPM 세 장을 넣고 시나리오·백업/복구·결론·Q&A 장을 뺀 상태였다.
슬라이드를 계약 쪽으로 되돌리지 않고 대본을 실제 파일에 맞췄다. 발표는 이 파일로 하고, 대본이
파일과 어긋나면 대본이 쓸모없어지기 때문이다. 대신 옛 notes가 슬라이드와 어긋난 채 남아 있어
19장 CSPM 화면에 `관측·탐지·사람 승인` 계약이 붙어 있는 식이었고, 이 22장분을 전부 걷어냈다.
`13. 발표 아키텍처·슬라이드·대본`의 22장 구성 계약 문단은 이제 stale이다. 이 문단을 다시 쓰는 것은
대본 작업의 소유 범위 밖이므로 별도 보정 작업이 필요하다.

낭독 시간은 추정하지 않고 글자 수로 계산했다. 초안은 6,124자로 분당 340자 기준 17분 17초였고,
세 번에 걸쳐 문장을 걷어내 4,785자·845초(14분 5초)로 맞췄다. 장별 목표 시간은 최종 글자 수에서
다시 계산해 대본과 편차 0건이며, 320자/분으로 느리게 읽어도 14분 57초라 전환·호흡을 포함한
1회 리허설이 15분을 넘지 않는다. 대본에는 슬라이드에 있는 값만 넣었고, 15장 Jenkins 41번이
11단계를 다 넘겼다는 사실처럼 화면 밖에 있는 근거는 본문에서 빼고 질문이 올 때 답하도록 남겼다.

발표 중 주의로 세 가지를 남겼다. 7~11장이 강조 위치만 다른 같은 그림이라는 점, 11장 그림의
`조치는 사람 승인 뒤에만 실행된다`와 18장의 CIEM 자동 차단이 서로 다른 층위라는 점, 19장 CSPM
캡처에 AWS 계정 번호가 마스킹 없이 보인다는 점이다. 세 번째는 심사 자리에서는 문제가 없지만
외부 공개용으로 다시 쓸 때 가려야 하므로 발표 자료 README에도 적었다. 라이브·Secret·사용자 원본은
변경하지 않았다. 이 작업의 직접 후속은 발표이므로 여는 작업 ID는 없다.
