...
| `WAZUH-04-FIX-01 READY` | `wazuh-04-relay-agent`의 `client.keys`가 pod 파일시스템에만 있어 재시작마다 사라지고, `wazuh-agentd`의 자동 재등록이 같은 이름의 기존(비활성) 등록과 충돌해 영구 재시도에 빠지는 결함을 정적 키 렌더링으로 근본 수정 (`gitops/apps/wazuh/`) | `WAZUH-04` | `ARGO-ROOT` | `WAZUH-04` 6개 소스 수집 가용성 | 매니저에 이미 살아 있는 `wazuh-04-relay` agent의 `client.keys` 한 줄을 재사용해(신규 enrollment 유발 금지) `kv/wazuh/manager`에 새 필드로 추가하고(기존 필드는 `vault kv patch`로 보존), `vault-agent-relay-agent.hcl`이 이를 `/var/ossec/etc/client.keys`로 렌더링하도록 바꾼다. `ossec.conf`의 `<enrollment>`는 `WAZUH-03` host agent와 동일하게 `<enabled>no</enabled>`로 고정해 `wazuh-agentd`가 아예 재등록을 시도하지 않게 한다. `ARGO-ROOT` 잠금 아래 실제 rollout restart로 재현하고, 재시작 전후 같은 agent ID 유지·`Active` 상태·매니저 로그의 `Duplicate agent name` 오류 0건을 확인한다. rollback은 `client.keys` 템플릿·enrollment 설정·Vault 필드를 WAZUH-04 병합 시점으로 원복 |
