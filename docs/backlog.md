...
| `WAZUH-04 READY` | Keycloak·Pomerium·Vault·CrowdSec AppSec·Falco·Warpgate 6개 소스를 Wazuh D30/A90 직접 수집에 온보딩 (`gitops/apps/wazuh/`, `infra/ansible/`) | `WAZUH-01`, `KC-01`, `POM-01`, `VAULT-02`, `CROWDSEC-FIX-01`, `FALCO-01`, `WG-01` | `ARGO-ROOT` | 계정·접근·WAF·runtime 탐지 중앙화 | `docs/audit-event-standard.md` 소스별 routing 표대로 Keycloak·Pomerium·Vault·Warpgate(A90), CrowdSec AppSec·Falco(D30) 6개를 온보딩한다. k3s Pod 4종(Keycloak·Pomerium·Vault·CrowdSec AppSec·Falco 중 in-cluster 소스)은 `WAZUH-01`의 K8s audit과 같은 hostPath `<localfile>` tail 방식으로 manager에 태우고, Warpgate는 `WAZUH-03`이 이미 설치한 host agent의 기존 manager 연결을 재사용해 감사 로그 파일 하나만 `<localfile>`로 추가한다(신규 방화벽 규칙 0건). 6개 소스의 전용 decoder·rule은 기존 예약 범위(A90 `100102`~`100109` 8개, D30 `100121`~`100129` 9개) 안에서만 배정하고, 소스당 1개를 기본으로 하되 `WAZUH-05`(NetBird, 같은 A90 범위 사용 예정)를 위해 A90 최소 1개 이상을 비워 둔다. 각 소스는 감사 표준의 마스킹 규칙(token·cookie·password·admin representation body 등 제거)을 소스 단계에서 적용하고, 대표 event 1건씩(로그인 실패, allow/deny, audit request, WAF match, runtime rule match, target 인가)을 D30/A90 index에서 검색해 확인한다. Loki에는 이 6개 소스의 보안 event 복제본이 0건이어야 하고, 배포 후 실측 저장 증가량이 D30/A90 16 GiB hard cap 안인지 재판정한다(미달이면 소스별 필드를 더 좁힌다). rollback은 6개 localfile·decoder·rule 제거와 Warpgate host agent를 `WAZUH-03` 종료 상태로 복원하고 최신 main의 `platform-root`·`wazuh` `Synced/Healthy`, 신규 방화벽 규칙이 없으므로 OPNsense drift 확인은 대상 아님 |
| `WAZUH-05 READY` | NetBird `events.db` 감사 이벤트를 Wazuh A90 직접 수집에 온보딩 (`gitops/apps/wazuh/`, `infra/ansible/`) | `WAZUH-01`, `NB-01` | `ARGO-ROOT` | VPN peer·정책 변경 탐지 | NetBird 감사 기록은 로그 파일이 아니라 SQLite `events.db`라 `WAZUH-04`의 hostPath tail 패턴을 그대로 쓸 수 없다. `netbird-01`에 이미 연결된 `WAZUH-03` host agent를 재사용하되(신규 방화벽 규칙 0건), `events.db`의 새 row만 마지막 처리 row ID 기준으로 주기 polling해 JSON 한 줄씩 append하는 최소 systemd timer 스크립트를 추가하고 그 출력 파일을 agent `<localfile>`로 물린다. account·user·peer·policy·setup-key 변경과 접근 event만 다루고 setup key/token 값·geo/city·이메일은 소스 단계에서 제거한다. `WAZUH-04` 적용 뒤 A90 예약 범위(`100102`~`100109`)의 실제 잔여 ID를 먼저 확인하고 그 안에서만 전용 decoder·rule을 배정하며, 부족하면 배정을 중단하고 범위 재검토를 먼저 보고한다. 대표 event(예: peer 등록 또는 정책 변경) 1건을 A90 index에서 검색해 확인하고, 재시작 후에도 중복 없이 이어서 읽는지 확인한다. Loki 복제본 0건, 배포 후 D30/A90 16 GiB hard cap 재판정. rollback은 polling 스크립트·타이머·localfile·decoder·rule 제거와 host agent를 `WAZUH-03` 종료 상태로 복원하고 최신 main의 `platform-root`·`wazuh` `Synced/Healthy` |
| `LOKI-02 READY` | 6개 Linux 대상 host journald/syslog를 Loki로 수집 (`infra/ansible/`, `gitops/apps/obs/`) | `LOKI-01`, `OBS-11` | `ARGO-ROOT` | 호스트 장애 조사 | `audit-event-standard.md`에 host 운영 로그(O7 확장) 분류를 추가해 대상을 `sshd`·`sudo`·systemd unit 실패·kernel(dmesg)로 한정하고, 인증 성공/실패 시도가 `WAZUH-03`의 보안 이벤트와 중복되지 않도록 소스별로 정확히 한쪽에만 보낸다. `k3s-01`·`postgres-01`·`object-01`·`warpgate-01`·`netbird-01`·`proxmox-01`에 checksum 고정 로그 수집 agent를 설치해 journald만 읽고(파일시스템 전체 tail 금지) hostname·unit label만 쓰며 PID·세션 단위 label은 만들지 않는다. Loki 14 GiB hard cap 안에서 실측 저장 증가량을 계산하고 초과 시 allowlist를 줄인다. 기존 K8s O7과 새 host O7이 `OBS-07` Log Explorer에서 host selector로 구분 조회됨을 확인한다. rollback은 6개 host agent 제거와 Loki 설정 원복이며 최신 main의 `platform-root`·`obs` `Synced/Healthy` |
| `OBS-14 READY` | PostgreSQL·SeaweedFS·Warpgate·NetBird 등 앱 native metric 조사·수집 (`gitops/apps/obs/`) | `OBS-11`, `PG-01`, `S3-01`, `WG-02`, `NB-02` | `ARGO-ROOT` | 서비스 내부 상태 가시성 | 각 제품이 이미 Prometheus 형식 `/metrics`를 노출하는지 read-only로 먼저 확인한다(PostgreSQL은 기본 노출이 없어 별도 `postgres_exporter` 설치가 필요할 수 있고, SeaweedFS는 `-metricsPort` 지원 가능성이 있으며, Warpgate·NetBird는 미확인이다). 실제 노출을 확인한 제품만 checksum 고정 scrape를 추가하고, 노출하지 않는 제품은 이번 작업에서 새 exporter를 만들지 않고 조사 결과와 후속 작업 필요 여부만 기록한다. 추가하는 대상마다 `up=1`과 대표 시계열 최소 1개(예: `pg_stat_database_xact_commit`, SeaweedFS volume 사용량)를 확인하고, 방화벽·NetworkPolicy는 `OBS-11` 패턴대로 Prometheus source에만 정확히 한정한다. rollback은 신규 ScrapeConfig·NetworkPolicy·(설치했다면) exporter unit만 원복하고 최신 main의 `platform-root`·`obs` `Synced/Healthy` |
