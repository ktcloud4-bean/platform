...
| `POM-01-FIX-06 READY` | Dashy 포털을 Minimal 뷰로 고정(`startingView`)하고 텍스트 색을 흰색으로 통일(`customCss`) | `POM-01-FIX-05` | 없음 | 포털 UX | `appConfig.startingView: minimal`로 `/`가 바로 minimal 레이아웃, `customCss`로 body·heading·tile 제목/설명/URL·input만 `#fff`로 강제(아이콘 `color`·`fill: currentColor`는 건드리지 않도록 아이콘 클래스 제외), YAML 유효성 확인, 라이브 확인은 사용자가 진행 |
