...
| `OBS-05 READY` | Core Services & Traffic Infrastructure 대시보드 구축 (`gitops/apps/obs/`) | `OBS-04` | 없음 | 관측·운영 | `obs-05-core-services` ConfigMap 선언 및 Grafana 마운트, Traefik(QPS/Status/Latency)·Pomerium(SSO/Route)·PostgreSQL(쿼리/커넥션/WAL)·Vault(Unseal/Cert-TTL) 패널 표시, Argo child `obs` `Synced/Healthy` |
