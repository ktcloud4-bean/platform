...
| `QUALITY-03 READY` | 미사용으로 판정된 SonarQube와 그 파생 경로를 보존·승인 후 정확히 폐기하고 `E2E-01` 합성 Sonar gate 의존성을 먼저 제거·대체 (`gitops/apps/sonarqube/`, `gitops/apps/jenkins/`, `gitops/apps/e2e-01/`, `docs/evidence/quality-02/`) | `QUALITY-02`, `E2E-01`, `BKP-02`, `BKP-03`, `BKP-04` | `ARGO-ROOT`, `VAULT-CONFIG` | 30 GiB PVC·SonarQube DB와 불필요 credential 회수 | PostgreSQL DB native backup·SonarQube PVC filesystem backup 및 BKP-04 오프사이트 사본을 먼저 확보하고 격리 restore를 확인. E2E-01 gate의 보존·대체 결정을 기록한 뒤에만 SonarQube Application/namespace/PVC, DB·role, Vault path·role·policy, Pomerium route/egress, Jenkins 파생 credential을 exact 목록으로 삭제하고 관련 child `Synced/Healthy`를 확인한다. |
...
| C — 사용성 판정 | `QUALITY-02` → `QUALITY-03` | `QUALITY-02` 완료 후 폐기 승인 | 문서·read-only 조사는 lane A/B와 병렬 가능; PVC·DB 삭제는 `QUALITY-03`에서 backup·승인 뒤 수행 |
