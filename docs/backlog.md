...
| `POM-01-FIX-04 READY` | simple-icons 유무와 무관하게 모든 타일을 각 서비스 자신이 제공하는 실제 favicon(`favicon-local`)으로 통일 | `POM-01-FIX-03` | 없음 | 포털 UX | 12개 타일 전부 `icon: favicon-local`로 교체하고 `si-`·`color`·범용 Font Awesome 값 제거, YAML 유효성 확인; 브라우저가 각 내부 hostname에서 직접 favicon을 받아오는 방식이라 Pomerium 세션·그룹 게이팅과 실제 렌더링 결과는 라이브에서 확인 필요 |
