...
| `QUALITY-06 READY` | 검증 digest 배포 뒤 전용 합성 identity·fixture로 HR System의 최소 read-only Playwright E2E를 별도 실행하고 Jenkins test 이력에 합류 | `QUALITY-05`, `AWS-HR-03-FIX-01` | `ARGO-ROOT`, `IDENTITY-LIVE`, `VAULT-CONFIG` | 배포 후 사용자 경로 검증 | 합성 employee·HR identity와 합성 데이터만 존재함을 먼저 확인하고 Chromium 1 worker·retry 0으로 employee 본인 profile/history, employee의 HR route/API 거부, HR 화면 접근과 HR 사용자 목록 제외 세 흐름만 검증한다. JUnit은 Jenkins에 게시하고 합성 화면의 실패 screenshot만 접근 제한·단기 보존하며 인증 cookie·network payload를 담을 수 있는 trace·video·상시 screenshot은 만들지 않는다. coverage 합산, 실제 직원 생성·수정, Aurora 감사이력 추가와 product image 재build는 0건이다. 합성 전용 데이터 조건이 깨지면 실행하지 않고, write E2E는 폐기 가능한 환경 확보 전까지 열지 않는다. |
