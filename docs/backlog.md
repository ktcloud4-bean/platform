...
| `PRESENT-EVIDENCE-01 READY` | 최신 실제 상태의 HR 포털, Argo CD `hr-system`, Jenkins/SonarQube, EKS Kyverno, Grafana, Wazuh/Shuffle 화면을 발표 안전영역에 맞춰 캡처하고 민감정보를 마스킹한 증거 자산과 출처·판정 설명을 준비 (`docs/presentation/assets/evidence/`, `docs/presentation/assets/SOURCES.md`) | `PRESENT-DECK-SKELETON-01`, `DEMO-AWS-HR-01` | 없음 | `PRESENT-DECK-01` | 화면별 exact 캡처 범위와 마스킹 checklist를 먼저 확정하고, HR 포털 정상 상태, Argo `hr-system` `Synced/Healthy`, Jenkins test·Sonar quality gate, Kyverno dry-run `DENIED`/`ALLOWED`, Grafana 운영 상태, Wazuh 탐지와 Shuffle 사람 승인 근거를 실제 UI에서 확보한다. 모든 화면을 한 장에 몰지 않고 16~17장에서 주장별로 읽히는 최소 crop만 선택한다. 계정·email·내부 URL/IP·token·cookie·OTP·hook·Secret·고객 데이터 원문은 노출하지 않으며 생성 UI·장식용 대체 화면 0건이다. 각 crop이 placeholder에서 읽히고 출처와 캡처 시점·판정 문구가 일치함을 확인한다. |
