...
| `SUPPLY-01 BLOCKED` | EKS에 Kyverno와 이미지 admission 정책 신설 (`gitops/root/`, `gitops/apps/kyverno/`) | `SUPPLY-DESIGN-01` | `ARGO-ROOT` | `SUPPLY-06` | EKS destination Kyverno Application `Synced/Healthy`와 노드 available 자원이 정지 기준 밖, Audit 모드에서 기존 워크로드 위반 0건을 확인한 뒤 Enforce 전환, 자기 ECR과 AWS 관리형 애드온 레지스트리 밖 이미지의 Pod 생성 거부와 digest 미고정 거부를 각각 실증, hr-system 세 Deployment·migration Job·ALB controller·관리형 애드온 정상 기동 유지, Audit 복귀 rollback 뒤 기존 Pod 회귀 없음 |
| `SUPPLY-02 BLOCKED` | k3s Kyverno 이미지 정책 범위를 `e2e-01` 밖으로 확대하고 Audit으로 현황 확보 (`gitops/apps/kyverno/`, `policies/`) | `SUPPLY-DESIGN-01` | `ARGO-ROOT` | `SUPPLY-03` | 전 네임스페이스 Audit 정책 `Synced/Healthy`와 기존 `e2e-01` Enforce 정책 불변, 레지스트리별·네임스페이스별 위반 목록과 digest 미고정 목록을 기준선(이미지 참조 160건·digest 고정 119건·Harbor 경유 1건)과 대조해 확보, Enforce 전환 0건과 기존 워크로드 거부 0건 |
