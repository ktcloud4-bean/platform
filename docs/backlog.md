...
| `HEADLAMP-02 BLOCKED` | Headlamp Keycloak OIDC·Kubernetes RBAC·Pomerium Route; 이전 main 적용은 verifier SSH 인수 분리 오류로 rollback됨 | `HEADLAMP-01`, `POM-01` | `K3S-BOOTSTRAP` | k3s 일상 관리·`CAP-02` | 같은 ID로 재시도하지 않으며 `HEADLAMP-02-FIX-01`의 전체 재검증 성공 후에만 완료 상태를 다시 판정 |
| `HEADLAMP-02-FIX-01 READY` | rollback 기준선에서 Headlamp OIDC/RBAC 선언을 재도입하고, Argo 상태 JSON 검증의 SSH 인수 경계를 보정한 뒤 전체 라이브 증거를 다시 확보 | `HEADLAMP-01`, `POM-01` | `K3S-BOOTSTRAP` | k3s 일상 관리·`CAP-02` | HEADLAMP-02의 모든 완료 증거와 rollback·break-glass를 새 세션에서 재확인; 공유 cluster-admin SA/token 없음 |
