...
| `OBS-13 READY` | Prometheus 상시 alert rule과 실제 알림 채널 (`gitops/apps/obs/`) | `OBS-01`, `OBS-06`, `OBS-11` | `ARGO-ROOT` | 운영 장애 조기 탐지 | host down(`up{job="node-exporter"}==0` 5분 지속)·root/PVC 사용률 85%(경고)·95%(심각) 2단계·TLS 인증서 만료 14일 미만(`probe_ssl_earliest_cert_expiry` 기반)·Velero backup 실패(`velero_backup_failure_total` 증가) 최소 4종 PrometheusRule을 선언한다. Alertmanager receiver는 임시 verification pod가 아니라 실제 상시 채널(기존 Wazuh manager나 Shuffle 중 하나로 webhook forward)로 고정하고, silence 변경은 기존 `/platform-privileged` 경계를 유지한다. 각 rule은 인위로 만든 실제 조건(예: 대상 node-exporter systemd 정지, PVC 사용량 임계 초과)에서 firing→Alertmanager active→실제 채널 수신까지 한 번씩 실증하고, 배포 직후 정상 상태에서는 0건 firing임을 확인한다. `for` 기간·threshold 선정 근거를 문서화하고, 배포 전후 `prometheus_tsdb_head_series`·PVC·available RAM을 기존 정지 기준 안에 둔다. rollback은 PrometheusRule·AlertmanagerConfig·webhook egress만 원복하고 최신 main의 `platform-root`·`obs` `Synced/Healthy` |
| `WAZUH-03 READY` | 6개 Linux 대상 Wazuh agent 설치와 OPNsense 기존 agent HIDS 확장 (`infra/ansible/`, `gitops/apps/wazuh/`) | `WAZUH-02`, `OBS-11` | `OPNSENSE-LIVE` | 호스트 침해 탐지, `SOAR-01` 신호 품질 | OPNsense 기존 `os-wazuh-agent`(`suricata_eve_log`만 켜진 상태)의 `syscheck`(FIM)·`rootcheck`를 활성화하되 감시 경로는 설정·인증서·방화벽 관련 디렉터리로 최소화하고 전체 파일시스템 감시나 `active_response`·`remote_commands` 활성화는 하지 않는다. `k3s-01`·`postgres-01`·`object-01`·`warpgate-01`·`netbird-01`·`proxmox-01` 여섯 대상에 checksum 고정 Wazuh agent를 설치해 기존 manager(`10.10.20.10:31514/31515`)에 등록하고, 켜는 모듈은 `syscheck`(핵심 설정·바이너리 경로만)·`rootcheck`뿐이며 agent auth key는 저장소 밖에서만 생성·주입한다. syscheck 최초 baseline scan 완료와 의도적 테스트 파일 변경 1건의 FIM alert 검색, rootcheck 대표 policy check 결과 존재, 7개(OPNsense+6대) agent 모두 manager에서 `active`를 확인한다. Wazuh D30/A90 16 GiB hard cap 안에서 실측 저장 증가량을 재판정하고 미달이면 syscheck 대상 경로를 더 좁히며, Loki에는 FIM/rootcheck 이벤트 복제본을 넣지 않는다. rollback은 6개 agent 제거와 OPNsense agent의 `syscheck`·`rootcheck`를 `0`으로 원복하고 최신 main의 `platform-root`·`wazuh` `Synced/Healthy`, OPNsense drift 없음 |
