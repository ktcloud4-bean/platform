...
| `DEMO-ONPREM-01 READY` | 동일 가상 기업의 경량 합성 고객 포털과 결정론적 공격·통제·증거·reset 하네스를 추가해 온프레미스 데모 4편의 실제 촬영 인터페이스를 확정한다 (`gitops/apps/`, `gitops/tools/`, `docs/`) | `DEMO-PLAN-01`, `SUPPLY-05-FIX-04` | `ARGO-ROOT`, `IDENTITY-LIVE`, `VAULT-CONFIG` | `DEMO-RECOVERY-01` | 신규 PVC·공개 DNS·OPNsense·실데이터 없이 합성 고객정보와 통제된 실제 취약점만 사용한다. 계정 탈취는 미승인 장비의 내부 경로 실패와 승인 장비에서 동일 계정의 대조 경로 200·그룹 통제 403, SQLi는 동일 payload의 대조군 노출과 CrowdSec 403·Wazuh 증거, 컨테이너 침해는 합성 DB 횡적 이동의 NetworkPolicy 차단·Falco/Wazuh 탐지·Shuffle 사람 Continue/Stop 기록(자동 대응 0건), 공급망은 비승인 이미지 거부와 서명된 exact digest 실행을 각각 재현한다. 네 세션의 확정 화면 URL·입력·HTTP/연결 결과·rule/evidence marker·마스킹 경계를 문서화하고 한 명령 attack/control/evidence/reset을 제공하며, reset 뒤 transient 공격 자원 0건과 최신 main의 관련 Argo child `Synced/Healthy`를 확인한다. |
| `DEMO-RECOVERY-01 BLOCKED` | 데모 전용 합성 고객 데이터와 소형 `local-path` PVC, 결정론적 훼손·격리 복원 하네스를 추가해 세션 5의 GitOps 선언 복구와 Velero/Kopia 데이터 복구 대조를 촬영 가능한 한 흐름으로 만든다 (`gitops/apps/`, `gitops/tools/`, `docs/`) | `DEMO-ONPREM-01`, `BKP-02`, `BKP-10`, `POL-03` | `ARGO-ROOT` | `DEMO-OPENING-01` | 시작 전 현재 PVC 선언 합계와 `k3s-01` 파일시스템 여유를 재측정하고 96 GiB 경고·25% 여유 경계에 걸리면 적용하지 않으며, 1 GiB 이하 데모 전용 PVC와 합성 데이터만 만든다. Pod에 `backup.velero.io/backup-volumes`를 명시해 정상 marker·hash를 가진 Velero Backup `Completed`와 PVB 전송 바이트를 확보한다. 전용 fixture는 이 PVC 데이터만 훼손하고 데모 Deployment를 제거하며, Argo CD가 선언을 self-heal한 뒤에도 데이터 오류가 남는 대조를 재현한다. 알려진 정상 Backup을 격리 namespace에 한 번 복원해 Restore `Completed`, PVR, 원본과 같은 marker·hash, 서비스 정상 응답과 측정 RTO를 확인한다. 실제 랜섬웨어·기존 workload/PVC·AWS apply·AWS 화면·전체 DR 성공 주장은 0건이며, reset 뒤 이 작업이 만든 Backup/Restore/PVB/PVR·namespace·PVC·공격 자원 0건과 최신 main의 관련 Argo child `Synced/Healthy`를 확인한다. |
