...
| `BKP-12 READY` | `k3s-datastore-backup`(`BKP-01`)용 freshness health-check 신규 구축과 경보 연결 | `BKP-01`, `BKP-11` | `ARGO-ROOT` | k3s datastore 백업 RPO 가시성 | `k3s-datastore-backup`은 `BKP-03`(postgres·vault)이 공용 role로 이미 갖고 있는 freshness health-check 패턴이 없음을 확인(`systemctl list-timers`에 해당 항목 0건) — timer가 꺼지거나 k3s-01이 며칠 죽어도 감지할 장치가 아예 없다. `bkp03_backup_common`의 health-check script·timer·service 패턴을 참고해 `k3s-datastore-backup-health` 전용 unit 신규 작성(freshness 기준은 `BKP-11`과 동일 36시간 또는 하루 주기에 맞게 재계산), staleness 실제 감지 1회 실증, `K3sDatastoreBackupHealthStale` alert 추가와 `obs-13-receiver`·`obs-18-platform-slack` 등록, `ARGO-ROOT` 잠금 아래 검증 후 main 복귀 |
