...
| `QUALITY-06 READY` | 검증 digest 배포 뒤 전용 합성 identity·fixture로 HR System의 최소 read-only Playwright E2E를 별도 실행하고 Jenkins test 이력에 합류 | `QUALITY-05`, `AWS-HR-03-FIX-01`, `SUPPLY-01-FIX-01` | `ARGO-ROOT`, `IDENTITY-LIVE`, `VAULT-CONFIG` | 배포 후 사용자 경로 검증 | SUPPLY-01-FIX-01의 EKS TUF egress blocker 해소 후 실행 가능하다. 합성 employee·HR identity와 합성 데이터만으로 Chromium 1 worker·retry 0의 employee profile/history, employee HR route/API 거부, HR 화면과 사용자 목록 제외 세 흐름만 검증한다. |
