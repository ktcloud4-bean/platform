...
| `OBS-16-FIX-01 READY` | OBS-16의 native metric 두 경보를 기존 상시 receiver route에 포함하고, 수신까지 한 번만 안전 검증 (`gitops/apps/obs/`) | `OBS-16` | `ARGO-ROOT` | PostgreSQL·SeaweedFS·NetBird 장애 알림 | `NativeMetricsTargetDown`·`PostgreSQLDatabaseMetricsDown`만 기존 `obs-13-receiver` route에 추가한다. 실제 서비스·exporter·방화벽을 멈추지 않고 만료 시각이 있는 `test=true` Alertmanager test alert 한 건으로 **정확히 같은 alertname route**의 receiver firing 수신을 확인한 뒤 자동 만료·수신 종료를 확인한다. 기존 5개 route·기본 discard·경보 임계치와 외부 egress는 바꾸지 않는다. rollback은 두 alertname route만 제거하고 최신 main의 `platform-root`·`obs` `Synced/Healthy` |
| `OBS-17 BLOCKED` | Warpgate systemd·ACME timer와 private TLS 만료의 최소 경보 (`gitops/apps/obs/`, `infra/opnsense/`) | `OBS-16-FIX-01` | `OPNSENSE-LIVE`, `ARGO-ROOT` | 특권 접근 경로의 가용성·인증서 갱신 | 기존 node_exporter의 `node_systemd_unit_state`로 `warpgate.service`와 `warpgate-acme-renew.timer`가 5분 이상 active가 아닌 경우만 critical로 만든다. `Type=oneshot`인 `warpgate-acme-renew.service`의 inactive는 성공 후 정상 상태이므로 어떤 경보 조건에도 넣지 않는다. 기존 blackbox exporter의 private SNI probe로 `warpgate.imcherry5778.xyz:8888` 인증서 만료가 14일 미만일 때만 critical을 추가하고, 필요한 `k3s-01`→`warpgate-01` TCP 8888 한 경로와 `obs` NetworkPolicy만 최소 허용한다. public DNS/NAT·새 외부 노출·인증서 재발급·합성 장애는 금지한다. service·timer의 현재 active, one-shot inactive, TLS probe 성공·잔여 14일 초과, receiver route와 OPNsense drift를 확인한다. rollback은 rule·probe·정확한 egress/firewall rule만 원복하고 최신 main의 `platform-root`·`obs` `Synced/Healthy` |
| `OBS-18 BLOCKED` | Alertmanager의 팀 운영 Slack 통지 경로 (`gitops/apps/obs/`, Vault, `infra/opnsense/`) | `OBS-16-FIX-01` | `VAULT-CONFIG`, `OPNSENSE-LIVE`, `ARGO-ROOT` | 클러스터 밖 운영자 인지 | Alertmanager에서 `#platform-alerts`로 보낸다. critical은 firing·resolved 모두 `@channel`, warning은 firing·resolved를 남기되 멘션하지 않으며 info는 보내지 않는다. payload는 alertname·severity·instance·시각·Grafana link만 허용한다. 재발급된 Incoming Webhook 원문은 Git·ConfigMap·로그·명령 출력에 두지 않고 전용 `kv/obs/alertmanager`의 `slack_webhook_url` 한 field와 별도 Kubernetes auth policy·role로만 주입한다. Kubernetes NetworkPolicy는 Alertmanager Pod의 TCP 443만, OPNsense는 `hooks.slack.com` FQDN alias의 TCP 443만 허용한다. FQDN alias 검증이 불가능하면 포괄 egress로 완화하지 않고 중단한다. 승인된 일회성 `[TEST]` critical은 firing·resolved를 팀 채널에서 확인한 뒤 test rule·silence·임시 상태를 모두 제거한다. 기존 internal receiver와 보안 Slack secret·채널은 공유하지 않으며 최신 main의 `platform-root`·`obs` `Synced/Healthy`와 OPNsense drift를 확인한다. |
| `WAZUH-06 READY` | Wazuh high-severity 보안 사건의 분리된 Slack 통지 relay (`gitops/apps/wazuh/`, Vault, `infra/opnsense/`) | `WAZUH-05`, `SOAR-01` | `VAULT-CONFIG`, `OPNSENSE-LIVE`, `ARGO-ROOT` | 보안 사건의 팀 인지·조사 | Wazuh는 기존 Shuffle read-only·사람 승인 흐름을 보존하고, `level >= 14`만 별도 internal notifier relay로 보낸다. relay는 별도 `kv/wazuh/security-notifier` field·Kubernetes auth policy·role 및 `#security-alerts` Webhook만 사용하며 운영 Slack secret·receiver·권한을 공유하지 않는다. Slack payload는 severity·rule ID·group·agent alias·시각·Wazuh Dashboard link만 allowlist하고 raw log·IP·사용자명·token·request body·full label을 보내거나 저장하지 않는다. 보안 이벤트는 상태 경보가 아니므로 resolved 알림을 만들지 않고, level 7~13은 실제 true positive 검토 전에는 외부 전송하지 않는다. Kubernetes NetworkPolicy는 전용 notifier Pod의 TCP 443만, OPNsense는 `hooks.slack.com` FQDN alias의 TCP 443만 허용한다. FQDN alias 검증이 불가능하면 포괄 egress로 완화하지 않는다. 승인된 임시 level 14 test event 한 건이 `[TEST][SECURITY][CRITICAL] @channel`로 수신되고 원문이 없음을 확인한 뒤 rule/input/test record를 제거한다. active response·방화벽 자동 차단·Shuffle의 외부 messaging은 0건이어야 하며 최신 main의 `platform-root`·`wazuh` `Synced/Healthy`와 OPNsense drift를 확인한다. |
| `OBS-19 BLOCKED` | PostgreSQL·SeaweedFS·NetBird 사용량 baseline 후 임계치 경보 결정 (`gitops/apps/obs/`) | `OBS-15`, `OBS-16` | `ARGO-ROOT` | 경보 품질·운영 소음 | 2026-08-13부터 최소 28일(2026-09-10 KST 이후) 동안 PostgreSQL 연결/DB 상태, SeaweedFS volume 용량·disk error, NetBird connected stream·peer 상태의 일·주간 변동을 집계한다. 이 기간 전에는 새 용량·부하 임계치 경보를 추가하지 않는다. 기간 뒤 p50/p95/max와 실제 장애·점검 시간을 분리해 각 후보를 추가·보류·기각으로 판정하고, 근거가 있는 rule만 추가한다. `up`/`pg_up` 가용성 경보와 raw query·S3 object name·peer identity 수집 범위는 바꾸지 않는다. rollback은 새 threshold rule과 해당 route만 원복하며 최신 main의 `platform-root`·`obs` `Synced/Healthy` |
