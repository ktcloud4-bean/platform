...
| `AWS-SEC-DESIGN-01 READY` | AWS 보안 통제 도입 경계를 ADR로 확정 (`docs/adr/0026-aws-security-control-boundary.md`) | 없음 | 없음 | `AWS-CI-FIX-02`, `AWS-SEC-01`~`06` | 계정 전역 `tofu-account-baseline`과 앱 `tofu-app-security` 두 root의 경계·apply 순서 의존, 계정 단위 변경의 수용 범위, CIEM 조치를 AWS 안에서 끝내고 Shuffle로 통합하지 않는 근거, Object Lock COMPLIANCE 4일 선택, Slack 인터랙티브 유지 결정과 방어선, [ADR-0019](adr/0019-private-aws-service-egress.md)가 전제한 NAT Gateway 부재의 명문화, 검토한 대안과 재검토 조건 기록; 닫힌 PR #53의 브랜치 `feat/aws-security`(`74cf782`)가 `AWS-SEC-01`~`AWS-SEC-06`·`AWS-DB-SEC-01`의 파일 원본임을 ADR과 백로그 서술 양쪽에 남기고, `AWS-SEC-06` 완료 전까지 이 브랜치를 삭제하지 않는다는 조건을 기록 |
...
| `SUPPLY-01 READY` | EKS에 Kyverno와 이미지 admission 정책 신설 (`gitops/root/`, `gitops/apps/kyverno/`) | `SUPPLY-DESIGN-01` | `ARGO-ROOT` | `SUPPLY-06` | EKS destination Kyverno Application `Synced/Healthy`와 노드 available 자원이 정지 기준 밖, Audit 모드에서 기존 워크로드 위반 0건을 확인한 뒤 Enforce 전환, 자기 ECR과 AWS 관리형 애드온 레지스트리 밖 이미지의 Pod 생성 거부와 digest 미고정 거부를 각각 실증, hr-system 세 Deployment·migration Job·ALB controller·관리형 애드온 정상 기동 유지, Audit 복귀 rollback 뒤 기존 Pod 회귀 없음 |
| `SUPPLY-02 READY` | k3s Kyverno 이미지 정책 범위를 `e2e-01` 밖으로 확대하고 Audit으로 현황 확보 (`gitops/apps/kyverno/`, `policies/`) | `SUPPLY-DESIGN-01` | `ARGO-ROOT` | `SUPPLY-03` | 전 네임스페이스 Audit 정책 `Synced/Healthy`와 기존 `e2e-01` Enforce 정책 불변, 레지스트리별·네임스페이스별 위반 목록과 digest 미고정 목록을 기준선(이미지 참조 160건·digest 고정 119건·Harbor 경유 1건)과 대조해 확보, Enforce 전환 0건과 기존 워크로드 거부 0건 |
| `SUPPLY-03 BLOCKED` | Harbor upstream proxy cache project 구축 (`gitops/apps/harbor/`) | `SUPPLY-02` | `ARGO-ROOT`, `VAULT-CONFIG` | `SUPPLY-04` | 배포 직전 `k3s-01` guest available과 SeaweedFS volume slot을 재실측해 정지 기준 밖임을 확인(`capacity-plan.md`가 공급망 workload에 요구하며 직전 실측은 경고선 대비 103 MiB 여유·slot free 3), `SUPPLY-02` 목록이 실제 사용하는 upstream registry만 proxy project로 생성하고 Docker Hub는 인증 pull로 rate limit 회피, 대표 이미지의 pull-through 성공과 재요청 캐시 적중, robot credential은 Vault 소유이며 평문 0건 |
| `SUPPLY-04 BLOCKED` | k3s 이미지 참조를 Harbor proxy 경유로 전환 (`gitops/apps/`) | `SUPPLY-03` | `ARGO-ROOT` | `SUPPLY-05` | upstream 직접 참조를 Harbor proxy 경로로 전환하되 digest 보존, 전환 대상 child Application 전부 `Synced/Healthy`와 Pod 재기동 성공, 전환하지 않은 항목은 이유와 함께 목록화, Audit 위반 수가 전환 전 대비 감소했음을 같은 쿼리로 대조 |
| `SUPPLY-05 BLOCKED` | k3s 이미지 정책 Enforce 전환 (`policies/`) | `SUPPLY-04` | `ARGO-ROOT` | `SUPPLY-06` | Audit 잔여 위반 0건을 확인한 뒤 Enforce, 허용 목록 밖 레지스트리와 digest 미고정 Pod 생성 거부를 각각 실증, 기존 전 워크로드 정상 기동 유지, Audit 복귀 rollback 절차 확인 |
| `SUPPLY-06 BLOCKED` | Harbor에서 ECR로의 replication 구성 (`gitops/apps/harbor/`, `infra/aws/tofu-app-ci/`) | `SUPPLY-05`, `SUPPLY-01`, `AWS-SEC-06` | `ARGO-ROOT`, `TOFU-STATE`, `VAULT-CONFIG` | `SUPPLY-07` | Harbor `aws-ecr` adapter endpoint와 replication rule이 digest 기준으로 동작하고 ECR `IMMUTABLE` 태그 정책과 충돌 0건, 시험 아티팩트 한 건의 복제와 EKS pull 성공, Cosign 서명과 SBOM accessory가 복제 뒤에도 검증 가능, Harbor용 ECR 자격증명은 Vault 소유이며 평문 0건, 시험 아티팩트 제거 |
| `SUPPLY-07 BLOCKED` | hr-system 이미지 빌드의 push 대상을 Harbor로 전환 | `SUPPLY-06` | `ARGO-ROOT` | `SUPPLY-08` | 이 저장소 밖 `ktcloud4-bean/hr-system`의 `Jenkinsfile` 변경을 같은 작업자가 함께 소유, Jenkins가 Harbor에 push하고 replication으로 ECR에 도달하며 EKS는 기존 ECR 경로 그대로 pull, 세 이미지의 digest·Cosign 서명·SBOM이 Harbor와 ECR 양쪽에서 검증 가능, 빌드부터 Pod Ready까지 1회 왕복 성공, `SUPPLY-01` Enforce 정책 계속 통과 |
| `SUPPLY-08 BLOCKED` | Jenkins ECR publisher IAM 사용자 제거 (`infra/aws/tofu-app-ci/`) | `SUPPLY-07` | `TOFU-STATE`, `VAULT-CONFIG` | 없음 | Jenkins가 더는 ECR에 직접 push하지 않음을 파이프라인 1회 실행으로 확인한 뒤 IAM user·policy·access key 제거, Vault의 해당 자격증명 삭제, `tofu plan` 무변경, hr-system 배포 왕복 정상, 계정 잔여 정적 access key가 `backup`·`vault_auto_unseal`·`argocd_credential_issuer` 세 건으로 수렴 |
