...
| `OBS-10 BLOCKED` | Traefik 공식 Traffic Drill-down 대시보드 GitOps vendoring (`gitops/apps/obs/`) | `OBS-09`, `TRAEFIK-METRICS` | 없음 | ingress 장애 분류·운영 | Grafana.com Traefik Official Standalone Dashboard ID 17346의 특정 revision과 upstream source hash를 고정해 가져오고 Prometheus datasource UID `prometheus`, Grafana 13 panel schema, read-only provider로 정규화한다. 구현 전 Prometheus에서 template가 쓸 `traefik_*` native metric·target `up`과 실제 label set을 한 번 inventory해 entrypoint별 QPS, HTTP 상태군/5xx 비율, p95/p99 latency, entrypoint·router·service별 상위 항목 패널만 남긴다. path·client IP·사용자·인증 header와 확인되지 않은 label은 변수·패널에서 제외하고 metric 부재는 `or vector(0)`로 숨기지 않고 `No data`로 표시한다. `OBS-05`의 Traefik QPS·상태·지연시간 3개 요약 패널은 새 drill-down으로 가는 단일 링크로 바꿔 같은 시각화를 중복하지 않는다. Grafana read-only Viewer에서 dashboard와 drill-down이 표시되고 Prometheus API 결과와 panel query/model이 일치해야 하며, synthetic traffic 생성·Traefik static config/HelmChartConfig·Pod 재기동·ServiceMonitor·NetworkPolicy·PVC·Secret·Ingress·외부 노출 변경은 0건이어야 한다. 2026-08-11 Prometheus inventory에서 Traefik target·`traefik_*` metric이 모두 0건이라 dashboard와 `OBS-05` link를 배포하지 않고 [`docs/evidence/obs-10/README.md`](evidence/obs-10/README.md)에 근거를 남겼다. `TRAEFIK-METRICS` 완료 뒤 재개한다. rollback은 dashboard ConfigMap과 `OBS-05` 링크만 원복하고 최신 main의 `platform-root`·`obs` `Synced/Healthy` |
| `TRAEFIK-METRICS READY` | packaged Traefik의 private Prometheus scrape 경로 준비 (`gitops/apps/ingress/`, `gitops/apps/obs/`) | `OBS-09` | `TRAEFIK-LIVE` | `OBS-10`·ingress 관측 | 2026-08-11 read-only inventory에서 Traefik은 `metrics` entrypoint와 Prometheus metric을 이미 켰지만, `traefik` Service에는 80/443만 있고 Traefik ServiceMonitor·Prometheus target·`traefik_*` metric은 모두 0건이었다. check-first로 기존 metric entrypoint·Pod args를 보존하고, 필요할 때만 static config/HelmChartConfig를 바꿔 유일한 Traefik Pod 재기동 영향과 rollback을 판정한다. TCP 9100은 기존 외부 serving Service에 추가하지 않고 Traefik Pod selector의 private ClusterIP metrics Service, `obs` namespace의 `release=obs` ServiceMonitor, Prometheus→Traefik Pod TCP 9100 exact NetworkPolicy만으로 수집한다. 실제 native metric과 entrypoint·code·router·service label set을 값 없이 inventory해 `OBS-10`이 사용할 범위를 확정하고, target `up=1`, QPS·HTTP 상태군/5xx·p95/p99·top entrypoint/router/service 대표 series, 전후 Prometheus head series·working set·k3s available RAM/swap을 판정한다. 기존 web/websecure Service, 공개 DNS/NAT/외부 9100, access-log header 정책, Secret/PVC/Ingress와 확인된 것 외 label 수집은 변경하지 않으며, rollback은 metric entrypoint 변경(있을 때), private Service·ServiceMonitor·NetworkPolicy를 함께 원복한다. 최신 main의 `platform-root`·`ingress`·`obs` `Synced/Healthy` |
| `OBS-11 READY` | Proxmox·Linux VM node-exporter fleet과 Node Exporter Full 대시보드 (`infra/ansible/`, `gitops/apps/obs/`) | `OBS-01`, `VM-01`, `NET-04` | `OPNSENSE-LIVE` | 물리·VM 자원 장애 분류 | `ip-plan.md`의 Proxmox와 `k3s-01`·`postgres-01`·`object-01`·`warpgate-01`·`netbird-01` 여섯 Linux 대상의 OS·TCP 9100 점유·기존 exporter를 read-only inventory한 뒤, `k3s-01`의 기존 DaemonSet은 중복 agent 없이 재사용하고 나머지 host/guest에는 checksum 고정 node-exporter systemd unit을 선언한다. 기본 Linux collector와 Node Exporter Full ID 1860의 특정 revision·source hash를 Prometheus datasource UID `prometheus`, Grafana 13 panel schema, read-only provider에 맞춰 vendoring하고, dashboard 변수는 실제 `job`·`instance` label로 정규화한다. `systemd`·`processes`는 default-disabled collector이므로 한 Linux 대상 canary에서 `scrape_duration_seconds`·`scrape_samples_post_metric_relabeling`·Prometheus head series 증가를 판정해 정지 기준 안일 때만 전 대상에 확대하며, 미달이면 collector가 필요한 panel만 제거하고 권한·kernel 설정은 약화하지 않는다. 각 target의 node CPU·memory·filesystem·disk I/O·network 대표 시계열과 가능한 systemd/process 시계열, `up=1` 여섯 건, Grafana Viewer의 host 전환·panel query/model 일치를 확인한다. Prometheus는 private target TCP 9100만 ScrapeConfig로 수집하고 obs egress·OPNsense rule·각 host listen/firewall은 Prometheus source에만 정확히 한정하며, OPNsense의 기존 CPU·meminfo·netdev collector와 scrape는 변경하지 않는다. 적용 전후 `prometheus_tsdb_head_series`·Prometheus working set·retention 3d/6GiB·k3s available RAM/swap·Proxmox available RAM/swap·thin data/metadata를 같은 측정으로 정지 기준 밖에 둔다. 신규 VM·디스크·PVC·Secret·공개 DNS/NAT·외부 TCP 9100은 0건이며 rollback은 exporter unit·ScrapeConfig·egress/firewall rule·dashboard ConfigMap과 canary collector flag를 함께 원복, 최신 main의 `platform-root`·`obs` `Synced/Healthy` |
