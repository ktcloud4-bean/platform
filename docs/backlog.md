...
| `S3-02 READY` | SeaweedFS admin 웹 UI(`object-admin.imcherry5778.xyz`)를 Pomerium Route로 노출해 `/platform-privileged`(`imcherry5778-admin`)만 접근하도록 제한 | `S3-01`, `POM-01` | `OPNSENSE-LIVE`, `ARGO-ROOT` | `object-01` 운영 가시성 | 신규 `seaweedfs-admin` systemd unit(자체 TLS·자체 로그인)을 선언하고 신규 PF rule로 k3s-01(`10.10.20.10/32`) 단일 source만 허용(기존 8333 rule과 동일 패턴)하며 master(9333)·volume(8080)·filer(8888, 변경 없음) 관리 포트는 loopback 유지, Unbound alias `object-admin.imcherry5778.xyz` → `k3s-01`(`10.10.20.10`) 등록과 `ip-plan.md` 노출 정의 갱신, Pomerium Route는 `/platform-privileged`만 allow(`/platform-users`·미인증 요청 403/redirect 실측)하고 backend는 admin 자체 TLS(`tls_skip_verify`)로 연결, `pomerium`→`object-01` NetworkPolicy egress 명시, `imcherry5778-admin`의 실제 로그인 성공과 admin 자체 인증(WEED_ADMIN_PASSWORD) 성공·실패 양쪽 실측, admin이 filer보다 넓은 관리 권한(volume·EC·maintenance)을 가진다는 한계를 runbook에 명시, 기존 S3 API 8333·filer loopback 경로·PF rule·credential 모델 회귀 없음, `ARGO-ROOT` 잠금 아래 `platform-root`·`pomerium` 커밋 SHA 검증 후 main·Synced/Healthy 복귀, rollback은 Pomerium Route·PF rule·`seaweedfs-admin` unit 제거 |
...
것이다. filer는 이 작업으로 바뀌지 않고 계속 loopback 전용으로 남는다.
