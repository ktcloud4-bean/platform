...
| `HEADLAMP-02-FIX-02 DONE` | rollback 기준선에서 OIDC/RBAC 선언을 재도입하고, Argo 상태의 SSH 인수 경계를 JSON+pipe 사전검증으로 보정 | `HEADLAMP-01`, `POM-01` | `K3S-BOOTSTRAP` | k3s 일상 관리·`CAP-02` | HEADLAMP-02 전체 live 증거·rollback·break-glass 재검증, shared cluster-admin SA/token 없음 |
