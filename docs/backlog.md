...
| `BKP-08 READY` | NetBird·Warpgate 정기 백업 스케줄 신설 | `NB-01`, `WG-01`, `S3-01` | `OPNSENSE-LIVE` | NetBird·Warpgate 구성·세션 기록 복구 | `architecture.md`가 "NetBird·Warpgate 제품 DB·구성 백업"이라 문서화했지만 `NB-01`·`WG-01`은 배포 당시 백업 메커니즘을 한 번 검증만 했을 뿐(`/var/backups/netbird/` tar, Warpgate 격리 인스턴스 복원) 정기 timer가 실제로는 없음을 라이브로 확인(두 host 모두 `systemctl list-timers`에 backup 관련 항목 0건). `BKP-03`과 같은 패턴(전용 비로그인 system user, `Read/List/Write:<bucket>`만 가진 최소권한 S3 identity, systemd timer, 실패·freshness health check)으로 `bkp-08-netbird`·`bkp-08-warpgate` 두 bucket에 각각 매일 백업하는 unit 신설, NetBird-01·Warpgate-01에서 `object-01:8333`으로 향하는 신규 exact PF rule(k3s-01 rule과 같은 sequence 배치 원칙 적용) 추가, 실제 백업 성공과 격리 환경 복원 1회 검증, 재부팅 후 timer 유지 확인 |
