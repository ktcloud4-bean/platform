...
| `DEMO-OPENING-01 READY` | 세션 1·2·3·5의 온프레미스 흐름과 세션 4의 HR System/EKS 흐름으로 데모 5편의 16:9 오프닝 이미지용 영문 프롬프트를 만들고, 사용자가 chatgpt.com에서 생성한 후보를 함께 보정해 세션별 최종 이미지 한 장을 선정한다 (Notion `데모 시나리오`) | `DEMO-RECOVERY-01`, `DEMO-AWS-HR-01` | 없음 | `DEMO-SCRIPT-01` | 세션을 한 편씩 진행해 영문 prompt v1 → 사용자 생성 후보 → 구성·기술 정확성 검토 → 수정 prompt 순서로 반복하고 사용자가 각 최종본을 승인한다. 프롬프트는 공통 가상 기업의 일관된 dark cinematic security infographic 디자인, 16:9·좌우 안전영역·한눈에 읽히는 단방향 흐름을 유지하되 각 편은 독립적으로 이해되어야 한다. 실제 등장하는 핵심 제품의 공식 SVG/PNG 원본과 공식 출처를 참조자료로 제공하고, 생성 모델이 로고·글자를 다시 그리게 하지 않는다. 모델이 아이콘을 왜곡하면 아이콘 없는 배경과 배치 공간을 생성한 뒤 편집에서 공식 원본을 그대로 합성한다. 각 Notion 페이지에 최종 영문 prompt·금지 조건·아이콘 출처·선정 이미지를 남기며 Secret·실데이터·가짜 UI·읽을 수 없는 생성 텍스트는 0건이다. |
| `DEMO-SCRIPT-01 BLOCKED` | 확정된 촬영 인터페이스와 선정된 오프닝 이미지를 기준으로 데모 5편의 실제 녹화·후시녹음 대본을 작성한다 (Notion `데모 시나리오`) | `DEMO-OPENING-01` | 없음 | `DEMO-RECORD-01` | 각 Notion 페이지의 `데모 흐름` 아래에 오프닝 이미지 설명, 화면 동작, 자연스러운 내레이션, 컷 포인트, 기술 증거와 마무리를 시간대별로 기록한다. 이야기 약 70%·기술 증거 약 30%, 얼굴 없는 화면 녹화와 음성/후시녹음을 기준으로 하며 구현에서 실제 확인된 URL·입력·상태·rule ID만 사용한다. 세션 4는 HR System/EKS의 dry-run 차단·기존 서비스 무중단을 사용하고 나머지는 확정된 온프레미스 인터페이스를 따른다. JWT·cookie·OTP·Secret·실데이터 원문은 노출하지 않고 세션별 리허설 한 번에서 3~5분, attack/control/evidence/reset 순서와 최종 사용자 승인을 확인한다. |
| `DEMO-RECORD-01 BLOCKED` | 사용자가 데모 5편을 한 편씩 실제 녹화하면서 에이전트와 채팅으로 리허설·문제 판별·재촬영 피드백을 반복해 최종 take를 확정 | `DEMO-SCRIPT-01` | 없음 | 데모 영상 편집·최종본 | 세션마다 에이전트가 촬영 전 화면 순서·창/확대·예상 결과·reset·민감정보 마스킹 checklist로 GO/STOP을 판정하고, 사용자가 녹화한 화면·스크린샷·구간별 결과를 채팅으로 공유하면 기술 주장·가독성·호흡·컷·3~5분 길이를 검토한다. 문제는 실패 화면과 원인을 먼저 기록한 뒤 대본·동선 또는 안전한 fixture만 보정해 같은 세션을 재촬영하고, 사용자가 최종 take를 승인한다. 에이전트는 recorder·사람 승인 버튼·credential 입력을 대신 조작하지 않으며, 다섯 최종 take 모두 실제 구현 증거와 일치하고 JWT·cookie·OTP·Secret·실데이터·가짜 성공 장면 노출 0건, 각 촬영 뒤 attack/control/evidence/reset 또는 HR dry-run no-op 정리를 확인한다. |
