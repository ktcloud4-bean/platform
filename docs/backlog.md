...
| `OBS-04 READY` | Platform Capacity Sentinel 대시보드 구축 (`gitops/apps/obs/`) | `OBS-03` | 없음 | 관측·운영 경보 | `obs-04-capacity-sentinel` ConfigMap 선언 및 Grafana 마운트, RAM(8/12GiB)·PVC(96/120GiB)·thin-pool(60/70%) 정지 기준 Threshold 시각화, Argo child `obs` `Synced/Healthy` |
| `OBS-05 BLOCKED` | Core Services & Traffic Infrastructure 대시보드 구축 (`gitops/apps/obs/`) | `OBS-04` | 없음 | 관측·운영 | `obs-05-core-services` ConfigMap 선언 및 Grafana 마운트, Traefik(QPS/Status/Latency)·Pomerium(SSO/Route)·PostgreSQL(쿼리/커넥션/WAL)·Vault(Unseal/Cert-TTL) 패널 표시, Argo child `obs` `Synced/Healthy` |
| `OBS-06 BLOCKED` | Data Protection & Backup Pipelines 대시보드 구축 (`gitops/apps/obs/`) | `OBS-05` | 없음 | 관측·백업 | `obs-06-backup-pipelines` ConfigMap 선언 및 Grafana 마운트, Velero 백업 성공률·SeaweedFS S3 용량/일일 증가율·AWS S3 오프사이트 헬스 시각화, Argo child `obs` `Synced/Healthy` |
| `OBS-07 BLOCKED` | Loki Operational Log Explorer 대시보드 구축 (`gitops/apps/obs/`) | `OBS-06` | 없음 | 관측·로그 | `obs-07-loki-log-explorer` ConfigMap 선언 및 Grafana 마운트, Namespace/App별 5분 단위 `level=error|warn` 로그 발생률 차트 및 dynamic filter Log Stream 패널 표시, Argo child `obs` `Synced/Healthy` |
