...
| `BKP-10 READY` | Gitea(`hr-system`)·Jenkins PVC 데이터의 백업 경로 확보 | `SCM-01`, `CI-01`, `BKP-07` | 없음 | 소스 저장소·CI 자격증명·빌드 이력 복구 가능성 | `BKP-07`에서 Velero `defaultVolumesToFsBackup: true`가 cluster 전체 거의 모든 volume에서 실패해(112 errors) resource-only로 축소했고, 그 결과 `gitea-data`(10Gi)·`jenkins-home`(20Gi) PVC는 지금 백업이 전혀 없음. 원인(Kopia FS backup이 왜 광범위하게 실패하는지) 조사부터 시작해, (a) 원인이 좁혀지면 해당 namespace만 opt-in `defaultVolumesToFsBackup`으로 재시도하거나 (b) 안 되면 앱별 native export(Gitea `dump`, Jenkins 설정/이력 export)로 전용 S3 bucket에 업로드하는 경로 중 실제로 작동하는 것 최소 하나를 확보. 데이터 손실 시나리오로 실제 복원 1회 검증하고, `hr-system`이 GitHub 미러 대상인지도 확인해 결과를 문서화(미러 대상이면 위험도 재평가) |
