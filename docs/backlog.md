...
| `HEADLAMP-02-FIX-03 DONE` | OIDC/RBAC 선언을 rollback 기준선에서 재도입하고, master recovery Authorization Code callback을 캡처하도록 KC-01 검증을 보정 | `HEADLAMP-01`, `POM-01` | `K3S-BOOTSTRAP` | k3s 일상 관리·`CAP-02` | HEADLAMP-02 전체 live 증거·rollback·break-glass 재검증, callback code 보존, shared cluster-admin SA/token 없음 |
