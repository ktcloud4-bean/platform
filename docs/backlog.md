...
| `BKP-09 READY` | `BKP-01`(k3s datastore)·`BKP-02`(Velero) bucket을 AWS 오프사이트 대상에 추가 | `BKP-01`, `BKP-02`, `BKP-04` | 없음 | k3s·Velero 리소스의 물리 재해 대응 | 현재 `object-01`의 `OFFSITE_SOURCE_BUCKETS`는 `bkp-03-postgres bkp-03-vault`만 있고 `bkp-01-k3s-datastore`·`bkp-02-velero`는 빠져 있어, `object-01`을 호스팅하는 Proxmox NVMe가 죽으면 이 두 백업은 원본과 함께 소멸함을 실측으로 확인(offsite.env 직접 조회). 기존 `bkp-03-offsite-reader`와 같은 패턴으로 두 bucket에 `Read/List`만 가진 전용 offsite reader identity를 발급하고 `OFFSITE_SOURCE_BUCKETS`에 추가, 실제 전송 성공(heartbeat이 아닌 실 object)과 AWS 측 별도 위치 샘플 복원 1회 검증, 기존 postgres·vault 오프사이트 경로 회귀 없음 |
