...
| `SUPPLY-01-FIX-02 READY` | EKS `hr-db-migrate` PreSync가 Pod를 생성하지 못한 채 deadline을 초과한 원인을 Kyverno controller의 `failed to build cosign verification opts: getting Rekor public keys: rekor URL must be provided`로 특정하고, EKS `ImageValidatingPolicy`의 Rekor/CTLog 검증 계약을 최소 보정 (`gitops/apps/kyverno-eks/`, `gitops/tools/supply-01-fix-02/`, `docs/evidence/supply-01-fix-02/`) | `SUPPLY-01-FIX-01` | `ARGO-ROOT`, `IDENTITY-LIVE` | HR desired release 수렴, EKS 공급망 admission | 적용 전에 Git이 선언한 현재 HR service digest의 image signature와 signed CycloneDX attestation을 ECR에서 current 또는 previous 공개키로 독립 검증한다. hardcoded digest allowlist·broad 예외·`failurePolicy=Ignore`·migration deadline 연장 없이 같은 digest의 server-side admission은 통과하고 미서명 fixture는 거부되어야 하며, Kyverno 로그의 Rekor URL 오류 0건, 실제 `hr-db-migrate` Pod 생성과 Job `Succeeded`, 현재 bootstrap HR 관리자 record 보존, 최신 main의 `platform-root`·`kyverno-eks`·`hr-system` `Synced/Healthy`를 확인한다. |
