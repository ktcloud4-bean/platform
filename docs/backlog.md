...
| `HEADLAMP-02 DONE` | Headlamp Keycloak OIDC·Kubernetes RBAC·Pomerium Route; 이전 rollback은 `HEADLAMP-02-FIX-01`에서 verifier SSH 인수 경계를 보정해 재검증 | `HEADLAMP-01`, `POM-01` | `K3S-BOOTSTRAP` | k3s 일상 관리·`CAP-02` | 공유 cluster-admin SA 없음, 사용자별 조회·로그·exec·변경 allow/deny, bootstrap token 폐기, GitOps drift 없음, IdP 장애 시 break-glass kubeconfig |
| `HEADLAMP-02-FIX-01 DONE` | rollback 기준선에서 Headlamp OIDC/RBAC 선언을 재도입하고, Argo 상태 JSON 검증의 SSH 인수 경계를 보정해 전체 라이브 증거를 재확보 | `HEADLAMP-01`, `POM-01` | `K3S-BOOTSTRAP` | k3s 일상 관리·`CAP-02` | HEADLAMP-02의 모든 완료 증거와 rollback·break-glass를 새 세션에서 재확인; 공유 cluster-admin SA/token 없음 |
