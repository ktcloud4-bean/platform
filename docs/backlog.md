...
| `AWS-SEC-DESIGN-01 READY` | AWS 보안 통제 도입 경계를 ADR로 확정 (`docs/adr/0026-aws-security-control-boundary.md`) | 없음 | 없음 | `AWS-CI-FIX-02`, `AWS-SEC-01`~`06` | 계정 전역 `tofu-account-baseline`과 앱 `tofu-app-security` 두 root의 경계·apply 순서 의존, 계정 단위 변경의 수용 범위, CIEM 조치를 AWS 안에서 끝내고 Shuffle로 통합하지 않는 근거, Object Lock COMPLIANCE 4일 선택, Slack 인터랙티브 유지 결정과 방어선, [ADR-0019](adr/0019-private-aws-service-egress.md)가 전제한 NAT Gateway 부재의 명문화, 검토한 대안과 재검토 조건 기록 |
