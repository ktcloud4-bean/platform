...
| `BKP-11 READY` | postgres·vault backup health-check(freshness) 유닛 실패를 Prometheus·Slack에 연결 | `BKP-06`, `BKP-07` | `ARGO-ROOT` | 인증·플랫폼 데이터 RPO 가시성 | `postgres-native-backup-health.service`·`vault-raft-backup-health.service`(36시간 `bkp03_health_max_age_seconds` freshness 기준으로 이미 존재하는 장치)는 지금 실패해도 아무 데도 통보되지 않음(`BKP-07`의 3개 alert는 backup 본체 `.service`만 보고, timer가 꺼지거나 host가 죽어 아예 실행 시도조차 없는 경우는 `failed`가 아니라 `inactive`로 얼어붙어 못 잡음 — health-check가 이 사각지대를 잡는 유일한 장치). 새 exporter·방화벽 불필요(기존 fleet가 이미 `node_systemd_unit_state`로 두 unit을 수집 중). `PostgreSQLNativeBackupHealthStale`·`VaultRaftBackupHealthStale` alert 2개를 `state="failed"==1`로 추가하고 기존 `obs-13-receiver`·`obs-18-platform-slack` matcher에 등록, `ARGO-ROOT` 잠금 아래 검증 후 main 복귀 |
