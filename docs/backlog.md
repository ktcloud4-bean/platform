...
| `DEMO-RECOVERY-01 READY` | 데모 전용 합성 고객 데이터와 소형 `local-path` PVC, 결정론적 훼손·격리 복원 하네스를 추가해 세션 5의 GitOps 선언 복구와 Velero/Kopia 데이터 복구 대조를 촬영 가능한 한 흐름으로 만든다 (`gitops/apps/`, `gitops/tools/`, `docs/`) | `DEMO-ONPREM-01`, `BKP-02`, `BKP-10`, `POL-03`, `POL-03-FIX-01` | `ARGO-ROOT` | `DEMO-OPENING-01` | 시작 전 현재 PVC 선언 합계와 `k3s-01` 파일시스템 여유를 재측정하고 96 GiB 경고·25% 여유 경계에 걸리면 적용하지 않으며, 1 GiB 이하 데모 전용 PVC와 합성 데이터만 만든다. Pod에 `backup.velero.io/backup-volumes`를 명시해 정상 marker·hash를 가진 Velero Backup `Completed`와 PVB 전송 바이트를 확보한다. 전용 fixture는 이 PVC 데이터만 훼손하고 데모 Deployment를 제거하며, Argo CD가 선언을 self-heal한 뒤에도 데이터 오류가 남는 대조를 재현한다. 알려진 정상 Backup을 격리 namespace에 한 번 복원해 Restore `Completed`, PVR, 원본과 같은 marker·hash, 서비스 정상 응답과 측정 RTO를 확인한다. 실제 랜섬웨어·기존 workload/PVC·AWS apply·AWS 화면·전체 DR 성공 주장은 0건이며, reset 뒤 이 작업이 만든 Backup/Restore/PVB/PVR·namespace·PVC·공격 자원 0건과 최신 main의 관련 Argo child `Synced/Healthy`를 확인한다. |
| `DEMO-OPENING-01 BLOCKED` | `DEMO-ONPREM-01`과 `DEMO-RECOVERY-01`에서 확정한 기술 흐름으로 온프레미스 데모 5편의 16:9 오프닝 흐름 이미지용 영문 프롬프트를 만들고, 사용자가 chatgpt.com에서 생성한 후보를 함께 보정해 세션별 최종 이미지 한 장을 선정한다 (Notion `데모 시나리오`) | `DEMO-RECOVERY-01` | 없음 | `DEMO-SCRIPT-01` | 세션을 한 편씩 진행해 영문 prompt v1 → 사용자 생성 후보 → 구성·기술 정확성 검토 → 수정 prompt 순서로 반복하고 사용자가 각 최종본을 승인한다. 프롬프트는 공통 가상 기업의 일관된 dark cinematic security infographic 디자인, 16:9·좌우 안전영역·한눈에 읽히는 단방향 흐름을 유지하되 각 편은 독립적으로 이해되어야 한다. 실제 등장하는 핵심 제품의 공식 SVG/PNG 원본과 공식 출처를 참조자료로 제공하고, 생성 모델이 로고·글자를 다시 그리게 하지 않는다. 모델이 아이콘을 왜곡하면 아이콘 없는 배경과 배치 공간을 생성한 뒤 편집에서 공식 원본을 그대로 합성한다. 각 Notion 페이지에 최종 영문 prompt·금지 조건·아이콘 출처·선정 이미지를 남기며 Secret·실데이터·가짜 UI·읽을 수 없는 생성 텍스트는 0건이다. |
