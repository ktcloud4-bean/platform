...
| HR 애플리케이션 | HR System | private EKS; internal ALB, Pomerium·shared S2S VPN 뒤, private Aurora PostgreSQL·서비스별 IRSA·Secrets Manager DB credential |
...
[ADR-0018](adr/0018-public-keycloak-frontchannel.md)은 신규 외부 장치의 NetBird OIDC bootstrap을
위해 `sso`의 Keycloak `platform` realm 사용자 프런트엔드만 Cloudflare proxy 뒤에 공개한다.
Cloudflare hostname Origin Rule은 NetBird WAN TCP 443과 다른 origin port를 사용하고 OPNsense는
Cloudflare source만 k3s ingress HTTPS로 전달한다. `/admin/`, 관리 realm, health·metrics,
...
| Cloudflare | `EDGE-02`의 `sso` 사용자 프런트엔드 WAF·프록시·origin port 선택; DNS-01과 NetBird DNS-only record는 별도 |
...
외부 사람 장치의 온보딩은 관리자가 사전 생성한 `/platform-users` 일상 ID의 NetBird OIDC와
MFA를 사용한다. 공개 self-registration과 사람용 setup key는 사용하지 않는다. NetBird peer라는
장치 신원과 Keycloak 사용자 신원은 서로 다른 수명주기이므로 offboarding은 Keycloak session·
group과 NetBird 사용자·peer를 함께 회수한다. `/platform-privileged`만 가진 특권 ID는 peer
소유자가 아니며 이미 연결된 승인 장치에서 필요한 서비스에 별도 로그인한다.
