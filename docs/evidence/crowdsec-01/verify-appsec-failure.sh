...
appsec=$(remote_kubectl -n crowdsec-01 get pod -l k8s-app=crowdsec-01,type=appsec -o json)
...
remote_kubectl -n crowdsec-01 rollout status deployment/crowdsec-01-appsec --timeout=180s >/dev/null
new_appsec=$(remote_kubectl -n crowdsec-01 get pod -l k8s-app=crowdsec-01,type=appsec -o json)
