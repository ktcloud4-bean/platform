...
cleanup() {
  docker rm -f "$TRAEFIK" "$APPSEC" "$LAPI" "$BACKEND" "$SEED" "$REGISTER" >/dev/null 2>&1 || true
...
for object in "$TRAEFIK" "$APPSEC" "$LAPI" "$BACKEND" "$SEED" "$REGISTER"; do
...
cp -a "$APP_DIR/files/crs/." "$scratch/appsec-data/"
cp "$APP_DIR/crs-snapshot.SHA256" "$scratch/appsec-data/SHA256SUMS"
(
