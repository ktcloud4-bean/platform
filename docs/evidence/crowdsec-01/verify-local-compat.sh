...
readonly PLUGIN_NETWORK=crowdsec-01-local-plugin-net
...
cleanup() {
  ...
  docker network rm "$PLUGIN_NETWORK" "$EDGE_NETWORK" "$APPSEC_NETWORK" >/dev/null 2>&1 || true
...
for network in "$APPSEC_NETWORK" "$EDGE_NETWORK" "$PLUGIN_NETWORK"; do
...
DOCKER_START_SHA256=$(metadata_value CROWDSEC_DOCKER_START_SHA256)
readonly DOCKER_START_SHA256
OFFLINE_STARTUP_SHA256=$(metadata_value CROWDSEC_OFFLINE_STARTUP_SHA256)
readonly OFFLINE_STARTUP_SHA256
OFFLINE_STARTUP_BUILDER_SHA256=$(metadata_value CROWDSEC_OFFLINE_STARTUP_BUILDER_SHA256)
readonly OFFLINE_STARTUP_BUILDER_SHA256
[[ $(sha256sum "$APP_DIR/files/appsec/prepare-offline-startup.sh" | awk '{print $1}') == "$OFFLINE_STARTUP_BUILDER_SHA256" ]]
docker run --rm --name "$INIT" --entrypoint sh \
  -v "$APP_DIR/files/appsec/prepare-offline-startup.sh:/builder/prepare-offline-startup.sh:ro" \
  -v "$scratch/offline-startup:/offline-startup" \
  "$CROWDSEC_IMAGE" -c \
  "sh /builder/prepare-offline-startup.sh /docker_start.sh /offline-startup/docker_start.sh $DOCKER_START_SHA256 $OFFLINE_STARTUP_SHA256" \
  >/dev/null
[[ $(sha256sum "$scratch/offline-startup/docker_start.sh" | awk '{print $1}') == "$OFFLINE_STARTUP_SHA256" ]]
docker run --rm --name "$INIT" --entrypoint sh \
...
docker network create --internal "$APPSEC_NETWORK" >/dev/null
docker network create --internal "$EDGE_NETWORK" >/dev/null
docker network create "$PLUGIN_NETWORK" >/dev/null
[[ $(docker network inspect "$APPSEC_NETWORK" --format '{{.Internal}}') == true ]]
[[ $(docker network inspect "$EDGE_NETWORK" --format '{{.Internal}}') == true ]]
[[ $(docker network inspect "$PLUGIN_NETWORK" --format '{{.Internal}}') == false ]]
...
docker run -d --name "$APPSEC" --network "$APPSEC_NETWORK" \
  ...
  -v "$scratch/offline-startup/docker_start.sh:/offline-startup/docker_start.sh:ro" \
  --entrypoint sh "$CROWDSEC_IMAGE" -c 'exec /offline-startup/docker_start.sh' >/dev/null
docker network connect "$EDGE_NETWORK" "$APPSEC"
docker run -d --name "$BACKEND" --network "$EDGE_NETWORK" \
  "$BACKEND_IMAGE" --port=8080 >/dev/null
docker create --name "$TRAEFIK" --network "$PLUGIN_NETWORK" \
...
docker network connect "$EDGE_NETWORK" "$TRAEFIK"
...
docker logs "$LAPI" > "$scratch/lapi.log" 2>&1
grep -Fqx 'CROWDSEC-01_OFFLINE_STARTUP: Hub preparation disabled' "$scratch/crowdsec.log"
if rg -n 'Running hub update|version\.crowdsec\.net|cscli .* (parsers|collections|scenarios|postoverflows|appsec-configs|appsec-rules) (install|upgrade)' \
  "$scratch/crowdsec.log" "$scratch/lapi.log"; then
  printf '%s\n' '실패: egress 없는 호환 시험에서 Hub 설치·갱신 호출이 발견됐다.' >&2
  exit 1
fi
