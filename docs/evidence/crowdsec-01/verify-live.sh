#!/usr/bin/env bash
# 승인·main 통합·Argo sync 뒤 CROWDSEC-FIX-01 기능과 기존 ingress 회귀를 읽기 전용 검증한다.
set -euo pipefail
...
current_rs_uid=$(jq -r \
  '.items[0].metadata.ownerReferences[] | select(.kind == "ReplicaSet") | .uid' \
  <<< "$traefik")
jq -e --arg uid "$current_rs_uid" \
  'any(.items[]; .metadata.uid == $uid and .spec.replicas == 1 and .status.replicas == 1)' \
  <<< "$current_rs" >/dev/null
...
decisions=$(remote_kubectl -n crowdsec-01 exec "deploy/${helm_release}-lapi" -- \
  cscli decisions list -o json)
jq -e 'length == 0' <<< "$decisions" >/dev/null
unset decisions whoami

apps=$(remote_kubectl -n argocd get application platform-root ingress keycloak crowdsec -o json)
  ...
  jq -e 'any(.items[];
    ...
    (.status.phase == "Running" and
      ((.status.containerStatuses // [] | length) == 0 or
       ((.status.containerStatuses // []) | any(.ready != true)))))' >/dev/null; then
...
printf '%s\n' 'PASS: CROWDSEC-FIX-01 기능, decision 0, 기존 ingress object/TLS/301/source-IP, root/ingress/keycloak/crowdsec Argo와 cluster health를 검증했습니다.'
