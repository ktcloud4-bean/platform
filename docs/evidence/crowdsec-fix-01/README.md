...
- NetworkPolicy selector의 `k8s-app=crowdsec`와 실제 Deployment의
  `k8s-app=crowdsec-01` 불일치를 release name 단일 원본으로 보정했다.
...
- 3 round p95·CPU·RSS, AppSec fail-closed live gate
- 수정 enablement 상태에서의 기존 ingress 전체 회귀
- 성공 enablement의 Argo rollback·merge 후 Git revert drill
