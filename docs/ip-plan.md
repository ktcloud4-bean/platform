...
| `10.10.20.12` | `k3s-01` SUPPLY-01 Kyverno TUF egress source | `LIVE`; node의 TUF 전용 outbound source identity |
...
| `seaweedfs.imcherry5778.xyz` | service alias | SeaweedFS admin 웹 UI | Pomerium; `/platform-privileged`만 허용(admin 자체 로그인은 SSO 미지원이라 S3-02-FIX-01에서 끔) | Unbound alias → `k3s-01`(`10.10.20.10`) 등록(S3-02); Pomerium이 k3s-01→object-01 TCP 23646(자체 TLS)로 전달, 내부 AAAA·공개 A/AAAA 0건 |
