...
| `vmbr0`에 `bridge-vlan-aware`·`bridge-vids` 없음 | 설치 시점에는 Phase 1 untagged가 맞았다. Proxmox 쪽은 `NET-02`에서 VLAN-aware bridge와 tagged 관리 인터페이스로 전환됐고, OPNsense 쪽도 `NET-02R`에서 영속 할당·재부팅 후 tagged-only 경로를 검증했다 ([`opnsense-proxmox-tagged-trunk.md`](opnsense-proxmox-tagged-trunk.md)) |
