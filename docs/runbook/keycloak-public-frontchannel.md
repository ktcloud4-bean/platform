...
- 현재 실행 상태: **설계 완료, 라이브 미적용**

## 목적과 현재 경계

`EDGE-02`는 외부 신규 장치가 기존 Keycloak 일상 ID와 MFA로 NetBird OIDC 로그인을 시작할 수
있게 `sso` 사용자 프런트엔드만 공개한다. 이 문서가 main에 들어가도 공개 DNS, Cloudflare,
OPNsense, Traefik과 Keycloak live 상태는 바뀌지 않는다. 적용 완료 전 현재 공개 경계는
[`EDGE-01`](netbird-public-edge.md)의 DNS-only `netbird` 하나와 기존 NetBird NAT뿐이다.
