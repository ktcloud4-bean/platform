...
`https://object-admin.imcherry5778.xyz`(Pomerium + 자체 로그인, `/platform-privileged`만
허용).
...
- `/etc/seaweedfs/security.toml`의 `[https.admin]`에 기존 S3 gateway TLS
  ...
  켠다.
- `/etc/seaweedfs/admin.env`(mode `0600`, root 전용, `no_log`)에
  `WEED_ADMIN_USER`·`WEED_ADMIN_PASSWORD`를 저장소 밖 extra-vars로 주입한다. 빈
  값이면 role이 실패해 평문 미인증 관리 콘솔이 만들어지지 않는다.
...
- Unbound alias `object-admin.imcherry5778.xyz` → `k3s-01`(`10.10.20.10`)을 등록한다.
  다른 Pomerium Route와 같은 이유로 실제 backend(`object-01`)가 아니라 Pomerium이
  있는 host를 가리킨다.
- Pomerium Route `object-admin`은 `to: https://10.10.50.20:23646`(`tls_skip_verify:
...
**filer보다 넓은 권한 범위라는 한계.** admin은 자체 로그인·TLS로 filer의 인증
공백은 메우지만, 다루는 범위는 filer(파일 읽기/쓰기/삭제)보다 넓다 — volume 재배치,
EC(erasure coding) 작업, maintenance worker, cluster 설정 변경까지 이 UI 하나로
가능하다. `S3-01`의 bucket-scoped S3 credential 최소권한 모델과는 여전히 별개의,
더 넓은 신뢰 경계로 다룬다.

로그인 성공(session cookie 발급, `/admin` 200)과 잘못된 비밀번호 거부(`/login?error=
Invalid credentials`)를 `object-01` loopback에서 직접 확인했다.
