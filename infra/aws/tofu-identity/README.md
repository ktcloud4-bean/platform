...
`AWS-ID-01`이 소유하는 세 번째 AWS OpenTofu root다. Keycloak `platform` realm의
SAML assertion으로만 AWS 콘솔 `AssumeRoleWithSAML` 임시 세션을 발급한다.

## state 경계

이 root는 IAM SAML provider 하나와 사람용 IAM role 두 개만 소유한다.
...
|---|---|---|---|
| `/platform-users` | `platform-saml-observer` | VPN/VPC 관련 EC2 `Describe*`, 현재 임시 session 확인 | 모든 변경, IAM, S3·backup bucket |
| `/platform-privileged` | `platform-saml-identity-reader` | 위 조회 + 이 root의 두 role과 이 SAML provider 읽기 | 모든 변경, IAM user/access key, `PassRole`, S3·backup bucket |

두 role의 trust policy는 이 root가 만든 federated SAML provider ARN과
...
1. plan의 추가 대상이 `aws_iam_saml_provider.keycloak_platform`, 두 `aws_iam_role`, 두
   `aws_iam_role_policy`뿐이고 `change=0`, `destroy=0`이다.
...
scripts/reconcile-keycloak-saml.sh --apply
scripts/reconcile-keycloak-saml.sh --check
```

`reconcile-keycloak-saml.sh`는 existing realm import를 실행하지 않는다. 새 SAML client,
그 client의 두 role, protocol mapper만 만들고, 기존 두 group에는 **새 client role 관계만**
추가한다. 기존 OIDC client, group 이름·속성·하위 group, 사용자 또는 사용자 membership은
변경하지 않는다. 같은 client ID가 이미 있거나 group이 정확히 하나가 아니면 fail-closed 한다.
...
1. `/platform-users` 및 `/platform-privileged` 각각으로 TOTP 포함 browser SAML 로그인 후
   오직 배정 role 하나만 AWS console에서 선택 가능한지 확인한다.
...
폐기는 먼저 Keycloak client만 전용 rollback으로 지워 새 SAML 진입을 멈춘 뒤, 별도 폐기
branch에서 `prevent_destroy` 두 block을 명시적으로 해제하고 `tofu destroy` plan을 검토한다.
